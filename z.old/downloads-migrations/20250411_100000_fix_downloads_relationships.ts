...
export async function up({ db }: MigrateUpArgs): Promise<void> {
    ...
    `);
      ...
      `);
			...
			// Parse counts safely
			const total = Number.parseInt(counts.rows[0]?.total as string) || 0;
			const bidirectional =
				Number.parseInt(bidirectionalCounts.rows[0]?.total as string) || 0;
		...
		for (const collection of collections) {
			...
			console.log(
				`${collection}: ${bidirectional}/${total} bidirectional relationships`,
			);
		}
		...
		console.log(
			"Creating view and function for handling UUID temporary tables...",
		);
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
    ...
    `);
		...
		await db.execute(sql`ROLLBACK;`);
		console.error(
			"Error in down migration for fix downloads relationships:",
			error,
		);
		throw error;
	}
