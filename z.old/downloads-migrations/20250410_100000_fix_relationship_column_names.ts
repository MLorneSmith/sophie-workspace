...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  try {
    ...
    for (const row of relsTables.rows) {
      ...
      else if (hasParentId.rows[0].exists && !hasUnderscoreParentId.rows[0].exists) {
      ...
      // If neither exists, create them both
        ...
        payload.logger.warn(`Neither parent_id nor _parent_id exists in ${tableName}`)
      }
    }
    ...
    // If _parent_id exists but parent_id doesn't
    if (sqoHasUnderscoreParentId.rows[0].exists && !sqoHasParentId.rows[0].exists) {
      payload.logger.info(`Adding parent_id column to survey_questions_options`)
    ...
    // If parent_id exists but _parent_id doesn't
    else if (sqoHasParentId.rows[0].exists && !sqoHasUnderscoreParentId.rows[0].exists) {
      payload.logger.info(`Adding _parent_id column to survey_questions_options`)
    ...
    // If neither exists, warn
      ...
      payload.logger.warn(`Neither parent_id nor _parent_id exists in survey_questions_options`)
    }

    // Commit transaction
    await db.execute(sql`COMMIT;`)

    payload.logger.info('Relationship column fix migration completed successfully')
  } catch (error) {
...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
  ...
  try {
    ...
    if (sqoHasUnderscoreParentId.rows[0].exists && sqoHasParentId.rows[0].exists) {
      // We'll keep _parent_id as the standard for down migration
      // So if parent_id was added by this migration, remove it
      payload.logger.info(`Removing parent_id column from survey_questions_options`)
    ...
    payload.logger.info('Relationship column fix migration reversion completed successfully')
  } catch (error) {
    await db.execute(sql`ROLLBACK;`)
    payload.logger.error('Relationship column fix migration reversion failed:', error)
    throw error
  }
}
