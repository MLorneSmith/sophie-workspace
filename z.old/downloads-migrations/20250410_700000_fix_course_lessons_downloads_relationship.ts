...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
    ...
    `);
		...
		const oldTableExistsValue = (oldTableExists.rows[0] as { exists: boolean })
			.exists;
    ...
    `);
		const hasDownloadsIdColumnValue = (
			hasDownloadsIdColumn.rows[0] as { exists: boolean }
		).exists;
      ...
      `);
		...
		console.log(
			"Migration completed: Fixed relationship between course lessons and downloads",
		);
	} catch (error) {
		// Rollback on error
		await db.execute(sql`ROLLBACK;`);
		console.error(
			"Error in course_lessons-downloads relationship fix migration:",
			error,
		);
		throw error;
	}
...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
    ...
    `);
		const tableExistsValue = (tableExists.rows[0] as { exists: boolean })
			.exists;
    ...
    `);
		...
		await db.execute(sql`ROLLBACK;`);
		console.error(
			"Error in down migration for course_lessons-downloads relationship fix:",
			error,
		);
		throw error;
	}
