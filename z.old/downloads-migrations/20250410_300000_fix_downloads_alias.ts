...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  try {
    ...
    for (const row of tables.rows) {
      ...
      if (/^[0-9a-f]{8}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{12}$/.test(tableName)) {
        ...
        if (!hasDownloadsId.rows[0].exists) {
          try {
            payload.logger.info(`Adding downloads_id column to table ${tableName}`)
            ...
            payload.logger.warn(`Could not add downloads_id column to ${tableName}: ${error}`)
          }
        }
      }
    }
    ...
    for (const row of courseLessonsTables.rows) {
      const tableName = row.tablename as string
      if (tableName !== 'course_lessons' && tableName !== 'course_lessons_rels') {
        const hasDownloadsId = await db.execute(sql`
          SELECT EXISTS (
            SELECT FROM information_schema.columns 
            WHERE table_schema = 'payload' 
...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
  ...
  payload.logger.info('No action needed for down migration of fix downloads alias')
}
