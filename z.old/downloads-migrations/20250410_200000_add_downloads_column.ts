...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  try {
    ...
    if (!hasDownloadsId.rows[0].exists) {
      payload.logger.info('Adding downloads_id column to course_lessons_rels table')
    ...
    if (!hasDownloadsIdId.rows[0].exists) {
      payload.logger.info('Adding downloads_id_id column to course_lessons table')
      await db.execute(sql`
        ALTER TABLE payload.course_lessons
    ...
    payload.logger.info('Add downloads column migration completed successfully')
  } catch (error) {
...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
  ...
  try {
    ...
    if (hasDownloadsId.rows[0].exists) {
      payload.logger.info('Removing downloads_id column from course_lessons_rels table')
      await db.execute(sql`
        ALTER TABLE payload.course_lessons_rels
    ...
    if (hasDownloadsIdInLessons.rows[0].exists) {
      payload.logger.info('Removing downloads_id column from course_lessons table')
      await db.execute(sql`
        ALTER TABLE payload.course_lessons
        DROP COLUMN downloads_id;
      `)
    ...
    if (hasDownloadsIdId.rows[0].exists) {
      payload.logger.info('Removing downloads_id_id column from course_lessons table')
      await db.execute(sql`
        ALTER TABLE payload.course_lessons
    ...
    payload.logger.info('Add downloads column migration reversion completed successfully')
  } catch (error) {
    await db.execute(sql`ROLLBACK;`)
    payload.logger.error('Add downloads column migration reversion failed:', error)
    throw error
  }
}
