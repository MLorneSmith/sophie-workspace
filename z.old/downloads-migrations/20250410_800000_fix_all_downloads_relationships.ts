  ...
  try {
    ...
    for (const row of relationTables.rows) {
      ...
      if (tableName.includes('rels')) {
        ...
        const newTableExistsValue = (newTableExists.rows[0] as { exists: boolean }).exists
        ...
        if (!newTableExistsValue) {
          ...
          const hasOrderColumnValue = (hasOrderColumn.rows[0] as { exists: boolean }).exists
    ...
    for (const row of contentTables.rows) {
      ...
      const hasDownloadsColumnValue = (hasDownloadsColumn.rows[0] as { exists: boolean }).exists
      ...
      if (hasDownloadsColumnValue) {
        ...
        const relationTableExistsValue = (relationTableExists.rows[0] as { exists: boolean }).exists
    ...
    try {
      ...
      console.warn(`Warning in UUID tables section: ${err.message || String(err)}`)
    }

    // Commit transaction
    await db.execute(sql`COMMIT;`)
    console.log('Migration completed: Fixed all downloads relationships across collections')
  } catch (error) {
    // Rollback on error
