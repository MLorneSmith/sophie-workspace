...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  try {
    ...
    console.log('Migration completed: Created relationship between course lessons and downloads')
  } catch (error) {
    // Rollback on error
...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
  try {
    ...
    // Rollback on error
    await db.execute(sql`ROLLBACK;`)
    console.error('Error in down migration for lesson-downloads relationship:', error)
    throw error
  }
}
