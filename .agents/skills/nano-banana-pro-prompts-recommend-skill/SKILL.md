...
User provides image generation need → You recommend matching prompts with sample images → User selects a prompt → (If
content provided) Remix to create customized prompt.

### Two Usage Modes

1. **Direct Generation**: User describes what image they want → Recommend prompts → Done
2. **Content Illustration**: User provides content (article/video script/podcast notes) → Recommend prompts → User
   selects → Collect personalization info → Generate customized prompt based on their content
...
| File | Category | Count |
|------|----------|-------|
| `profile-avatar.json` | Profile / Avatar | 860 |
| `social-media-post.json` | Social Media Post | 5010 |
| `infographic-edu-visual.json` | Infographic / Edu Visual | 401 |
| `youtube-thumbnail.json` | YouTube Thumbnail | 140 |
| `comic-storyboard.json` | Comic / Storyboard | 239 |
| `product-marketing.json` | Product Marketing | 2711 |
| `ecommerce-main-image.json` | E-commerce Main Image | 282 |
| `game-asset.json` | Game Asset | 261 |
| `poster-flyer.json` | Poster / Flyer | 403 |
| `app-web-design.json` | App / Web Design | 146 |
| `others.json` | Uncategorized | 739 |
...
**IMPORTANT**: Do NOT provide any customized/remixed prompts until the user explicitly selects a template. The
...
**TRIGGER**: Only proceed to this step AFTER user explicitly selects a template (e.g., "I choose 1", "Let's go with
the second one", "Option 2").
...
- Remixed: "Professional man in his 30s in modern office, contemplative expression, soft dramatic lighting, startup
  environment with whiteboard in background"
...
- Remixed: "Futuristic cityscape with holographic AI assistants walking alongside humans, warm neon lights suggesting
  harmony, cyberpunk style with optimistic undertones"
