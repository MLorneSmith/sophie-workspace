...
if (!CMS_HOST || !CMS_API_KEY) {
	console.error(
		"Error: CMS_HOST and CMS_API_KEY environment variables are required",
	);
	process.exit(1);
}
...
async function fetchAllPrompts(): Promise<CMSPrompt[]> {
	...
	while (hasMore) {
		...
		console.log(
			`Fetched page ${page}/${data.totalPages} (${allPrompts.length}/${data.totalDocs} prompts)`,
		);
