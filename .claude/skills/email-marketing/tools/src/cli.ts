...
export function createCli(): Command {
	...
	program
		...
		.action(async (options: RunExportOptions) => {
...
async function runAuth(options: { credentials: string }): Promise<void> {
...
async function listLabels(options: { credentials: string }): Promise<void> {
...
async function runExport(options: RunExportOptions): Promise<void> {
	...
	try {
		...
		if (!fs.existsSync(options.credentials)) {
			spinner.fail(chalk.red("Credentials file not found"));
			console.log(
				chalk.yellow(`Run: email-export auth -c ${options.credentials}`),
			);
			return;
		}
		...
		if (options.threadId) {
			...
			rawEmails = await gmailClient.fetchEmails(
				exportOptions,
				(current, total) => {
		...
		const exportedEmails: ExportedEmail[] = [];
		...
		for (let i = 0; i < rawEmails.length; i++) {
			...
			const extractAttachments = (
				parts: typeof rawEmail.payload.parts,
			): void => {
			...
			const exportedEmail: ExportedEmail = {
				metadata: {
					id: rawEmail.id,
		...
		const result = await writeEmailsToYaml(
			exportedEmails,
		...
		if (result.success) {
			writeSpinner.succeed(
				chalk.green(
					`Exported ${result.emailsExported} emails to ${result.outputDirectory}`,
				),
			);
		} else {
			writeSpinner.warn(
				chalk.yellow(
					`Exported ${result.emailsExported} emails with ${result.errors.length} errors`,
				),
			);
			console.log();
		...
		console.log(
			chalk.dim("3. Use the annotated files with the style capture system"),
		);
	} catch (error) {
