...
- **Dynamic Context Loading**: 40-60% token reduction using context-loader.cjs script
...
- Use context-loader.cjs script for dynamic context loading
...
2. **Rich Command Analysis with AST-grep**:
   ```bash
   # Analyze command file to extract rich metadata
   COMMAND_METADATA=$(node .claude/scripts/command-analyzer.cjs "$FILE_PATH" --json)

   # Extract key patterns for enhanced context selection
   ...
   # Build enriched query with AST-extracted patterns
   ENRICHED_QUERY="$TOOL_PATTERNS $AGENT_SPECIALISTS $TECHNOLOGIES $WORKFLOW_PHASES enhancement optimization"
   ```
3. **Execute Dynamic Context Loading with Enhanced Query**:
   ```bash
   # Use context-loader.cjs with rich metadata from AST analysis
   ...
   # Read each returned context file
...
<context_loading>
**Advanced Context Loading Pattern:**

```bash
# Dynamic context selection using context-loader.cjs
load_dynamic_context() {
  local query="$1"
  local command_type="$2"

  # Execute context loader
  local context_output=$(node .claude/scripts/context-loader.cjs \
    --query="$query" \
  ...
  # Process output and read files
  echo "$context_output" | while IFS= read -r line; do
    if [[ $line =~ ^Read ]]; then
      local file_path=$(echo "$line" | sed 's/Read //')
...
# Usage in workflow
EXTRACTED_QUERY="$(extract_command_purpose)"
load_dynamic_context "$EXTRACTED_QUERY enhancement optimization" "command-enhance"
```
</context_loading>
...
<error_handling>
### Common Issues
1. **Context Loader Failure**: Fallback to manual context selection
...
- context-loader.cjs integration
- Post-enhancement validation

**Requirements:**
- Target command file must exist
- context-loader.cjs script must be available
