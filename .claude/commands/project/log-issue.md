---
title: Log Issue Command
description: Create comprehensive issue reports with automated diagnostics and GitHub integration
usage: "/log-issue [output_format]"
...
**Action**: Create comprehensive issue reports with automated diagnostics, error categorization, and GitHub integration.

**Usage**: `/log-issue [output_format]` (default: github)

**Output Formats**:
- `github`: Create GitHub issue with automated diagnostics (default)
- `local`: Save to `.claude/z.archive/issues/` only (no GitHub issue)
- `file`: Same as `local` (alternative syntax)

**GitHub-First Workflow**: GitHub issues serve as the single source of truth with automated local synchronization via debug-issue command.
...
# PRIME Framework Implementation

## P - PREPARATION (ROLE)

**Adopt specialized issue tracking and error analysis expertise:**
...
2. **Activate expert capabilities**:
   ...
   - Diagnostic tool orchestration and analysis
   - Bug tracking workflow optimization
   - Cross-system error correlation and pattern recognition
   - Automated issue lifecycle management

3. **Initialize diagnostic toolchain**:
   - Browser automation for UI issue reproduction
   - Database analysis for performance and data issues
   - API monitoring for service integration problems
