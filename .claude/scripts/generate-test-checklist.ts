...
const PRIORITY_PATHS = {
  ...
  2: {
    'Payload Custom Collections': [
      'apps/payload/src/collections/*.ts',
    ],
    'Payload Custom Blocks': [
      'apps/payload/src/blocks/**/Field.tsx',
    ],
    'Payload Enhanced Systems': [
      'apps/payload/src/lib/*.ts',
    ],
  },
  3: {
    'Custom API Routes': [
    ...
    'Kanban System': [
      'apps/web/app/home/(user)/kanban/_lib/**/*.ts',
    ],
  },
};
...
async function analyzeFile(filePath: string): Promise<Partial<TestTarget>> {
  ...
  const exportedArrowFunctions = (content.match(/export\s+const\s+\w+\s+=\s+(async\s+)?\(/g) || []).length;
  const functionCount = exportedFunctions + exportedArrowFunctions;
  
  // Detect dependencies
  const dependencies: string[] = [];
  if (content.includes('createAIGatewayClient')) dependencies.push('AI Gateway');
  if (content.includes('supabase')) dependencies.push('Database');
  if (content.includes('fetch(')) dependencies.push('External API');
  if (content.includes('fs.') || content.includes('readFile')) dependencies.push('File System');
...
async function generateChecklist() {
  ...
  for (const [priorityLevel, categories] of Object.entries(PRIORITY_PATHS)) {
    const priority = parseInt(priorityLevel);
...
async function generateMarkdownChecklist(targets: TestTarget[]) {
  ...
  const coverage = totalFiles > 0 ? ((filesWithTests / totalFiles) * 100).toFixed(1) : '0';
  ...
  for (const [group, groupTargets] of Object.entries(grouped)) {
    ...
    for (const target of groupTargets) {
      ...
      if (target.hasTests && target.testFilePath) {
        const relativeTestPath = target.testFilePath.replace(process.cwd() + '/', '');
