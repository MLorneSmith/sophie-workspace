...
class CommandAnalyzer {
  ...
  extractToolPatterns(content) {
    ...
    const readRegex = /(?:Read|read)\s+([./][\w/.-]+\.(?:md|json|ts|tsx|js|jsx))/gi;
  ...
  extractAgentPatterns(content) {
    ...
    const specialistRegex = /([a-z]+-(?:expert|specialist|engineer|architect|writer|reviewer|investigator|orchestrator|loader|optimizer))/gi;
    let match;
    ...
    if (delegationMatch) {
      ...
      if (entries) {
        for (const entry of entries) {
          ...
          if (agentList) {
            agents.delegationMatrix[key.trim()] = agentList.map(a => a.replace(/['"]/g, ''));
          }
        }
      }
    }
  ...
  extractWorkflowPhases(content) {
    ...
    phases.initialization = /<initialization>|## \d+\.\s*Initialization/i.test(content);
    phases.analysis = /<(?:task_)?analysis>|## \d+\.\s*Analysis/i.test(content);
    phases.implementation = /<implementation>|## \d+\.\s*Implementation/i.test(content);
    phases.validation = /<validation>|## \d+\.\s*Validation/i.test(content);
    phases.delivery = /<(?:output_)?delivery>|## \d+\.\s*(?:Output|Delivery)/i.test(content);
    ...
    while ((match = customPhaseRegex.exec(content)) !== null) {
      const phase = match[1];
      const standardPhases = ['discovery', 'initialization', 'analysis', 'implementation', 'validation', 'delivery'];
  ...
  extractCodePatterns(content) {
    ...
    const fileTypeRegex = /\.(ts|tsx|js|jsx|json|md|css|scss|sql|yaml|yml|env)/gi;
  ...
  extractContextPatterns(content) {
    ...
    const essentialSection = content.match(/## Essential Context[\s\S]*?(?=##|$)/);
    if (essentialSection) {
      let match;
...
async function main() {
  ...
  try {
    ...
    if (outputJson) {
      ...
      console.log(`Delegated Agents: ${metadata.tools.task.join(', ') || 'None'}`);
      console.log(`Specialists: ${metadata.agents.specialists.length}`);
      console.log(`Active Phases: ${metadata.phases.activeCount}`);
      console.log(`Technologies: ${metadata.codePatterns.technologies.join(', ') || 'None'}`);
      console.log(`Essential Context: ${metadata.contextPatterns.essentialFiles.length} files`);
      console.log(`Uses Dynamic Context: ${metadata.contextPatterns.contextLoader ? 'Yes' : 'No'}`);
    } else {
      ...
      console.log(`Sections: ${metadata.structure.sections.map(s => s.title).join(', ')}`);
      console.log(`Has Essential Context: ${metadata.structure.hasEssentialContext}`);
      ...
      console.log(`Specialists: ${metadata.agents.specialists.join(', ') || 'None'}`);
      if (Object.keys(metadata.agents.delegationMatrix).length) {
        console.log('Delegation Matrix:');
        for (const [key, agents] of Object.entries(metadata.agents.delegationMatrix)) {
      ...
      console.log(`Frameworks: ${metadata.codePatterns.frameworks.join(', ') || 'None'}`);
      console.log(`Technologies: ${metadata.codePatterns.technologies.join(', ') || 'None'}`);
      console.log(`File Types: ${metadata.codePatterns.fileTypes.join(', ') || 'None'}`);
      console.log('');
      console.log('## Context Patterns');
      console.log(`Essential Files: ${metadata.contextPatterns.essentialFiles.join(', ') || 'None'}`);
      console.log(`Uses Context Loader: ${metadata.contextPatterns.contextLoader}`);
