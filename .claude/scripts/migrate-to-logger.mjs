...
function addLoggerImport(content, serviceName) {
	...
	if (imports.length > 0) {
		...
		return (
			content.slice(0, insertIndex) + loggerImport + content.slice(insertIndex)
		);
	} else {
...
function isInAsyncContext(content, position) {
	...
	const functionMatch = beforeContent.match(
	...
	const asyncFunctionRegex =
		/async\s+(?:function\s+\w+|\w+\s*=\s*async|(?:\(.*?\)|\w+)\s*=>\s*(?:async\s*)?)\s*\{[^}]*$/;
...
function replaceConsoleStatements(content, filePath) {
	...
	const multiArgPattern =
		/((?:await getLogger\(\))|logger)\.(info|error|warn|debug)\s*\(\s*([^,)]+)\s*,\s*([^)]+)\s*\)/g;

	modifiedContent = modifiedContent.replace(
		multiArgPattern,
		(match, logger, method, message, args) => {
...
async function processFile(filePath) {
	try {
		...
		const modifiedContent = addLoggerImport(content, serviceName);

		// Replace console statements
		const { content: finalContent, count } = replaceConsoleStatements(
			modifiedContent,
			filePath,
		);
...
async function main() {
	...
	if (args.length === 0) {
		...
		// biome-ignore lint/suspicious/noConsole: Usage information
		console.log(
			"   or: node migrate-to-logger.mjs --from-file <file-list.txt>",
		);
		process.exit(1);
	}
