...
class TestReliabilityValidator {
	...
	async validate(iterations = 10, testType = 'all') {
		...
		this.results.iterations = Math.min(iterations, this.results.successful + this.results.failed);
	...
	async runTestIteration(testType) {
		...
		// Build test command
		let testArgs = ['node', '.claude/scripts/test-controller.cjs'];
		...
		return new Promise((resolve) => {
			...
			proc.on('close', (code) => {
				...
				result.infrastructureFailure = this.detectInfrastructureFailure(result.output);
	...
	analyzeIterationResult(result, iteration) {
		...
		this.results.averageDuration = 
			((this.results.averageDuration * (totalRuns - 1)) + result.duration) / totalRuns;
	}
	...
	async generateReport() {
		...
		report.push(`Status: ${this.results.successRate >= this.targetSuccessRate ? '✅ PASSED' : '❌ FAILED'}`);
		
		// Performance
		report.push('\n⏱️ PERFORMANCE');
		report.push('─'.repeat(30));
		report.push(`Average Duration: ${this.results.averageDuration.toFixed(0)}s`);
		const totalTime = Math.round((new Date(this.results.endTime) - new Date(this.results.startTime)) / 1000);
		...
		if (this.results.failed > 0) {
			report.push('\n❌ FAILURE ANALYSIS');
			report.push('─'.repeat(30));
			report.push(`Infrastructure Failures: ${this.results.infrastructureFailures}`);
			...
			if (Object.keys(this.results.failurePatterns).length > 0) {
				report.push('\nFailure Patterns:');
				for (const [pattern, count] of Object.entries(this.results.failurePatterns)) {
		...
		if (this.results.successRate < this.targetSuccessRate) {
			...
			report.push(`   Achieved ${this.results.successRate.toFixed(1)}% success rate`);
		}
		...
		// Save report to file
		const reportPath = path.join(process.cwd(), '.claude/test-reliability-report.txt');
		await fs.writeFile(reportPath, reportText + '\n');
		console.log(`\n📁 Report saved to: ${reportPath}`);
		
		// Save JSON results
		const jsonPath = path.join(process.cwd(), '.claude/test-reliability-results.json');
	...
	async quickCheck() {
		...
		const checks = [
			{ name: 'Supabase Status', cmd: 'cd apps/e2e && npx supabase status 2>&1 | grep -E "(running|API URL)" | head -1' },
			{ name: 'Port 3000', cmd: 'lsof -ti:3000 || echo "Free"' },
			{ name: 'Port 3020', cmd: 'lsof -ti:3020 || echo "Free"' },
			{ name: 'Playwright Processes', cmd: 'pgrep -f playwright || echo "None"' },
			{ name: 'Lock Files', cmd: 'ls /tmp/.claude_test_locks 2>/dev/null | wc -l' },
		];
		...
		for (const check of checks) {
			try {
				...
				const isHealthy = !status.includes('error') && !status.includes('failed');
				console.log(`${isHealthy ? '✅' : '❌'} ${check.name}: ${status.substring(0, 50)}`);
			} catch (error) {
...
if (require.main === module) {
	...
	(async () => {
		if (args.includes('--validate') || args.includes('-v')) {
			...
			const testType = args.find(a => ['--unit', '--e2e'].includes(a))?.replace('--', '') || 'all';
			...
			console.log('  --iterations=N       Number of test iterations (default: 10)');
			console.log('  --unit              Test unit tests only');
			console.log('  --e2e               Test E2E tests only');
			console.log('');
			console.log('Examples:');
			console.log('  node test-reliability-validator.cjs --validate --iterations=5');
