...
const tokenCacheByEnv: Record<string, TokenCache> = {};
 ...
 */
export async function getEnhancedPayloadClient(
  ...
  const client: PayloadClient = {
    ...
    batchCreate: async ({ collection, data, batchSize = 10 }) => {
      ...
      for (let i = 0; i < data.length; i += batchSize) {
        ...
        console.log(
          `Processing batch ${i / batchSize + 1} of ${Math.ceil(data.length / batchSize)}`,
        );
