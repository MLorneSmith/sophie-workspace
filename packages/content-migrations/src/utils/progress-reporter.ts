...
export class ProgressReporter extends EventEmitter {
  ...
  constructor(private readonly reportingIntervalMs: number = 5000) {
    super();
    this.startTime = new Date();
  }

  registerStep(id: string, name: string, totalItems?: number): void {
    this.steps.set(id, {
      id,
      name,
      totalItems,
  ...
  printProgress(): void {
        ...
        case 'running':
          ...
          if (step.totalItems && step.processedItems !== undefined) {
            const percent = Math.round(
              (step.processedItems / step.totalItems) * 100,
            );
            progressInfo = chalk.white(
              ` (${step.processedItems}/${step.totalItems}, ${percent}%)`,
            );
          }
          break;
        case 'completed':
    ...
    const runningCount =
      totalSteps - pendingCount - completedCount - failedCount;
