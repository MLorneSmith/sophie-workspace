    ...
    try {
      ...
      for (const collection of collectionsToTest) {
        ...
        try {
          ...
          const missingColumns = collection.requiredFields.filter(
            (field) => !columns.includes(field),
          );

          if (missingColumns.length > 0) {
            console.warn(
              `Warning: Table is missing required columns: ${missingColumns.join(
                ', ',
              )}`,
            );
          } else {
            console.log(
              `Table has all required columns: ${collection.requiredFields.join(
                ', ',
              )}`,
            );
          }
          ...
          const docsResult = await client.query(
            `SELECT * FROM payload.${collection.name} LIMIT 1`,
          );
          ...
          if (docsResult.rows.length > 0) {
            ...
            const missingFields = collection.requiredFields.filter(
              (field) => doc[field] === null || doc[field] === undefined,
            );

            if (missingFields.length > 0) {
              console.warn(
                `Warning: Document is missing values for required fields: ${missingFields.join(
                  ', ',
                )}`,
              );
            } else {
              console.log(
                `Document has values for all required fields: ${collection.requiredFields.join(
                  ', ',
                )}`,
              );
            }
          ...
          const countResult = await client.query(
            `SELECT COUNT(*) FROM payload.${collection.name}`,
          );
          console.log(
            `Collection ${collection.name} exists with ${countResult.rows[0].count} documents`,
          );
        } catch (error) {
      ...
      try {
        const lessonsResult = await client.query(
          `SELECT id, course_id FROM payload.course_lessons LIMIT 1`,
        );
        ...
        if (lessonsResult.rows.length > 0) {
          ...
          if (lesson.course_id) {
            console.log(
              `Lesson ${lesson.id} has course_id: ${lesson.course_id}`,
            );

            // Try to find the course
            const coursesResult = await client.query(
              `SELECT id, title FROM payload.courses WHERE id = $1`,
              [lesson.course_id],
            );

            if (coursesResult.rows.length > 0) {
              console.log(
                `Found related course: ${coursesResult.rows[0].title}`,
              );
            } else {
              console.warn(
      ...
      try {
        const questionsResult = await client.query(
          `SELECT id, quiz_id FROM payload.quiz_questions LIMIT 1`,
        );
        ...
        if (questionsResult.rows.length > 0) {
          ...
          if (question.quiz_id) {
            ...
            const quizzesResult = await client.query(
              `SELECT id, title FROM payload.course_quizzes WHERE id = $1`,
              [question.quiz_id],
            );
