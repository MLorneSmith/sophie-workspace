/**
 * Script to repair courses relationships in the database
 */
...
/**
 * Repairs courses relationships in the database
 */
async function repairCoursesRelationships() {
  ...
  try {
    ...
    try {
      ...
      // Get all tables with courses relationships
      const collections = ['courses'];

      for (const collection of collections) {
        console.log(`Repairing courses relationships for ${collection}...`);
        ...
        for (const field of relationshipFields.rows) {
          ...
          // Update relationship tables with courses_id
          await client.query(`
            UPDATE payload.${collection}_rels
            SET courses_id = ${collection}.${fieldName}
      ...
      // Create a function to handle dynamic UUID tables for courses
      console.log('Creating dynamic table handler function for courses...');
      await client.query(`
        -- Create a function to handle dynamic UUID tables for courses
        CREATE OR REPLACE FUNCTION payload.handle_dynamic_courses_table(uuid_table text)
        RETURNS void AS $$
        BEGIN
      ...
      console.log('Courses relationships repaired!');
    } finally {
      client.release();
    }
  } finally {
...
// Run the script
repairCoursesRelationships().catch((error) => {
  console.error('Failed to repair courses relationships:', error);
  process.exit(1);
});
