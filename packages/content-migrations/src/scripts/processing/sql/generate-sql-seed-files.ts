...
`;
    ...
    console.log(
      `Quizzes SQL generated successfully (${Object.keys(QUIZZES).length} quizzes).`,
    );
  } catch (error) {
    ...
    for (const quizSlug in QUIZZES) {
      ...
      if (quiz.questions && quiz.questions.length > 0) {
        ...
        quiz.questions.forEach((question, index) => {
          ...
          const optionsWithIds = question.options.map(
...
INSERT INTO payload.quiz_questions (
  ...
  correct_answer = EXCLUDED.correct_answer;`; // Also update correct_answer on conflict
    ...
    console.log(
      `Questions SQL generated successfully from source of truth (${allInserts.length} questions).`,
    );
  } catch (error) {
    console.error(
      'Error generating questions SQL from source of truth:',
      error,
    );
    throw error;
  }
