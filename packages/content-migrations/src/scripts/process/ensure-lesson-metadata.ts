...
const CREATOR_SCRIPT_PATH = path.resolve(
  __dirname,
  '../create-full-lesson-metadata.ts',
);

/**
 * Check if the lesson metadata YAML file exists and create it if needed
 */
export async function ensureLessonMetadata(): Promise<boolean> {
  ...
  if (fs.existsSync(METADATA_PATH)) {
    ...
    if (fileAgeInDays > 7) {
      console.warn(
        `WARNING: Lesson metadata file is ${Math.floor(fileAgeInDays)} days old.`,
      );
