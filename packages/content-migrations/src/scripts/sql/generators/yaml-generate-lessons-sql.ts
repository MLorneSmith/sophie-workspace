 ...
 */
export function generateLessonsSqlFromYaml(lessonsDir: string): string {
  ...
  let lessonMetadata;
  try {
    lessonMetadata = yaml.load(fs.readFileSync(METADATA_PATH, 'utf8')) as {
      lessons: any[];
    };
  ...
  for (const lesson of lessonMetadata.lessons) {
    ...
    if (fs.existsSync(mdocFilePath)) {
      try {
        ...
        console.error(
          `Error processing content for lesson ${lessonSlug}:`,
          error,
        );
      }
    } else {
...
if (
  process.argv[1]?.endsWith('yaml-generate-lessons-sql.ts') ||
  ...
  const LESSONS_DIR =
    ...
    path.resolve(
      process.cwd(),
      'packages/content-migrations/src/data/raw/courses/lessons',
    );
