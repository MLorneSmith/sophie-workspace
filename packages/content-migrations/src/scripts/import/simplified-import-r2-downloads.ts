...
const downloadMap: Record<
  string,
  { filename: string; url: string; description?: string }
> = {
  'slide-templates': {
...
export async function generateInsertSQL(): Promise<string> {
  ...
  for (const [key, id] of Object.entries(DOWNLOAD_ID_MAP)) {
    ...
    if (!downloadInfo) {
      console.warn(
        `Warning: No download info found for key ${key}, using defaults`,
      );
      continue;
    }
...
export async function runImport(): Promise<void> {
  try {
    console.log(
      'Starting to generate SQL statements for downloads import with consistent IDs',
    );
    ...
    console.log(
      `- ${Object.keys(DOWNLOAD_ID_MAP).length} total downloads with consistent IDs\n`,
    );
