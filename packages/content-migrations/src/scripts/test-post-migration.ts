  ...
  try {
    ...
    for (const file of mdocFiles) {
      try {
        ...
        fs.writeFileSync(
          path.join(debugDir, `${slug}-content.txt`),
          mdContent,
          'utf8',
        );
        ...
        console.log(
          `Converted to Lexical format: ${resultStr.length} characters`,
        );

        // Verify if the content was truncated
        if (resultStr.length < 500 && mdContent.length > 1000) {
          console.error(
            `WARNING: Possible truncation in ${slug} - markdown: ${mdContent.length} chars, lexical: ${resultStr.length} chars`,
          );
        } else {
