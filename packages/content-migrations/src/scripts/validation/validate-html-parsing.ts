 ...
 */
function findMatchingLesson(lessons: any[], title: string): any | undefined {
  ...
  if (title.includes(':')) {
    ...
    const similarLessons = lessons.filter(
      (l) =>
...
async function validateHtmlParsing() {
  ...
  for (const lessonSection of Array.from(lessonSections)) {
    ...
    if (yamlLesson && yamlLesson.todoFields) {
      ...
      if (
        todoSections['course project'] &&
        !yamlLesson.todoFields.courseProject
      ) {
        hasMatchingContent = false;
        console.log(
          `  Missing course project content for "${yamlLesson.title}"`,
        );
      }
      ...
      if (yamlLesson) {
        ...
        console.log(
          `  No matching lesson found for HTML title "${lessonTitle}"`,
        );
      }
    }
  }
  ...
  console.log(
    `Lessons with all matching content: ${lessonsWithMatchingContent}`,
  );

  const validationSuccess =
    lessonsWithMatchingContent === lessonSections.length;
