...
export async function fixDownloadR2Mappings(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Found ${existingDownloads.rows.length} downloads with placeholder values`,
    );
    ...
    for (const [id, data] of Object.entries(downloadMappings)) {
      try {
        ...
        const existingDownload = existingDownloads.rows.find(
          (row) => row.id === id,
        );
        ...
        if (existingDownload) {
          ...
          console.log(
            `Skipping download ${id} - not found in database or already updated`,
          );
        }
      } catch (error) {
    ...
    const unmappedPlaceholders = existingDownloads.rows.filter(
      (row) => !Object.keys(downloadMappings).includes(row.id),
    );

    if (unmappedPlaceholders.length > 0) {
      console.log(
        `Found ${unmappedPlaceholders.length} unmapped placeholder downloads:`,
      );
