...
// Types that match our enhanced verification report
interface EnhancedVerificationReport {
  valid: boolean;
...
async function main() {
  console.log(
    chalk.blue('🚀 Starting UUID table verification (improved version)...'),
  );
  ...
  if (!databaseUrl) {
    if (!envLoaded) {
      ...
      console.log(
        chalk.yellow(`Using default connection string: ${databaseUrl}`),
      );
    } else {
      console.error(
        chalk.red(
  ...
  try {
    ...
    if (tables.length === 0) {
      console.warn(
        chalk.yellow(
          '⚠️ No UUID pattern tables or relationship tables found in the database.',
        ),
      );
      console.log(
        chalk.blue(
          'ℹ️ This may be normal for a fresh installation, continuing with verification...',
        ),
      );
    } else {
    ...
    if (report.totalTables > 0) {
      ...
      const criticalIssuePercent =
        report.totalTables > 0
          ? Math.round(
              (report.missingCriticalColumnsTables / report.totalTables) * 100,
            )
          : 0;

      const optionalIssuePercent =
        report.totalTables > 0
          ? Math.round(
              (report.missingOptionalColumnsTables / report.totalTables) * 100,
            )
          : 0;
      ...
      if (report.missingCriticalColumnsTables > 0) {
        console.log(
          chalk.red(
            `❌ Tables with missing critical columns: ${report.missingCriticalColumnsTables} (${criticalIssuePercent}%)`,
          ),
        );
      } else {
      ...
      if (report.missingOptionalColumnsTables > 0) {
        console.log(
          chalk.yellow(
            `⚠️ Tables with missing optional columns only: ${report.missingOptionalColumnsTables} (${optionalIssuePercent}%)`,
          ),
        );
      } else {
      ...
      if (report.missingCriticalColumnsTables > 0) {
        ...
        report.tableDetails
          ...
          .forEach((detail) => {
            console.log(
              chalk.red(
                `  - ${detail.tableName}: missing ${detail.missingCriticalColumns.join(', ')}`,
              ),
            );
          });
      }
      ...
      if (report.missingOptionalColumnsTables > 0) {
        ...
        report.tableDetails
          ...
          .forEach((detail) => {
            console.log(
              chalk.yellow(
                `  - ${detail.tableName}: missing ${detail.missingOptionalColumns.join(', ')}`,
              ),
            );
          });
      }
    ...
    if (report.criticalValid) {
      if (report.valid) {
        ...
        console.log(
          chalk.yellow(
            '   This is acceptable for the migration to continue, but you may want to fix later.',
          ),
        );
        console.log(
          chalk.blue(
            '   Run the UUID table repair script to add missing columns: pnpm run uuid:repair',
          ),
        );
      }
      ...
      console.log(
        chalk.blue(
          '   Run the UUID table repair script to fix this issue: pnpm run uuid:repair',
        ),
      );
      ...
      if (process.env.ALLOW_WARNINGS === 'true') {
        console.log(
          chalk.yellow(
            '   Continuing despite critical column issues due to ALLOW_WARNINGS=true',
          ),
        );
        process.exit(0);
      } else {
    ...
    if (process.env.ALLOW_ERRORS === 'true') {
      console.log(
        chalk.yellow('Continuing despite errors due to ALLOW_ERRORS=true'),
      );
      process.exit(0);
    } else {
      process.exit(1);
