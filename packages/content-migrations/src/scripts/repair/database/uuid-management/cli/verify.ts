...
import { getVerificationReport, verifyUuidTables } from '../verification.js';
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Tables with missing columns: ${report.missingColumnsTables} (${Math.round(
        (report.missingColumnsTables / report.totalTables) * 100,
      )}%)`,
    );
    ...
    if (report.missingColumnsTables > 0) {
      ...
      report.tableDetails
        ...
        .forEach((detail) => {
          console.log(
            `- ${detail.tableName}: missing ${detail.missingColumns.join(
              ', ',
            )}`,
          );
        });
