  ...
  try {
    ...
    try {
      ...
      if (uuidTables.length === 0) {
        console.log(
          chalk.yellow('No UUID tables found in the payload schema.'),
        );
        return { success: true, message: 'No UUID tables found' };
      }

      console.log(
        chalk.blue(`Found ${uuidTables.length} UUID tables to check`),
      );
      ...
      for (const tableName of uuidTables) {
        ...
        if (missingCriticalColumns.length > 0) {
          console.log(
            chalk.yellow(
              `Table ${tableName} is missing critical columns: ${missingCriticalColumns.map((c) => c.name).join(', ')}`,
            ),
          );
          ...
          for (const column of missingCriticalColumns) {
            try {
              ...
              console.error(
                chalk.red(`Error adding ${column.name} to ${tableName}:`),
                error,
              );
            }
          }
          ...
          console.log(
            chalk.green(`Table ${tableName} has all required critical columns`),
          );
        }
        ...
        for (const column of OPTIONAL_COLUMNS) {
          if (!existingColumns.includes(column.name)) {
            try {
              ...
              console.log(
                chalk.gray(
                  `Added optional column ${column.name} to ${tableName}`,
                ),
              );
              ...
              console.warn(
                chalk.yellow(
                  `Could not add optional column ${column.name} to ${tableName}:`,
                  error.message,
                ),
              );
            }
      ...
      try {
        ...
        console.log(
          chalk.green('Created uuid_tables_overview view for monitoring'),
        );
      } catch (error) {
        console.warn(
          chalk.yellow('Could not create overview view:', error.message),
        );
      }
      ...
      console.log(
        `Tables with missing critical columns: ${tablesWithMissingCriticalColumns}`,
      );
