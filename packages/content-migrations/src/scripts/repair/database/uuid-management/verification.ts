...
export async function verifyUuidTables(tables: UuidTable[]): Promise<boolean> {
  ...
  if (allValid) {
    console.log(
      `✅ All ${tablesChecked} UUID tables have the required columns`,
    );
  } else {
    console.error(
      `❌ ${tablesWithMissingColumns} out of ${tablesChecked} UUID tables are missing required columns`,
    );
  }
