...
import {
  findAllUuidTables,
  verifyTableColumns,
} from './enhanced-uuid-detection.js';
 ...
 */
export async function addMissingColumnsToTable(
  tableName: string,
...
export async function fixAllUuidTables() {
  try {
    ...
    for (const tableName of uuidTables) {
      // Verify columns
      const { hasAllColumns, missingColumns } =
        await verifyTableColumns(tableName);

      if (!hasAllColumns) {
        console.log(
          `Fixing table ${tableName}, missing columns: ${missingColumns.join(', ')}`,
        );
