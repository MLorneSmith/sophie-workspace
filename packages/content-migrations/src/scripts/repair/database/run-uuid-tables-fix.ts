...
async function runUuidTablesFix(): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        'SQL script executed successfully via Node.js pg client with corrected schema',
      );
      return true;
    } catch (dbError: any) {
      console.error(
      ...
      console.log(
        'Fallback approach with schema corrections completed successfully',
      );
      return true;
    } finally {
      await pool.end();
...
runUuidTablesFix()
  .then((success) => {
    if (success) {
      console.log(
        'UUID tables scan completed successfully with schema alignment',
      );
    } else {
