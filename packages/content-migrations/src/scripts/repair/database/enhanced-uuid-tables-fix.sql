-- Enhanced UUID Tables Fix Script
-- This adds additional columns like documentation_id and includes direct table fixes
...
-- Direct fix for critical tables - ensure they always have the required columns
DO $$
BEGIN
  ...
  -- Ensure course_lessons_rels has all required fields
  ALTER TABLE IF EXISTS payload.course_lessons_rels 
  ADD COLUMN IF NOT EXISTS documentation_id UUID;
  
  -- Ensure courses_rels has all required fields
  ALTER TABLE IF EXISTS payload.courses_rels 
  ADD COLUMN IF NOT EXISTS documentation_id UUID;
  
  -- Ensure course_quizzes_rels has all required fields
  ALTER TABLE IF EXISTS payload.course_quizzes_rels 
  ADD COLUMN IF NOT EXISTS documentation_id UUID;
  
  RAISE NOTICE 'Applied direct fixes to critical tables';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Error applying direct fixes: %', SQLERRM;
END
$$;
