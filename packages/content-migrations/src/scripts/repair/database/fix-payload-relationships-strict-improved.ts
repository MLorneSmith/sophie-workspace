 ...
 */
async function validateDatabaseState(client: pg.Client): Promise<boolean> {
  ...
  try {
    ...
    const { course_quizzes_exists, course_quizzes_rels_exists } =
      tablesResult.rows[0];

    if (!course_quizzes_exists || !course_quizzes_rels_exists) {
      console.error('❌ Required tables do not exist:');
      console.error(
        `- course_quizzes: ${course_quizzes_exists ? 'exists' : 'missing'}`,
      );
      console.error(
        `- course_quizzes_rels: ${course_quizzes_rels_exists ? 'exists' : 'missing'}`,
      );
      return false;
    }
    ...
    const { course_id_column_exists, parent_id_column_exists } =
      columnsResult.rows[0];

    if (!course_id_column_exists || !parent_id_column_exists) {
      console.error('❌ Required columns do not exist:');
      console.error(
        `- course_quizzes.course_id_id: ${course_id_column_exists ? 'exists' : 'missing'}`,
      );
      console.error(
        `- course_quizzes_rels._parent_id: ${parent_id_column_exists ? 'exists' : 'missing'}`,
      );
      return false;
    }
...
function prepareOperations(): {
  ...
  if (!beginTxnMatch || !commitMatch) {
    console.warn(
      'Could not identify transaction boundaries, using full script as single operation',
    );
    return {
  ...
  const txnSql = mainSql.substring(
    beginTxnMatch.index,
    commitMatch.index + commitMatch[0].length,
  );
  ...
  console.log(
    `Found transaction SQL (${txnSql.length} chars) and ${verificationQueries.length} verification queries`,
  );
...
export async function fixPayloadRelationshipsStrict(): Promise<boolean> {
  ...
  console.log(
    'Starting Payload relationship fix with strict UUID typing (improved version)...',
  );
  ...
  try {
    ...
    console.log(
      `Running ${sqlOperations.length} SQL operations and ${verificationQueries.length} verification queries`,
    );
    ...
    try {
      ...
      for (let i = 0; i < verificationQueries.length; i++) {
        ...
        const result = await executeWithRetry(
          client,
          query,
          `verification query ${i + 1}/${verificationQueries.length}`,
        );
        results.push(result);
      }
...
if (
  import.meta.url.endsWith('fix-payload-relationships-strict-improved.js') ||
