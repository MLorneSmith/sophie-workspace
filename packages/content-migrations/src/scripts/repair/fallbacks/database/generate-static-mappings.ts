...
export async function generateStaticMappings() {
  ...
  try {
    ...
    for (const relation of mappingRelations) {
      if (!relation.collection || !relation.field_name) continue;
    ...
    if (lessonQuizData && lessonQuizData.length > 0) {
      const lessonQuizMapping: Record<string, string[]> = {};
    ...
    const quizQuestionsData = await drizzle.execute(
      sql.raw(quizQuestionsQuery),
    );

    if (quizQuestionsData && quizQuestionsData.length > 0) {
      const quizQuestionsMapping: Record<string, string[]> = {};
      ...
      if (Object.keys(quizQuestionsMapping).length > 0) {
        const filePath = path.join(
          mappingsDir,
          'course_quizzes_questions.json',
        );
        fs.writeFileSync(
          filePath,
          JSON.stringify(quizQuestionsMapping, null, 2),
        );
    ...
    logger.info(
      { script: 'generate-static-mappings' },
      'Static relationship mappings generated successfully',
    );

    return { success: true };
  } catch (error) {
    logger.error(
      { script: 'generate-static-mappings', error },
      'Failed to generate static mappings',
    );
