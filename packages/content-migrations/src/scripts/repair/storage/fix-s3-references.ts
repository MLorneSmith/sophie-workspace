      ...
      try {
        ...
        if (download.thumbnail) {
          ...
          if (
            ...
            console.log(
              `Missing thumbnail for download: ${download.id} (${download.filename})`,
            );
          }
        }
        ...
        if (download.url) {
          ...
          if (fileKey && !(await fileExistsInS3('downloads', fileKey))) {
            ...
            if (matchingFile) {
              ...
              results.fixedUrls++;
              console.log(
                `Fixed URL for download: ${download.id} (${download.filename})`,
              );
            } else {
              ...
              needsUpdate = true;
              console.log(
                `No matching file found for download: ${download.id} (${download.filename})`,
              );
            }
          }
        }
