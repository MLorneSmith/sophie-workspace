...
function fixLexicalFormat(lexicalStr: string | null): string | null {
  try {
    ...
    console.log(
      `Created new Lexical structure with text: "${extractedText.substring(0, 50)}${extractedText.length > 50 ? '...' : ''}"`,
    );
...
async function fixPostsLexicalFormat() {
  try {
    console.log(
      chalk.blue(
        'Starting to fix Lexical format in posts and private posts...',
      ),
    );
    ...
    console.log(
      chalk.yellow(`Found ${postsResult.rows.length} posts to process`),
    );
    ...
    for (const post of postsResult.rows) {
      try {
        ...
        console.error(
          chalk.red(`Error processing post ${post.slug}:`),
          postError,
        );
    ...
    console.log(
      chalk.yellow(
        `Found ${privateResult.rows.length} private posts to process`,
      ),
    );
    ...
    for (const post of privateResult.rows) {
      try {
        ...
        await client.query(updateQuery, [fixedContent, post.id]);
        console.log(
          chalk.green(`✅ Fixed Lexical format for private post '${slug}'`),
        );
        privateUpdatedCount++;
      } catch (privateError) {
        console.error(
          chalk.red(`Error processing private post ${post.slug}:`),
          privateError,
        );
        // Continue with other posts
      }
    ...
    console.log(
      chalk.yellow(`Posts with content: ${verifyPostsResult.rows[0].count}`),
    );
    ...
    console.log(
      chalk.yellow(
        `Private posts with content: ${verifyPrivateResult.rows[0].count}`,
      ),
    );
...
fixPostsLexicalFormat()
  .then((results) => {
    ...
    console.log(
      chalk.yellow(
        `- Posts with content after fix: ${results.postsWithContent}`,
      ),
    );
    console.log(
      chalk.yellow(
        `- Private posts with content after fix: ${results.privateWithContent}`,
      ),
    );
    process.exit(0);
  })
