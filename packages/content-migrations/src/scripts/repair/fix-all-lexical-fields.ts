 ...
 */
function createProperLexicalStructure(originalText: string = ''): any {
  try {
 ...
 */
async function fixLexicalField(
  pool: pg.Pool,
  table: string,
  field: string,
): Promise<number> {
  try {
    console.log(
      chalk.cyan(`Processing field '${field}' in table '${table}'...`),
    );
    ...
    console.log(
      chalk.yellow(
        `Found ${result.rows.length} records with non-null ${field} field`,
      ),
    );
    ...
    for (const row of result.rows) {
      try {
        ...
        console.error(
          chalk.red(`Error processing record ${row.id} in ${table}.${field}:`),
          recordError.message,
        );
      }
    }
    ...
    console.log(
      chalk.green(
        `✅ Fixed ${updatedCount}/${result.rows.length} records in ${table}.${field}`,
      ),
    );
    return updatedCount;
  } catch (error) {
    ...
    for (const collection of collectionsWithLexicalFields) {
      ...
      try {
        ...
        if (!tableExists.rows[0].exists) {
          console.log(
            chalk.yellow(`Table ${collection.table} does not exist, skipping`),
          );
          continue;
        }
        ...
        for (const field of collection.fields) {
          ...
          try {
            ...
            if (!fieldExists.rows[0].exists) {
              console.log(
                chalk.yellow(
                  `Field ${field} does not exist in table ${collection.table}, skipping`,
                ),
              );
              continue;
            }
            ...
            const updatedCount = await fixLexicalField(
              pool,
              collection.table,
              field,
            );
            ...
            console.error(
              chalk.red(
                `Error checking field ${field} in ${collection.table}:`,
              ),
              fieldError.message,
            );
          }
        ...
        console.error(
          chalk.red(`Error checking table ${collection.table}:`),
          tableError.message,
        );
      }
    }
...
fixAllLexicalFields()
  .then((result) => {
    if (result.success) {
      ...
      console.error(
        chalk.red(`Failed to fix all Lexical fields: ${result.error}`),
      );
      process.exit(1);
    }
  })
