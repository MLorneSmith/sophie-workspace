...
import { QuizQuestionFixResult } from '../core/types.js';
  ...
  try {
    ...
    for (const quiz of quizzes) {
      try {
        ...
        console.log(
          `Processing quiz "${quizTitle}" (${quizId}): using relationship table only`,
        );
        ...
        console.log(
          `Found ${relTableEntries.length} relationship table entries`,
        );
        ...
        for (const questionId of directQuestions) {
          ...
          if (!exists) {
            console.log(
              `WARNING: Question ${questionId} in quiz ${quizId} does not exist in the database`,
            );
        ...
        const missingInRelTable = directQuestions.filter(
          (questionId) =>
            !relTableEntries.some((entry) => entry.id === questionId),
        );

        if (missingInRelTable.length > 0) {
          console.log(
            `Quiz ${quizId}: ${missingInRelTable.length} questions missing in relationship table`,
          );
        ...
        if (missingInDirect.length > 0) {
          console.log(
            `Quiz ${quizId}: ${missingInDirect.length} questions missing in direct questions field`,
          );
          ...
          console.log(
            `Quiz ${quizId}: Would update direct questions field if it existed`,
          );
        }
        ...
        for (let i = 0; i < directQuestions.length; i++) {
          ...
          if (relEntry && relEntry.order !== i) {
            ...
            console.log(
              `Quiz ${quizId}: Question ${questionId} has order mismatch (${relEntry.order} vs ${i})`,
            );
    ...
    console.log(
      `Cleaned up ${result.cleanedOrphans} orphaned relationship entries`,
    );
    ...
    // Rollback on error
    console.error(
      'Error fixing quiz-question relationships, rolling back:',
      error,
    );
 ...
 */
export async function verifyQuizQuestionRelationships(): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      `Verification result: ${isConsistent ? 'Consistent' : 'Inconsistent'}`,
    );
 ...
 */
export async function fixAndVerifyQuizQuestionRelationships(): Promise<{
  fixResult: QuizQuestionFixResult;
