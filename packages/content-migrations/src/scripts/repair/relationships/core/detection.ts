...
import {
  ...
  RELATIONSHIP_MAP_FILE_PATH,
  RELATIONSHIP_TABLE_PATTERNS,
  UUID_TABLE_PATTERN,
} from './constants.js';
import { RelationshipInfo, RelationshipMap } from './types.js';
...
export async function detectAllRelationships(): Promise<RelationshipMap> {
  ...
  try {
    ...
    const collections = collectionsResult.rows.map(
      (row: any) => row.tablename as string,
    );

    console.log(`Found ${collections.length} collections`);

    // Step 2: Get relationship tables
    let relationshipTablesQuery =
      "SELECT tablename FROM pg_tables WHERE schemaname = 'payload' AND (";
    ...
    const relTables = relTablesResult.rows.map(
      (row: any) => row.tablename as string,
    );
    ...
    try {
      ...
      map.uuidTables = uuidRegistry.rows.map(
      ...
      map.uuidTables = uuidTablesResult.rows.map(
        (row: any) => row.tablename as string,
      );
    ...
    for (const collection of collections) {
      ...
      for (const relTable of relTableMatches) {
        ...
        const paths = relPathsResult.rows.map((row: any) => row.path as string);
        ...
        for (const path of paths) {
          ...
          let relationType: 'hasOne' | 'hasMany' = 'hasOne';
          ...
          try {
              ...
              if (!targetCollection) {
                ...
                if (sampleResult.rows.length > 1) {
                  const uniqueParentIds = new Set(
                    sampleResult.rows.map((row: any) => row.parent_id),
                  );
            ...
            console.error(
              `Error analyzing relationship for ${collection}.${path}:`,
              error,
            );
          }
          ...
          if (!targetCollection && path) {
            ...
            const possibleTargets = collections.filter(
              (c) =>
                (path && c === path) ||
      ...
      console.log(
        `Analyzed ${collection}: found ${map.collections[collection].relationships.length} relationships`,
      );
    }
 ...
 */
export async function loadRelationshipMap(): Promise<
  RelationshipMap | undefined
> {
  try {
    if (await fileExists(RELATIONSHIP_MAP_FILE_PATH)) {
      const data = await fs.readFile(
        path.join(process.cwd(), RELATIONSHIP_MAP_FILE_PATH),
        'utf-8',
      );
      return JSON.parse(data) as RelationshipMap;
    }
    return undefined;
  } catch (error) {
 ...
 */
export async function detectAndSaveRelationships(
  force = false,
...
export function findRelationship(
  ...
  return map.collections[sourceCollection]?.relationships.find(
    (rel) => rel.sourceField === field,
  );
}
