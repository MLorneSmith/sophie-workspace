 ...
 */
export function fixLessonsQuizReferencesSql(): void {
  console.log('Fixing lesson-quiz references in references SQL file...');

  // Find the project root
  const projectRoot = path.resolve(
    fileURLToPath(import.meta.url),
    '../../../../..', // Go from /packages/content-migrations/src/scripts/fix-lessons-quiz-references-sql.ts to project root
  );

  const referencesFilePath = path.join(
    projectRoot,
    'apps/payload/src/seed/sql/03a-lesson-quiz-references.sql',
  );

  // Ensure the file exists
  if (!fs.existsSync(referencesFilePath)) {
    console.error(
      `Error: Lesson quiz references SQL file not found at ${referencesFilePath}`,
    );
    return;
  }
  ...
  console.log(
    `Fixed ${replacementCount} lesson-quiz references in ${referencesFilePath}`,
  );
}
