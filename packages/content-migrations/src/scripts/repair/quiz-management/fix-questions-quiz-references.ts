...
function getQuizId(slug: string): string {
  const id = CORRECT_QUIZ_IDS[slug];
  if (!id) {
    console.warn(
      `Warning: Quiz ID for "${slug}" not found in CORRECT_QUIZ_IDS map`,
    );
...
const OLD_TO_NEW_ID_MAP: Record<string, string> = {
  ...
  'b618e70a-44e5-45ac-90b0-5bc075865744': getQuizId(
  ...
  'f4653ead-3233-44e6-8d9d-5f92299b427e': getQuizId(
 ...
 */
export function fixQuestionsQuizReferences(): void {
  console.log('Fixing quiz-question references in 04-questions.sql...');

  // Find the project root
  const projectRoot = path.resolve(
    fileURLToPath(import.meta.url),
    '../../../../..', // Go from /packages/content-migrations/src/scripts/fix-questions-quiz-references.ts to project root
  );
