...
async function updateQuizQuestionsArray() {
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        `Found ${inconsistentQuizzes.length} quizzes with inconsistent relationships`,
      ),
    );
    ...
    for (const quiz of inconsistentQuizzes) {
      ...
      if (result.rowCount > 0) {
        ...
        console.log(
          chalk.green(
            `Fixed quiz ${quiz.quiz_id}: Updated questions array from ${quiz.array_count} to ${quiz.rel_count} questions`,
          ),
        );
      } else {
        console.log(
          chalk.red(
            `Failed to update quiz ${quiz.quiz_id} - quiz not found in course_quizzes table`,
          ),
        );
      }
    }
    ...
    if (stillInconsistent > 0) {
      console.log(
        chalk.red(
          `${stillInconsistent} quizzes still have inconsistent relationships after repair`,
        ),
      );
      // List some examples
      verificationResults.rows.slice(0, 5).forEach((quiz) => {
        console.log(
          chalk.red(
            `- Quiz ${quiz.quiz_id}: has ${quiz.array_count} questions in array but ${quiz.rel_count} in relationship table`,
          ),
        );
      });
      ...
      console.log(
        chalk.yellow('Changes rolled back due to remaining inconsistencies'),
      );
      return false;
    } else {
      // Commit the changes
      ...
      console.log(
        chalk.green(
          `Successfully repaired ${totalFixed} quizzes' question arrays`,
        ),
      );
      return true;
    }
    ...
    await executeSQL('ROLLBACK;');
    console.error(
      chalk.red('Error repairing quiz question relationships:'),
      error,
    );
    return false;
  }
...
async function main() {
  try {
    ...
    console.error(
      chalk.red('Unhandled error repairing quiz relationships:'),
      error,
    );
    process.exit(1);
  }
