 ...
 */
export async function registerEnhancedQuizSolution(): Promise<void> {
  ...
  try {
    ...
    if (!packageJson.scripts['fix:missing-quiz-entries']) {
      ...
      console.log(
        'fix:missing-quiz-entries script already exists in package.json',
      );
    }
 ...
 */
async function updateLoadingPhaseScript(): Promise<void> {
  // Path to the loading.ps1 file
  const loadingPhaseScriptPath = path.resolve(
    __dirname,
  ...
  try {
    ...
    const loadingPhaseContent = await fs.readFile(
      loadingPhaseScriptPath,
      'utf8',
    );
    ...
    const insertPosition = loadingPhaseContent.indexOf(
      'fix:unidirectional-quiz-questions',
    );

    if (insertPosition === -1) {
      console.log(
        'Could not find fix:unidirectional-quiz-questions in loading.ps1',
      );
      // Try to find the quiz relationship section
      const quizFixPosition = loadingPhaseContent.indexOf(
        'Fix quiz relationships',
      );

      if (quizFixPosition === -1) {
        console.log(
          'Could not find appropriate insertion point in loading.ps1',
        );
        return;
      }

      // Find the end of that section
      const endOfQuizSection = loadingPhaseContent.indexOf(
        'Log-Success',
        quizFixPosition,
      );
      ...
      const beginningOfFile = loadingPhaseContent.substring(
        0,
        endOfQuizSection,
      );
      ...
      console.log(
        'Added fix:missing-quiz-entries to loading.ps1 in quiz section',
      );
    } else {
      ...
      await fs.writeFile(loadingPhaseScriptPath, newContent);
      console.log(
        'Added fix:missing-quiz-entries to loading.ps1 after unidirectional fix',
      );
    }
  } catch (error) {
