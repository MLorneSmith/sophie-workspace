/**
 * Comprehensive Quiz System Integrity Verification
 *
 * This script performs a thorough verification of all quiz-related data,
 * checking for issues in:
 * 1. Course-Quiz relationships
 * 2. Lesson-Quiz relationships
 * 3. Question-Quiz relationships
 * 4. Quiz-specific fields and metadata
 *
 * It produces a detailed report of any issues found and recommendations
 * for fixing them.
 */
 ...
 */
export async function verifyQuizSystemIntegrity(): Promise<void> {
...
async function verifyCourseQuizRelationships(
  ...
  try {
    ...
    `);
    const quizzesWithoutCourseId = parseInt(
      quizzesWithoutCourseIdResult.rows[0].count,
    );
    ...
    if (missingRels > 0) {
      ...
      result.issues.push({
        ...
        count: missingRels,
        recommendation:
          'Run fix:course-quiz-relationships to fix missing relationship entries',
      });
    }
    ...
    if (inconsistentRels > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:course-quiz-relationships to fix inconsistent relationships',
      });
    }
  ...
  try {
    ...
    if (fieldNameIssues > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:lesson-quiz-field-name to synchronize these fields',
      });
    }
    ...
    if (missingRels > 0) {
      ...
      result.issues.push({
        ...
        count: missingRels,
        recommendation:
          'Run fix:lesson-quiz-relationships-comprehensive to fix missing relationship entries',
      });
    }
    ...
    if (invalidQuizRefs > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:invalid-quiz-references to fix or clear these invalid references',
      });
    }
...
async function verifyQuestionQuizRelationships(
  ...
  try {
    ...
    `);
    const questionsWithoutQuiz = parseInt(
      questionsWithoutQuizResult.rows[0].count,
    );
    ...
    if (questionsWithoutQuiz > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:question-quiz-relationships-comprehensive to attempt association',
      });
    }
    ...
    if (inconsistentIds > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:question-quiz-relationships-comprehensive to fix these inconsistencies',
      });
    }
    ...
    if (missingRels > 0) {
      ...
      result.issues.push({
        ...
        count: missingRels,
        recommendation:
          'Run fix:question-quiz-relationships-comprehensive to fix relationship entries',
      });
    }
    ...
    if (invalidQuizRefs > 0) {
      ...
      result.issues.push({
        ...
        count: invalidQuizRefs,
        recommendation:
          'Run fix:question-quiz-relationships-comprehensive to fix these invalid references',
      });
    }
...
async function verifyQuizContentAndMetadata(
  ...
  try {
    ...
    `);
    const quizzesWithoutTitle = parseInt(
      quizzesWithoutTitleResult.rows[0].count,
    );
    ...
    if (duplicatedTitles > 0) {
      ...
      result.issues.push({
        ...
        details,
        recommendation:
          'Manual intervention required to ensure unique quiz titles',
      });
    }
    ...
    `);
    const emptyReferencedQuizzes = parseInt(
      emptyReferencedQuizzesResult.rows[0].count,
    );
    ...
    if (emptyReferencedQuizzes > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Run fix:quizzes-without-questions or fix:question-quiz-relationships-comprehensive',
      });
      ...
      result.issues.push({
        ...
        recommendation:
          'These quizzes are not used and may be ignored or deleted',
      });
    }
...
async function verifyCrossReferences(
  ...
  try {
    ...
    const avgQuestionsPerQuiz =
      totalQuizzes > 0 ? (totalQuestions / totalQuizzes).toFixed(2) : '0';
    ...
    if (unreferencedQuizzes > 0) {
      result.issues.push({
        ...
        recommendation:
          'These quizzes may be unused or orphaned - consider removal',
      });
    }
    ...
    if (orphanedLessonRels > 0) {
      ...
      result.issues.push({
        ...
        recommendation:
          'Consider cleaning up these dangling relationship entries',
      });
    }
    ...
    `);
    const orphanedQuestionRels = parseInt(
      orphanedQuestionRelsResult.rows[0].count,
    );
    ...
    if (multiQuizQuestions > 0) {
      result.issues.push({
        ...
        details:
          'These questions appear in more than one quiz - may be intended but worth checking',
      });
    }
...
async function generateReport(results: VerificationResult[]): Promise<void> {
  ...
  console.log(
    `Overall status: ${overallPass ? `${SUCCESS} PASSED` : `${ERROR} FAILED`}`,
  );
  console.log('');
    ...
    if (result.issues.length > 0) {
      ...
      sortedIssues.forEach((issue) => {
        ...
        if (issue.recommendation) {
          console.log(
            `${INDENT}${INDENT}${INDENT}Recommendation: ${issue.recommendation}`,
          );
        }
      });
    } else {
