...
export async function fixCourseQuizRelationships(): Promise<void> {
  ...
  console.log(
    'Fixing course-quiz and quiz-question relationships with comprehensive approach...',
  );
  ...
  try {
    ...
    console.log(
      `- Course-Quiz relationships: ${beforeStats.courseQuizCount} → ${afterStats.courseQuizCount}`,
    );
    console.log(
      `- Quiz-Question relationships: ${beforeStats.quizQuestionCount} → ${afterStats.quizQuestionCount}`,
    );
    console.log(
      `- Quizzes with questions: ${beforeStats.quizzesWithQuestionsCount} → ${afterStats.quizzesWithQuestionsCount}`,
    );

    // Analyze quiz statuses
    const invalidCourseQuizzes = courseQuizStatus.filter(
      (q) => q.status === 'INVALID',
    );
    const invalidQuizQuestions = quizQuestionStatus.filter(
      (q) => q.status === 'INVALID',
    );
    ...
    console.log('\nDetailed Status:');
    console.log(
      `- Quizzes without course links: ${invalidCourseQuizzes.length}`,
    );
    console.log(
      `- Quizzes with invalid question links: ${invalidQuizQuestions.length}`,
    );
    ...
    if (courseFixSuccess && questionFixSuccess) {
      ...
      afterStats.courseQuizCount > beforeStats.courseQuizCount ||
    ...
    if (invalidQuizQuestions.length > 0) {
      ...
      invalidQuizQuestions.forEach((quiz) => {
        console.log(
          `- "${quiz.title}" (${quiz.id}): direct_questions=${quiz.direct_question_count}, rel_questions=${quiz.rel_question_count}`,
        );
      });
    }
 ...
 */
async function getRelationshipStats(client: Client): Promise<{
  totalQuizzes: number;
  courseQuizCount: number;
  quizQuestionCount: number;
  quizzesWithQuestionsCount: number;
}> {
  try {
    // Query quiz stats
    ...
    const courseQuizCount = parseInt(
      statsResult.rows[0].course_quiz_count || '0',
    );
    const quizQuestionCount = parseInt(
      statsResult.rows[0].quiz_question_count || '0',
    );
    const quizzesWithQuestionsCount = parseInt(
      statsResult.rows[0].quizzes_with_questions_count || '0',
    );
