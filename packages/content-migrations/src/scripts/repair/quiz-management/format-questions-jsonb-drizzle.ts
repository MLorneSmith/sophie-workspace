...
export const formatQuestionsJSONBDrizzle = async (): Promise<boolean> => {
  try {
    ...
    for (const [quizId, quizData] of quizMap.entries()) {
      ...
      logAction(
        `Updating quiz "${quizData.title}" with ${formattedQuestions.length} formatted questions`,
      );
    ...
    if (problemQuizResult.rows.length > 0) {
      ...
      if (questionRels.rows.length > 0) {
        ...
        logAction(
          `Applied special fix to "${problemQuiz.title}" with ${correctlyFormattedQuestions.length} questions`,
        );
        ...
        logError(
          `No question relationships found for problem quiz "${problemQuiz.title}"`,
        );
      }
    } else {
      logWarning(
        'Problem quiz "The Fundamental Elements of Design in Detail Quiz" not found!',
      );
    }
    ...
    if (performanceQuizResult.rows.length > 0) {
      ...
      logAction(
        `Performance Quiz has ${perfQuizRels.rows[0].count} related questions`,
      );
    }
    ...
    if (verificationStats.formatted_count === verificationStats.total_count) {
      logSuccess(
        `All ${verificationStats.total_count} quizzes now have properly formatted questions!`,
      );
    } else {
      logWarning(
        `${verificationStats.formatted_count} of ${verificationStats.total_count} quizzes are properly formatted`,
      );
      ...
      if (remainingProblems.rows.length > 0) {
        logWarning(
          `Remaining problem quizzes: ${JSON.stringify(remainingProblems.rows.map((r) => r.quiz_title))}`,
        );
      }
    }
