/**
 * Quiz System Repair - Relationship Detection
 *
 * Analyzes the database to identify quiz and question entities and their relationships.
 * Maps the current state of relationships and identifies issues that need fixing.
 */
import { PostgresJsDatabase } from 'drizzle-orm/postgres-js';

import { getLogger } from '../../../utils/logging.js';
import { QuestionData, QuizData, QuizSystemState } from './types.js';
import {
  identifyRelationshipIssues,
  mapQuestionToQuiz,
  mapQuizToQuestions,
} from './utils/index.js';
 ...
 */
export async function detectQuizRelationships(
  db: PostgresJsDatabase,
): Promise<QuizSystemState> {
  ...
  try {
    ...
    await db.execute(
      `SELECT set_config('search_path', 'payload, public', false);`,
    );

    // Get all quizzes
    logger.info('Fetching quizzes...');
    const quizzes = (await db.execute(
      `SELECT id, title, slug, questions FROM course_quizzes`,
    ...
    const questions = (await db.execute(
      `SELECT id, question, options, correct_answer, type, explanation FROM quiz_questions`,
    ...
    `)) as unknown as any[];
    logger.info(
      `Found ${quizToQuestionRels.length} quiz-to-question relationships`,
    );
    ...
    `)) as unknown as any[];
    logger.info(
      `Found ${questionToQuizRels.length} question-to-quiz relationships`,
    );
    ...
    throw new Error(
      `Failed to detect quiz relationships: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
 ...
 */
export function logDetectionSummary(state: QuizSystemState): void {
  ...
  logger.info(`Found ${questions.length} questions`);
  logger.info(
    `Found ${Array.from(quizToQuestions.values()).reduce((sum, arr) => sum + arr.length, 0)} quiz-to-question relationships`,
  );
  ...
  if (issues.length > 0) {
    ...
    Object.entries(issueGroups).forEach(([type, typeIssues]) => {
      ...
      if (typeIssues.length > 0 && typeIssues.length <= 3) {
        ...
        logger.info(
          `    - ... and ${typeIssues.length - 2} more similar issues`,
        );
      }
    });
  }
