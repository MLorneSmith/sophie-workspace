/**
 * Quiz System Repair - JSONB Format
 *
...
import { JsonbFormatResult, QuizSystemState } from './types.js';
...
export async function fixJsonbFormat(
  ...
  try {
    ...
    logger.info(
      `Found ${checkResult[0]?.count || 0} quizzes that may need JSONB format update`,
    );
    ...
    throw new Error(
      `Failed to fix JSONB format: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
 ...
 */
export async function verifyJsonbFormat(
  db: PostgresJsDatabase,
): Promise<boolean> {
  ...
  try {
    ...
    if (!hasRels) {
      logger.info(
        'No quiz-question relationships exist yet. JSONB format check skipped.',
      );
      return true;
    }

    // Simplify the verification query to avoid complex set-returning functions
    const missingJsonbResult: any = await db.execute(`
    ...
    if (mismatchCount > 0) {
      ...
      rows.forEach((row) => {
        logger.warning(
          `  - Quiz "${row.quiz_title}" (${row.quiz_id}) has ${row.rel_count} relationships but ${row.json_count} questions in JSONB`,
        );
      });
      return false;
    }
