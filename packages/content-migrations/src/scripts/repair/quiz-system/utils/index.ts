 ...
 */
export function mapQuizToQuestions(
  relationships: QuizToQuestionRel[],
): Map<string, string[]> {
 ...
 */
export function mapQuestionToQuiz(
  relationships: QuestionToQuizRel[],
): Map<string, string> {
  const map = new Map<string, string>();
 ...
 */
export function identifyRelationshipIssues(
  quizzes: QuizData[],
  questions: QuestionData[],
  quizToQuestions: Map<string, string[]>,
  questionToQuiz: Map<string, string>,
) {
  const issues = [];
  ...
  for (const quiz of quizzes) {
    ...
    const relatedQuestions = questions.filter((q) =>
      questionIdsInRels.includes(q.id),
    );
 ...
 */
export function logRepairSummary(result: any, options: { verbose: boolean }) {
  ...
  if (result.primaryResult) {
    logger.info(
      `Primary Relationships: ${result.primaryResult.relationshipsCreated} created`,
    );
    if (verbose && result.primaryResult.relationshipsCreated > 0) {
      result.primaryResult.newRelationships.forEach((rel: any, idx: number) => {
        if (
          idx < 5 ||
          ...
          logger.info(
            `  - ... and ${result.primaryResult.newRelationships.length - 5} more`,
          );
        }
      });
    }
  ...
  if (result.bidirectionalResult) {
    logger.info(
      `Bidirectional Relationships: ${result.bidirectionalResult.relationshipsCreated} created`,
    );
    if (verbose && result.bidirectionalResult.relationshipsCreated > 0) {
      result.bidirectionalResult.newRelationships.forEach(
        (rel: any, idx: number) => {
          if (
            idx < 5 ||
            ...
            logger.info(
              `  - ... and ${result.bidirectionalResult.newRelationships.length - 5} more`,
            );
          }
        },
      );
  ...
  if (result.jsonbResult) {
    logger.info(
      `JSONB Format: ${result.jsonbResult.quizzesUpdated} quizzes updated`,
    );
    if (verbose && result.jsonbResult.quizzesUpdated > 0) {
      result.jsonbResult.updatedQuizzes.forEach((quiz: any, idx: number) => {
        if (idx < 5 || idx === result.jsonbResult.updatedQuizzes.length - 1) {
          logger.info(`  - Quiz "${quiz.title}" (${quiz.id})`);
        } else if (idx === 5) {
          logger.info(
            `  - ... and ${result.jsonbResult.updatedQuizzes.length - 5} more`,
          );
        }
      });
    }
  ...
  if (result.verificationResult) {
    ...
    if (!result.verificationResult.success && verbose) {
      result.verificationResult.issues.forEach((issue: any, idx: number) => {
        if (idx < 5 || idx === result.verificationResult.issues.length - 1) {
          logger.info(`  - Issue: ${JSON.stringify(issue)}`);
        } else if (idx === 5) {
          logger.info(
            `  - ... and ${result.verificationResult.issues.length - 5} more issues`,
          );
        }
      });
    }
