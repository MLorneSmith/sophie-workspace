/**
 * Quiz System Repair - Bidirectional Relationships
 *
 * Creates question → quiz relationships by creating entries in quiz_questions_rels
 * This ensures proper bidirectional navigation
 */
import { PostgresJsDatabase } from 'drizzle-orm/postgres-js';

import { getLogger } from '../../../utils/logging.js';
import { BidirectionalRepairResult, QuizSystemState } from './types.js';
...
export async function fixBidirectionalRelationships(
  ...
  try {
    ...
    throw new Error(
      `Failed to fix bidirectional relationships: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
 ...
 */
export async function verifyBidirectionalRelationships(
  db: PostgresJsDatabase,
): Promise<boolean> {
  ...
  try {
    ...
    if (!hasRels) {
      logger.info(
        'No primary quiz-question relationships exist yet. Bidirectional relationship check skipped.',
      );
      return true;
    }

    // Simplify the verification query
    const missingBidirectionalResult: any = await db.execute(`
    ...
    if (missingCount > 0) {
      logger.warning(
        `Found ${missingCount} missing bidirectional relationships`,
      );
      rows.forEach((row) => {
        logger.warning(
          `  - Question ${row.question_id} (${row.question_text?.substring(0, 30)}...) is missing relationship to Quiz "${row.quiz_title}" (${row.quiz_id})`,
        );
      });
      return false;
    }
