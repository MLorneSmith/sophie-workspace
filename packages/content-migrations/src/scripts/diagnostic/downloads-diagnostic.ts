...
async function runDownloadsDiagnostic(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `- MimeType: ${metadata.has_mimetype}/${metadata.total} downloads (${Math.round((metadata.has_mimetype / metadata.total) * 100)}%)`,
    );
    console.log(
      `- Filesize: ${metadata.has_filesize}/${metadata.total} downloads (${Math.round((metadata.has_filesize / metadata.total) * 100)}%)`,
    );
    console.log(
      `- Dimensions: ${metadata.has_dimensions}/${metadata.total} downloads (${Math.round((metadata.has_dimensions / metadata.total) * 100)}%)`,
    );
    console.log(
      `- Thumbnail URL: ${metadata.has_thumbnail_url}/${metadata.total} downloads (${Math.round((metadata.has_thumbnail_url / metadata.total) * 100)}%)`,
    );
    ...
    console.log(
      `- Downloads with lessons: ${distribution.downloads_with_lessons}/${distribution.download_count} (${Math.round((distribution.downloads_with_lessons / distribution.download_count) * 100)}%)`,
    );
    console.log(
      `- Maximum lessons per download: ${distribution.max_lessons_per_download}`,
    );
    console.log(
      `- Average lessons per download: ${distribution.avg_lessons_per_download}`,
    );
    ...
    sampleDownloadsResult.rows.forEach((download, index) => {
      ...
      console.log(
        `- Dimensions: ${download.width || 'NULL'} x ${download.height || 'NULL'}`,
      );
      console.log(`- Thumbnail URL: ${download.sizes_thumbnail_url || 'NULL'}`);
      console.log(`- Lesson Count: ${download.lesson_count}`);
      console.log(
        `- Related Lessons: ${download.related_lessons ? download.related_lessons.join(', ') : 'None'}`,
      );
    });
    ...
    if (orphanedCount > 0) {
      console.log(
        chalk.yellow('Warning: Some downloads have no lesson relationships.'),
      );
