...
interface DiagnosticResult {
  ...
  courseQuizRelsCount: number;
  quizQuestionRelsCount: number;
  nullPathCount: number;
  bidirectionalInconsistencies: number;
  duplicateRelationships: number;
  wrongFieldNameCount: number;
  isHealthy: boolean;
  detailedProblems: {
    missingQuizQuestionRels: any[];
    duplicateRels: any[];
    nullPathRels: any[];
    wrongFieldRels: any[];
  };
...
async function runDiagnostic(): Promise<DiagnosticResult> {
  ...
  try {
    ...
    const quizQuestionRelsResult = await executeSQL(`
      ...
      WHERE field = 'quiz_id'
    `);
    const quizQuestionRelsCount = parseInt(
      quizQuestionRelsResult.rows[0].count,
    );
...
function printDiagnosticResults(result: DiagnosticResult): void {
  ...
  console.log(
    `- Question → Quiz Relationships: ${result.quizQuestionRelsCount}`,
  );
  ...
  if (result.courseQuizRelsCount === result.quizQuestionRelsCount) {
    ...
    console.log(
      chalk.red(
        `✗ Relationship counts don't match! (Difference: ${Math.abs(result.courseQuizRelsCount - result.quizQuestionRelsCount)})`,
      ),
    );
  }
  console.log('');
  ...
  console.log(
    `- Bidirectional inconsistencies: ${result.bidirectionalInconsistencies}`,
  );
  ...
  if (result.isHealthy) {
    ...
    if (result.nullPathCount > 0) {
      console.log(
        chalk.red(`- ${result.nullPathCount} records have NULL path field`),
      );
      ...
      if (result.detailedProblems.nullPathRels.length > 0) {
        ...
        result.detailedProblems.nullPathRels.slice(0, 3).forEach((rec) => {
          console.log(
            chalk.gray(
              `  - Quiz ID: ${rec._parent_id}, Question ID: ${rec.quiz_questions_id}`,
            ),
          );
        });
        ...
        if (result.detailedProblems.nullPathRels.length > 3) {
          console.log(
            chalk.gray(
              `  ... and ${result.detailedProblems.nullPathRels.length - 3} more`,
            ),
          );
        }
      }
    ...
    if (result.bidirectionalInconsistencies > 0) {
      console.log(
        chalk.red(
          `- ${result.bidirectionalInconsistencies} missing bidirectional relationships`,
        ),
      );
      ...
      if (result.detailedProblems.missingQuizQuestionRels.length > 0) {
        ...
        if (result.detailedProblems.missingQuizQuestionRels.length > 3) {
          console.log(
            chalk.gray(
              `  ... and ${result.detailedProblems.missingQuizQuestionRels.length - 3} more`,
            ),
          );
        }
      }
    ...
    if (result.duplicateRelationships > 0) {
      console.log(
        chalk.red(`- ${result.duplicateRelationships} duplicate relationships`),
      );
    }
    ...
    if (result.wrongFieldNameCount > 0) {
      console.log(
        chalk.red(
          `- ${result.wrongFieldNameCount} relationships with wrong field names`,
        ),
      );
      ...
      if (result.detailedProblems.wrongFieldRels.length > 0) {
        ...
        const fieldNames = new Set(
          result.detailedProblems.wrongFieldRels.map((rel) => rel.field),
        );
  ...
  if (!result.isHealthy) {
    ...
    if (result.nullPathCount > 0) {
      ...
      console.log(
        "  UPDATE payload.course_quizzes_rels SET path = 'questions' WHERE field = 'questions' AND path IS NULL;",
      );
    }

    if (result.bidirectionalInconsistencies > 0) {
      console.log(
        chalk.yellow('- Create missing bidirectional relationships:'),
      );
  ...
  WHERE
    ...
    if (result.wrongFieldNameCount > 0) {
      ...
      console.log(
        "  UPDATE payload.quiz_questions_rels SET field = 'quiz_id' WHERE field != 'quiz_id';",
      );
    }
