...
async function run() {
  ...
  try {
    ...
    for (const quiz of quizzes) {
      ...
      console.log(
        `  - JSONB questions array: ${jsonbQuestions.length || 0} items`,
      );
      console.log(`  - JSONB format correct: ${properFormat ? 'Yes' : 'No'}`);

      if (jsonbQuestions.length > 0) {
        console.log(
          `  - Sample format: ${JSON.stringify(jsonbQuestions[0]).substring(0, 100)}...`,
        );
      }
      ...
      if (rels.length > 0 && jsonbQuestions.length > 0) {
        ...
        if (onlyInRels.length === 0 && onlyInJsonb.length === 0) {
          console.log(
            '  ✅ Relationship records and JSONB array match perfectly',
          );
        } else {
          console.log('  ❌ Inconsistencies detected:');
          if (onlyInRels.length > 0) {
            console.log(
              `    - ${onlyInRels.length} questions in relationships but missing from JSONB array`,
            );
          }
          if (onlyInJsonb.length > 0) {
            console.log(
              `    - ${onlyInJsonb.length} questions in JSONB array but missing from relationships`,
            );
          }
        ...
        console.log(
          '  ❌ Count mismatch between relationships and JSONB array',
        );
        issuesFound++;
      }
      ...
      if (
        ...
        console.log(
          `  ⚠️ This quiz corresponds to a lesson with reported errors`,
        );
      }
    }
    ...
    console.log(
      `- Found ${issuesFound} quizzes with relationship inconsistencies`,
    );
