  ...
  try {
    ...
    for (const tableName of uuidTables) {
      ...
      if (missingColumns.length > 0) {
        ...
        console.log(
          chalk.red(
            `Table ${tableName} is missing critical columns: ${missingColumns.join(', ')}`,
          ),
        );
      } else if (verbose) {
        console.log(
    ...
    console.log(
      `Tables with missing critical columns: ${tablesWithMissingColumns}`,
    );
    ...
    if (tablesWithIssues.length > 0) {
      ...
      for (const table of tablesWithIssues) {
        console.log(
          `  ${table.tableName}: missing ${table.missingColumns.join(', ')}`,
        );
      }
  ...
  verifyCriticalColumns(verbose)
    .then((result) => {
      if (result.success) {
        ...
        console.error(
          chalk.red(
            'Verification failed: Some tables are missing critical columns',
          ),
        );
        process.exit(1);
      } else {
