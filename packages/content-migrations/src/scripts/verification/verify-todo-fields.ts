...
async function verifyTodoFields() {
  console.log(
    chalk.blue(
      'Starting verification of todo fields in course_lessons table...',
    ),
  );
  ...
  try {
    ...
    const metadata = yaml.load(fileContent) as { lessons: any[] };
      ...
      fieldsToCheck.forEach((field) => {
        ...
        if (hasFieldInYaml) {
          ...
          if (hasFieldInDb) {
            fieldsPopulated++;
            console.log(
              chalk.green(`  ✓ Field ${field} is populated in database`),
            );
          } else {
            console.log(
              chalk.red(`  ✗ Field ${field} is NOT populated in database`),
            );
          }
        }
      });
    ...
    console.log(
      chalk.blue(`Fields expected to be populated: ${fieldsExpected}`),
    );
