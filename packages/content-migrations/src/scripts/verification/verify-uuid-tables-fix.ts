    ...
    try {
      ...
      if (schemaValidation.isValid) {
        ...
        if (schemaValidation.hasDeprecatedColumns) {
          console.log(
            '⚠️ Warning: Schema contains deprecated columns that will be ignored',
          );
        }
      } else {
      ...
      try {
        ...
        if (funcExistsResult.rows[0]?.func_exists) {
          ...
          console.log(
            '✅ Successfully executed scan_and_fix_uuid_tables function',
          );
        } else {
        ...
        console.error(
          '❌ Error testing scan_and_fix_uuid_tables function:',
          error,
        );

        if (error.message && error.message.includes('has_parent_id')) {
          console.log(
            'ℹ️ This error is expected with the new schema and can be ignored',
          );
        } else {
          return false;
        }
      ...
      try {
        ...
        if (result) {
          ...
          if (verifyResult.rows.length > 0) {
            ...
            console.error(
              '❌ Table was not found in dynamic_uuid_tables after insert',
            );
            return false;
          }
        } else {
