...
export async function verifyFallbacks() {
  try {
    ...
    const extensionPath = path.join(
      process.cwd(),
      'apps/payload/src/extensions/fallback-system.ts',
    );
    results.components.payloadExtension = fs.existsSync(extensionPath);

    // 6. Check if API endpoints exist
    const apiEndpointPath = path.join(
      process.cwd(),
      'apps/payload/src/routes/api/fallbacks/relationships.ts',
    );
    ...
    for (const category of Object.keys(results)) {
      for (const item of Object.keys(results[category])) {
        ...
        logger.info(
          { verification: 'fallbacks', category, item },
          `${status ? '✅' : '❌'} ${category}.${item}: ${status ? 'OK' : 'Missing'}`,
        );
      }
    }
    ...
    if (overallSuccess) {
      logger.info(
        { verification: 'fallbacks' },
        '✅ Fallback mechanisms verified - at least some components are working',
      );
    } else {
      logger.warn(
        { verification: 'fallbacks' },
        '❌ Fallback mechanisms verification failed - no components found',
      );
    }
    ...
    logger.error(
      ...
      'Error verifying fallback mechanisms',
    );
    return {
      success: false,
      error,
      ...
      if (result.results) {
        ...
        for (const category of Object.keys(result.results)) {
          ...
          for (const item of Object.keys(result.results[category])) {
            console.log(
              `  ${result.results[category][item] ? '✓' : '✗'} ${item}`,
            );
          }
        }
      }
