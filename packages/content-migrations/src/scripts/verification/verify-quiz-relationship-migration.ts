...
async function verifyQuizRelationshipMigration() {
  try {
    ...
    quizArrayCounts.rows.forEach((quiz) => {
      ...
      // For debugging
      console.log(
        `Debug: quiz_id=${quiz.quiz_id}, array_count=${arrayCount}(${typeof arrayCount}), rel_count=${relCount}(${typeof relCount}), is_consistent=${isConsistent}`,
      );
    ...
    if (inconsistentCount > 0) {
      console.error(
        chalk.red(
          `Found ${inconsistentCount} quizzes with inconsistent relationships:`,
        ),
      );
    ...
    console.log(
      chalk.green(`- Total questions across all quizzes: ${totalQuestions}`),
    );
    ...
    if (totalQuizzes > 0) {
      ...
      results.slice(0, 3).forEach((quiz) => {
        console.log(
          chalk.green(
            `- Quiz "${quiz.quiz_title}" has ${quiz.array_count} questions properly linked`,
          ),
        );
      });
    }
    ...
    console.error(
      chalk.red('Error verifying quiz relationship migration:'),
      error,
    );
    return false;
  }
