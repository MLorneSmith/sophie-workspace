  ...
  try {
    ...
    if (!quizHasQuestionsField && !relHasQuestionId) {
      ...
      const totalQuestions = parseInt(
        queryResult.rows[0].total_questions || '0',
      );
      const questionsInRelationships = parseInt(
        queryResult.rows[0].questions_in_relationships || '0',
      );
      ...
      if (verificationResult.rows.length > 0) {
        ...
        console.log(
          formatLogMessage(
            `Quiz-Question relationships: ${verificationResult.rows.reduce(
              (sum, row) => sum + parseInt(row.total_count),
              0,
            )} issues found`,
            'warn',
          ),
  ...
  try {
    ...
    if (!lessonHasQuizField && !relHasQuizId) {
      ...
      const totalRelationships = parseInt(
        queryResult.rows[0].total_relationships || '0',
      );
      ...
      if (verificationResult.rows.length > 0) {
        ...
        console.log(
          formatLogMessage(
            `Lesson-Quiz relationships: ${verificationResult.rows.reduce(
              (sum, row) => sum + parseInt(row.total_count),
              0,
            )} issues found`,
            'warn',
        ...
        console.log(
          formatLogMessage(
            'Lesson-Quiz relationships: No issues found',
            'info',
          ),
        );
      }
...
async function verifySurveyQuestionRelationships(
  ...
  try {
    ...
    if (!surveyTableExists || !questionTableExists || !relTableExists) {
      ...
      console.log(
        `survey_questions: ${questionTableExists ? 'exists' : 'missing'}`,
      );
    ...
    // Check if necessary columns exist
    const surveyHasQuestionsField = await columnExists(
      'payload',
      'surveys',
      'questions',
    );
    ...
    if (!surveyHasQuestionsField && !relHasQuestionId) {
      console.log(
        chalk.yellow(
          'Required relationship columns do not exist for survey-question relationships, skipping detailed verification',
        ),
      );
      ...
      if (verbose) {
        console.log(
          `Total survey-question relationships: ${totalRelationships}`,
        );
      }
      ...
      if (totalRelationships > 0) {
        console.log(
          chalk.green(
            `✓ Found ${totalRelationships} survey-question relationships`,
          ),
        );
      } else {
      ...
      if (verificationResult.rows.length > 0) {
        ...
        console.log(
          formatLogMessage(
            `Survey-Question relationships: ${verificationResult.rows.reduce(
              (sum, row) => sum + parseInt(row.total_count),
              0,
            )} issues found`,
            'warn',
...
async function verifyDownloadRelationships(
  ...
  try {
    ...
    const lessonsRelsTableExists = await tableExists(
      'payload',
      'course_lessons_rels',
    );
    ...
    if (
      ...
      console.log(
        chalk.yellow(
          'One or more required tables do not exist, skipping download relationship verification',
        ),
      );
      // Still mark the relationship as verified to allow migration to continue
      return result;
    ...
    // Check if necessary columns exist
    const lessonHasDownloadsField = await columnExists(
      'payload',
      'course_lessons',
      'downloads',
    );
    const courseHasDownloadsField = await columnExists(
      'payload',
      'courses',
      'downloads',
    );
    const lessonRelHasDownloadId = await columnExists(
      'payload',
      'course_lessons_rels',
      'downloads_id',
    );
    const courseRelHasDownloadId = await columnExists(
      'payload',
      'courses_rels',
      'downloads_id',
    );
    ...
    if (
      ...
      console.log(
        chalk.yellow(
          'Required relationship columns do not exist for download relationships, skipping detailed verification',
        ),
      );
      ...
      if (totalRelationships > 0) {
        console.log(
          chalk.green(`✓ Found ${totalRelationships} download relationships`),
        );
      } else {
      ...
      if (verificationResult.rows.length > 0) {
        ...
        console.log(
          formatLogMessage(
            `Download relationships: ${verificationResult.rows.reduce(
