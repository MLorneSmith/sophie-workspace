...
async function run() {
  ...
  try {
    ...
    console.log(
      `Found ${problematicQuizzes.length} quizzes with mismatched relationship counts`,
    );
    ...
    for (const quiz of problematicQuizzes) {
      console.log(
        `Fixing relationships for quiz: ${quiz.quiz_title} (${quiz.quiz_id})`,
      );
      ...
      if (allQuestionIds.length > 0) {
        ...
        console.log(
          `Fixed quiz ${quiz.quiz_title}: reconciled ${allQuestionIds.length} questions`,
        );
        repairCount++;
      } else {
    ...
    for (const quiz of reportedErrorQuizzes) {
      console.log(
        `Applying special fix for reported error quiz: ${quiz.title} (${quiz.id})`,
      );
    ...
    if (missingBidirectional.length > 0) {
      console.log(
        `Creating ${missingBidirectional.length} missing bidirectional relationships`,
      );
    ...
    console.log(`\nRepair Summary:`);
    console.log(
      `- Fixed ${repairCount} quizzes with relationship inconsistencies`,
    );
    console.log(
      `- Created ${missingBidirectional.length} missing bidirectional relationships`,
    );
    console.log(
      `- After fix: ${afterFixDiagnostic[0].synced_quizzes} of ${afterFixDiagnostic[0].total_quizzes} quizzes have synchronized relationships`,
    );

    if (parseInt(afterFixDiagnostic[0].remaining_problems) > 0) {
      console.warn(
        `Warning: ${afterFixDiagnostic[0].remaining_problems} quizzes still have inconsistent relationships`,
      );
      ...
      for (const quiz of remainingProblems) {
        console.log(
          `- ${quiz.quiz_title}: JSONB=${quiz.questions_jsonb_count}, Rels=${quiz.questions_rel_count}`,
        );
      }
    ...
    console.log(
      chalk.green(
        '✅ Quiz relationship consistency fix completed successfully',
      ),
    );
    return true;
  } catch (error) {
    ...
    console.error(
      chalk.red('Error in quiz relationship consistency fix:'),
      error,
    );
    return false;
  } finally {
