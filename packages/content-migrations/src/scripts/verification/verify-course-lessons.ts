...
async function verifyCourseLessons() {
  ...
  try {
    ...
    try {
      ...
      const lessonsWithQuizzesCount = parseInt(
        lessonsWithQuizzesResult.rows[0].count,
      );
      console.log(
        `Found ${lessonsWithQuizzesCount} lessons with quiz_id assigned`,
      );
      ...
      const mismatchedLessonsCount = parseInt(
        mismatchedLessonsResult.rows[0].count,
      );

      if (mismatchedLessonsCount > 0) {
        console.log(
          `❌ Found ${mismatchedLessonsCount} lessons where quiz_id_id does not match quiz_id`,
        );
        ...
        mismatchedLessonsDetailsResult.rows.forEach((row) => {
          console.log(
            `Lesson: ${row.title}, quiz_id: ${row.quiz_id}, quiz_id_id: ${row.quiz_id_id}`,
          );
        });
      ...
      const relationshipsCount = parseInt(
        relationshipsCountResult.rows[0].count,
      );
      console.log(
        `Found ${relationshipsCount} quiz relationships in course_lessons_rels table`,
      );
      ...
      const lessonsWithoutRelationshipsCount = parseInt(
        lessonsWithoutRelationshipsResult.rows[0].count,
      );

      if (lessonsWithoutRelationshipsCount > 0) {
        console.log(
          `❌ Found ${lessonsWithoutRelationshipsCount} lessons with quizzes that don't have relationships in course_lessons_rels`,
        );
        return false;
      }

      console.log(
        '✅ All lessons with quizzes have relationships in course_lessons_rels',
      );
      ...
      const invalidRelationshipsCount = parseInt(
        invalidRelationshipsResult.rows[0].count,
      );

      if (invalidRelationshipsCount > 0) {
        console.log(
          `❌ Found ${invalidRelationshipsCount} relationships with incorrect value`,
        );
        return false;
      }
    ...
    console.error(
      'Error verifying course_lessons table:',
