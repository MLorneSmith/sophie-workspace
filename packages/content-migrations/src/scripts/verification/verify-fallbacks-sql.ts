...
async function verifyFallbacks() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Found ${relationshipCount} relationships in unified_relationships view`,
      );
      if (relationshipCount === 0) {
        console.log(
          chalk.yellow(
            '⚠️ No relationships found in unified_relationships view',
          ),
        );
      } else {
      ...
      console.error(
        chalk.red('Error testing unified_relationships view:'),
        error,
      );
      throw error;
    }
    ...
    try {
      ...
      console.log(
        `Found ${courseContentCount} lessons in course_content_fallback view`,
      );
      if (courseContentCount === 0) {
        console.log(
          chalk.yellow('⚠️ No lessons found in course_content_fallback view'),
        );
      } else {
      ...
      console.error(
        chalk.red('Error testing course_content_fallback view:'),
        error,
      );
      throw error;
    }
    ...
    try {
      ...
      console.log(
        chalk.yellow('⚠️ Will skip function testing due to no course ID'),
      );
    }
    ...
    if (courseId) {
      try {
        ...
        console.log(
          `get_course_lessons function returned ${lessonCount} lessons`,
        );
        if (lessonCount === 0) {
          console.log(
            chalk.yellow(
              '⚠️ No lessons returned from get_course_lessons function',
            ),
          );
        } else {
          console.log(
            chalk.green('✓ get_course_lessons function works correctly'),
          );
          ...
          try {
            ...
            if (quizResult.rows.length > 0) {
              console.log(
                chalk.green('✓ get_lesson_quiz function works correctly'),
              );
              ...
              try {
                ...
                console.log(
                  `get_quiz_questions function returned ${questionCount} questions`,
                );
                if (questionCount === 0) {
                  console.log(
                    chalk.yellow(
                      '⚠️ No questions returned from get_quiz_questions function',
                    ),
                  );
                } else {
                  console.log(
                    chalk.green(
                      '✓ get_quiz_questions function works correctly',
                    ),
                  );
                }
              } catch (error) {
                console.error(
                  chalk.red('Error testing get_quiz_questions function:'),
                  error,
                );
              }
            } else {
              console.log(
                chalk.yellow(
                  '⚠️ No quiz returned from get_lesson_quiz function',
                ),
              );
            }
          } catch (error) {
            console.error(
              chalk.red('Error testing get_lesson_quiz function:'),
              error,
            );
          }
          ...
          try {
            ...
            console.log(
              `get_lesson_downloads function returned ${downloadCount} downloads`,
            );
            if (downloadCount === 0) {
              console.log(
                chalk.yellow(
                  '⚠️ No downloads returned from get_lesson_downloads function',
                ),
              );
            } else {
              console.log(
                chalk.green('✓ get_lesson_downloads function works correctly'),
              );
            }
          } catch (error) {
            console.error(
              chalk.red('Error testing get_lesson_downloads function:'),
              error,
            );
          }
        }
        ...
        console.error(
          chalk.red('Error testing get_course_lessons function:'),
          error,
        );
      }
    }
    ...
    try {
      ...
      if (fallbacksEnabled) {
        console.log(
          chalk.green('✓ ensure_fallbacks_enabled function returns true'),
        );
      } else {
        console.log(
          chalk.red('✗ ensure_fallbacks_enabled function returns false'),
        );
        throw new Error('Fallbacks are not fully enabled');
      }
    } catch (error) {
      console.error(
        chalk.red('Error testing ensure_fallbacks_enabled function:'),
        error,
      );
      throw error;
    }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  verifyFallbacks()
    .then((result) => {
      if (result.success) {
        console.log(
          chalk.green('Fallback verification completed successfully'),
        );
        process.exit(0);
      } else {
