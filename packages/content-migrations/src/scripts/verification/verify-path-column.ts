 ...
 * Verify that path column exists in relationship tables
 */
async function verifyPathColumn(): Promise<boolean> {
  ...
  try {
    ...
    if (functionCheck.rows.length > 0) {
      ...
      if (functionSource.includes('ADD COLUMN IF NOT EXISTS path')) {
        console.log(
          '✅ ensure_relationship_columns function includes path column',
        );
      } else {
        console.error(
          '❌ ensure_relationship_columns function does not include path column',
        );
        missingPathColumn = true;
      }
    } else {
