  ...
  try {
    ...
    try {
      ...
      if (!schemaValidation.isValid) {
        ...
        console.error(
          'This may cause issues during migration. Will attempt to continue.',
        );
      } else {
        console.log(
          'Schema validation successful. Available columns:',
          schemaValidation.columns.join(', '),
        );
      }
      ...
      try {
        ...
        if (funcExistsResult.rows[0]?.func_exists) {
          ...
          console.log(
            'UUID table scanner function not found, using direct column addition...',
          );
          ...
          console.log(
            `Added columns to ${uuidTablesResult.rowCount} UUID tables`,
          );
        }
      } catch (scanError) {
      ...
      console.log(
        `Starting migration of ${postFiles.length} private post files...`,
      );
      ...
      for (const file of postFiles) {
        try {
          ...
          const existingPostResult = await client.query(
            `SELECT id FROM payload.private WHERE slug = $1`,
            [slug],
          );
          ...
          if (existingPostResult.rows.length > 0) {
            ...
            console.log(
              `Updating existing private post with slug ${slug} and ID ${postId}`,
            );

            await client.query(
              `UPDATE payload.private SET
                title = $1,
                description = $2,
                content = $3,
                published_at = $4,
            ...
            try {
              await client.query(
                `DELETE FROM payload.private_categories WHERE _parent_id = $1`,
                [postId],
              );
            } catch (delError) {
              console.error(
                `Error deleting categories for private post ${postId}, but continuing:`,
                delError.message,
              );
            }
            ...
            try {
              await client.query(
                `DELETE FROM payload.private_tags WHERE _parent_id = $1`,
                [postId],
              );
            } catch (delError) {
              console.error(
                `Error deleting tags for private post ${postId}, but continuing:`,
                delError.message,
              );
            }
          } else {
            ...
            postId = uuidv4();
            console.log(
              `Creating new private post with slug ${slug} and ID ${postId}`,
            );
          ...
          if (data.categories && Array.isArray(data.categories)) {
            for (let i = 0; i < data.categories.length; i++) {
              try {
                ...
                console.error(
                  `Error adding category ${data.categories[i]} to private post ${postId}, but continuing:`,
                  catError.message,
                );
              }
            }
          ...
          if (data.tags && Array.isArray(data.tags)) {
            for (let i = 0; i < data.tags.length; i++) {
              try {
                ...
                console.error(
                  `Error adding tag ${data.tags[i]} to private post ${postId}, but continuing:`,
                  tagError.message,
                );
              }
            }
          ...
          try {
            ...
            const result = await safeInsertIntoUuidTablesTracking(
              client,
              uuidTableName,
            );
            if (!result) {
              console.log(
                `Note: UUID table tracking skipped for ${uuidTableName}`,
              );
            }
          } catch (trackingError) {
      ...
      try {
        const countResult = await client.query(
          `SELECT COUNT(*) FROM payload.private`,
        );
        const totalPosts = parseInt(countResult.rows[0].count);
        console.log(
          `Total private posts in database after migration: ${totalPosts}`,
        );

        if (totalPosts === 0) {
          console.warn(
            `WARNING: No private posts were found in the database after migration!`,
          );
          ...
          console.log(
            `Private table columns: ${tableResult.rows.map((r) => r.column_name).join(', ')}`,
          );
          ...
          console.log(
            `NOTE: No new private posts were migrated. All ${postFiles.length} private posts already exist in the database.`,
          );
        } else {
          console.log(
            `Successfully migrated/updated ${migratedCount} of ${postFiles.length} private posts. ✅`,
          );
        }
      } catch (verifyError) {
      ...
      console.log(
        `Total private posts in database: ${await client.query('SELECT COUNT(*) FROM payload.private').then((res) => parseInt(res.rows[0].count))}`,
      );
      console.log('=========================================');
    } finally {
      client.release();
