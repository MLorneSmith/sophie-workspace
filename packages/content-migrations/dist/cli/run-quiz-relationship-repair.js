...
async function main() {
    try {
        ...
        if (args['verify-only']) {
            ...
            const { verifyQuizSystem } = await import('../scripts/repair/quiz-system/verification.js');
            const db = await (await import('../scripts/repair/quiz-system/utils/db-connection.js')) // Corrected import path
        ...
        if (args['analyze-only']) {
            ...
            const { detectQuizRelationships, logDetectionSummary } = await import('../scripts/repair/quiz-system/detection.js');
            const db = await (await import('../scripts/repair/quiz-system/utils/db-connection.js')) // Corrected import path
                .getDbConnection({ schema: 'payload' });
            const state = await detectQuizRelationships(db);
            logDetectionSummary(state);
            logger.info(`Detection completed with ${state.issues.length} issues found`);
