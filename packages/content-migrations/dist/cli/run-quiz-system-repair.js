...
program
    ...
    .option('--analyze-only', 'Only analyze relationships, no repairs or verification', false)
    .parse(process.argv);
// Get options
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.primaryResult) {
                logger.info(`Created ${result.primaryResult.relationshipsCreated} primary relationships`);
            }
            if (result.bidirectionalResult) {
                logger.info(`Created ${result.bidirectionalResult.relationshipsCreated} bidirectional relationships`);
            }
            if (result.jsonbResult) {
                logger.info(`Updated JSONB format in ${result.jsonbResult.quizzesUpdated} quizzes`);
            }
            if (result.verificationResult) {
                logger.info(`Verification: ${result.verificationResult.success ? 'PASSED' : 'FAILED'}`);
                if (!result.verificationResult.success) {
                    logger.warning('Some verification issues remain. Try running again or check logs for details.');
                }
            }
            process.exit(0);
        }
