...
async function generateAllSqlSeedFiles() {
    try {
        ...
        await generateDownloadsSql(); // Add the downloads SQL generation
...
function loadLessonMetadata() {
    try {
        ...
        const metadata = jsYaml.load(fileContent);
...
`;
        ...
        const lessonValues = metadata.lessons
            ...
            if (lesson.slug === 'lesson-0') {
                ...
                console.log('todoFields object:', JSON.stringify(lesson.todoFields, null, 2));
            ...
            const todoContent = lesson.todoFields?.todo
                ...
                : 'NULL';
            const todoCompleteQuiz = lesson.todoFields?.completeQuiz === true ? 'true' : 'false';
...
`;
        ...
        metadata.lessons.forEach((lesson) => {
            ...
            if (!lesson.quiz || !quizMap[lesson.quiz])
                return;
            const lessonId = lesson.slug;
            const quizId = quizMap[lesson.quiz];
            // Add the relationship
            referenceValues.push(`(uuid_generate_v4(), '${lessonId}', '${quizId}', ${order++})`);
        });
...
async function generateSurveyQuestionsSql() {
    try {
        ...
        if (anyFileCopied) {
            console.log('Survey questions SQL generated successfully (from existing files)');
            return;
        }
...
async function generateDownloadsSql() {
    try {
        ...
        // Check if the existing file is already present - use it if so
        const existingFile = path.resolve(PAYLOAD_SEED_DIR, '11-downloads.sql');
        if (fs.existsSync(existingFile)) {
            const content = fs.readFileSync(existingFile, 'utf8');
            // Write to processed SQL directory
            const outputFile = path.resolve(SQL_DIR, '11-downloads.sql');
            fs.writeFileSync(outputFile, content);
            console.log('Downloads SQL generated successfully (from existing file)');
            return;
        }
...
('a5c7d9e8-3b21-4f67-9d85-2e7a41c0b593', 'The Why - Introductions', '203 The Why - Introductions.pdf', 'https://downloads.slideheroes.com/203 The Why - Introductions.pdf', 'application/pdf', 311899, 'reference'),
('b7c94e2d-1a63-4d85-b9f7-e21c8d9a6f04', 'The Why - Next Steps', '204 The Why - Next Steps.pdf', 'https://downloads.slideheroes.com/204 The Why - Next Steps.pdf', 'application/pdf', 285794, 'reference'),
('c8e5f931-2b74-4e96-a8c0-f32d7b0a5e18', 'Idea Generation', '301 Idea Generation.pdf', 'https://downloads.slideheroes.com/301 Idea Generation.pdf', 'application/pdf', 1593877, 'reference'),
('d9f6a042-3c85-5fa7-b9d1-143e8c1b6f29', 'What is Structure', '302 What is Structure.pdf', 'https://downloads.slideheroes.com/302 What is Structure.pdf', 'application/pdf', 480516, 'reference'),
('e017b153-4d96-6fb8-c0e2-354f9d2c7130', 'Using Stories', '401 Using Stories.pdf', 'https://downloads.slideheroes.com/401 Using Stories.pdf', 'application/pdf', 843451, 'reference'),
...
async function generateDocumentationSql() {
    try {
        ...
        if (fs.existsSync(existingFile)) {
            ...
            fs.writeFileSync(outputFile, content);
            console.log('Documentation SQL generated successfully (from existing file)');
            return;
        }
        // Generate placeholder SQL
