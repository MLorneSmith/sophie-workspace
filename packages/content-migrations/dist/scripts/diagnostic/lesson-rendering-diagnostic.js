...
export async function runLessonRenderingDiagnostic() {
    try {
      ...
      LIMIT 5`;
        ...
        if (contentResults.rowCount > 0) {
            contentResults.rows.forEach((row) => {
                console.log(`Lesson ${row.slug} (${row.id}): ${row.content_length} chars`);
      ...
      LIMIT 10`;
        ...
        if (downloadsResults.rowCount > 0) {
            downloadsResults.rows.forEach((row) => {
                console.log(`Lesson "${row.title}" (${row.slug}): ${row.download_count} downloads`);
            });
        }
        else {
      ...
      LIMIT 10`;
        ...
        if (tagsResults.rowCount > 0) {
            tagsResults.rows.forEach((row) => {
                console.log(`Lesson ${row.slug} (${row.id}): ${row.template_tag_count} template tags`);
            });
        }
        else {
      ...
      WHERE content IS NOT NULL`;
        ...
        LIMIT 5`;
            const sampleRelationshipsResult = await executeSQL(sampleRelationshipsQuery);
            ...
            if (sampleRelationshipsResult.rowCount > 0) {
                sampleRelationshipsResult.rows.forEach((row) => {
                    console.log(`Lesson ID: ${row.collection_id}, Download ID: ${row.download_id}`);
                });
            }
            else {
            ...
            console.error('Error querying downloads_relationships view:', error.message);
...
if (process.argv[1]?.endsWith('lesson-rendering-diagnostic.ts') ||
