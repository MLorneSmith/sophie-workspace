...
function checkPostFiles(postsDir) {
    ...
    try {
        ...
        const postFiles = fs
            ...
            file.endsWith('.mdoc') ||
            file.endsWith('.md'));
        console.log(`Found ${postFiles.length} potential post files (.html, .mdoc, .md)`);
        if (postFiles.length === 0) {
            ...
            if (allFiles.length > 0) {
                ...
                allFiles.forEach((file) => {
                    ...
                    const stats = fs.statSync(filePath);
                    console.log(`- ${file} [${path.extname(file) || 'NO EXTENSION'}] ${stats.isDirectory() ? '[DIR]' : `[FILE: ${stats.size} bytes]`}`);
                });
            }
            return;
        }
