...
function testLessonEnhancements() {
    ...
    try {
        ...
        try {
            ...
            execSync('npx tsc src/scripts/analyze-lesson-content.ts --outDir ./dist', {
                cwd: baseDir,
                stdio: 'inherit',
            });
            // Then run with node
        ...
        try {
            // Try to run with npx to ensure ts-node is available
            execSync('npx ts-node src/scripts/sql/generate-lesson-enhancements-sql.ts', {
                cwd: baseDir,
                stdio: 'inherit',
            });
        }
            ...
            console.log('Failed to run with npx ts-node, trying node with compiled JavaScript...');
            // Compile TypeScript first
            execSync('npx tsc src/scripts/sql/generate-lesson-enhancements-sql.ts --outDir ./dist', {
                cwd: baseDir,
                stdio: 'inherit',
            });
            // Then run with node
