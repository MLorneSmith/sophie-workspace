...
async function main() {
    ...
    `);
      ...
      `, [table]);
          ...
          `);
                    ...
                    const specialTableCheck = await pool.query(`SELECT column_name FROM information_schema.columns 
             ...
             LIMIT 5`, [table]);
                    if (specialTableCheck.rows.length > 0) {
                        ...
                        console.log(specialTableCheck.rows.map((r) => r.column_name).join(', '));
                    }
                }
            }
        }
    ...
    `);
      ...
      `);
        ...
        console.log('✅ Successfully ensured required columns exist on all UUID tables');
    }
    catch (error) {
