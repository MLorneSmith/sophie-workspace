 ...
 */
export async function generateSqlSeedFiles(outputDir) {
    ...
    fs.writeFileSync(path.join(outputDir, '01-relationship-tables.sql'), relationshipTablesSql);
    ...
    fs.writeFileSync(path.join(outputDir, '03a-lesson-quiz-references.sql'), referencesSql);
...
if (process.argv[1]?.endsWith('new-generate-sql-seed-files.ts') ||
    process.argv[1]?.endsWith('new-generate-sql-seed-files.js')) {
    // Default output directory
    const outputDir = process.argv[2] || path.resolve(process.cwd(), 'apps/payload/src/seed');
