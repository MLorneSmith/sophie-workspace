...
function loadEnvironmentVariables() {
    ...
    const payloadEnvPath = path.resolve(projectRoot, 'apps/payload/.env.development');
...
function logRelatedError(error, context) {
...
async function main() {
    ...
    try {
        ...
        if (!DATABASE_URI) {
            ...
            console.log('Using fallback database connection string:', `postgresql://${user}:***@${host}:${port}/${database}`);
            pool = new Pool({
                host,
    ...
    `);
        ...
        for (const table of tables) {
            ...
            try {
                ...
                const hasPathCol = await pool.query(`SELECT column_name FROM information_schema.columns 
           WHERE table_schema = 'payload' AND table_name = $1 AND column_name = 'path'`, [table]);
                if (hasPathCol.rows.length === 0) {
                    console.log(`  - Adding path column to ${table}`);
                    await pool.query(`ALTER TABLE payload.${table} ADD COLUMN IF NOT EXISTS path TEXT`);
                    tableFixed = true;
                }
                // Check and add parent_id column if needed
                const hasParentIdCol = await pool.query(`SELECT column_name FROM information_schema.columns 
           WHERE table_schema = 'payload' AND table_name = $1 AND column_name = 'parent_id'`, [table]);
                if (hasParentIdCol.rows.length === 0) {
                    console.log(`  - Adding parent_id column to ${table}`);
                    await pool.query(`ALTER TABLE payload.${table} ADD COLUMN IF NOT EXISTS parent_id TEXT`);
                    tableFixed = true;
                }
                // Check and add downloads_id column if needed
                const hasDownloadsIdCol = await pool.query(`SELECT column_name FROM information_schema.columns 
           WHERE table_schema = 'payload' AND table_name = $1 AND column_name = 'downloads_id'`, [table]);
                if (hasDownloadsIdCol.rows.length === 0) {
                    console.log(`  - Adding downloads_id column to ${table}`);
                    await pool.query(`ALTER TABLE payload.${table} ADD COLUMN IF NOT EXISTS downloads_id UUID`);
                    tableFixed = true;
                }
                // Check and add media_id column if needed
                const hasMediaIdCol = await pool.query(`SELECT column_name FROM information_schema.columns 
           WHERE table_schema = 'payload' AND table_name = $1 AND column_name = 'media_id'`, [table]);
                if (hasMediaIdCol.rows.length === 0) {
                    console.log(`  - Adding media_id column to ${table}`);
                    await pool.query(`ALTER TABLE payload.${table} ADD COLUMN IF NOT EXISTS media_id UUID`);
                    tableFixed = true;
                }
                // Check and add private_id column if needed
                const hasPrivateIdCol = await pool.query(`SELECT column_name FROM information_schema.columns 
           WHERE table_schema = 'payload' AND table_name = $1 AND column_name = 'private_id'`, [table]);
                if (hasPrivateIdCol.rows.length === 0) {
                    console.log(`  - Adding private_id column to ${table}`);
                    await pool.query(`ALTER TABLE payload.${table} ADD COLUMN IF NOT EXISTS private_id UUID`);
                    tableFixed = true;
                }
                ...
                console.error(`  ❌ Error fixing table ${table}: ${tableError.message}`);
            }
        }
      ...
      `);
            ...
            console.error('Warning: Error creating helper functions and views:', functionsError.message);
        ...
        try {
            ...
            console.error('Warning: Error executing scan_and_fix_uuid_tables function:', scanError.message);
            console.error('This is non-critical and will not affect the migration');
        }
        console.log(`\n=== SUMMARY ===`);
        ...
        if (overallSuccess) {
            console.log(`✅ Successfully ensured required columns exist on UUID tables (${Math.round(successRate * 100)}% success rate)`);
        }
        else {
            console.log(`⚠️ Partially ensured required columns exist (${Math.round(successRate * 100)}% success rate)`);
        }
    }
    catch (error) {
