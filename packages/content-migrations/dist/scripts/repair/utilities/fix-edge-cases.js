...
async function main() {
    ...
    `);
        ...
        console.log('Validating unidirectional relationships from quizzes to questions...');
      ...
      `);
            console.log('Available tables:', tableCheckResult.rows.map((r) => r.table_name).join(', '));
      ...
      `);
            console.log('course_quizzes_rels columns:', columnCheckResult.rows.map((r) => r.column_name).join(', '));
      ...
      `)
                ...
                if (invalidCount > 0) {
                    console.log(`Found ${invalidCount} references to non-existent questions. These will be left as-is.`);
                }
                else {
