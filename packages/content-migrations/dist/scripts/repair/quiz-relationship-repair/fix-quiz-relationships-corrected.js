...
export async function fixQuizRelationships() {
    ...
    `);
        ...
        for (const quiz of dbQuizzes) {
            ...
            const slugToFind = Object.keys(QUIZZES).find((slug) => QUIZZES[slug].id === quiz.id || QUIZZES[slug].slug === quiz.slug);
            if (!slugToFind) {
        ...
        `, [quiz.id]);
            ...
            console.log(`Found ${existingRelEntries.length} existing relationship entries`);
            ...
            if (existingRelEntries.length === 0) {
               ...
               VALUES 
                ($1, $2, 'questions', 'questions', $3, $4)`, [relationshipId, quiz.id, i, questionId]);
                        console.log(`Created relationship for question ${questionId} at order ${i}`);
                    }
                    catch (error) {
                        console.error(`Error creating relationship for question ${questionId}: ${error}`);
                   ...
                   WHERE id = $1 AND _parent_id = $2 AND path = 'questions'`, [questionId, quiz.id, i]);
                                console.log(`Updated existing relationship for question ${questionId} to order ${i}`);
                            }
                            catch (updateError) {
                                console.error(`Error updating relationship for question ${questionId}: ${updateError}`);
                            }
                        }
                    }
                }
                ...
                console.log(`Skipping questions array update as the column doesn't exist`);
            ...
            else if (existingRelEntries.length !== questionIds.length ||
                !areOrderedArraysEqual(existingRelEntries.map((r) => r.id), questionIds)) {
                console.log(`Updating relationship entries for quiz "${quiz.title}"`);
                // List out the differences for debugging
                console.log(`Existing relationships: ${existingRelEntries.map((r) => r.id).join(', ')}`);
                console.log(`Defined questions: ${questionIds.join(', ')}`);
                // Delete existing relationships
                await executeSQL(`DELETE FROM payload.course_quizzes_rels
           WHERE _parent_id = $1 AND path = 'questions'`, [quiz.id]);
                console.log(`Deleted ${existingRelEntries.length} existing relationships`);
    ...
    `);
        if (verificationResult.rows.length > 0) {
            console.warn(`${verificationResult.rows.length} quizzes still have no relationships:`);
        ...
        const reportFile = path.join(outputDir, 'quiz-relationship-repair-corrected.md');
...
function areOrderedArraysEqual(arr1, arr2) {
