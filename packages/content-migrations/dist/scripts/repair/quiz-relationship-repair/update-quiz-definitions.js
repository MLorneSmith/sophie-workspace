...
const quizzesFilePath = path.join(process.cwd(), '..', '..', 'packages', 'content-migrations', 'src', 'data', 'definitions', 'quizzes.ts');
// Path to output the new QUIZZES definition
const outputFilePath = path.join(process.cwd(), 'z.plan', 'quizzes', 'updated-quizzes.ts');
...
const TITLE_TO_SLUG_MAP = {
    ...
    'The Fundamental Elements of Design in Detail Quiz': 'elements-of-design-detail-quiz',
    ...
    'Overview of the Fundamental Elements of Design Quiz': 'overview-elements-of-design-quiz',
    ...
    `);
        ...
        for (const quiz of quizzes) {
            ...
            if (expectedId !== quiz.id) {
                console.warn(`Warning: ID mismatch for quiz "${quiz.title}": Database ID ${quiz.id}, Expected ID ${expectedId}`);
            }
            ...
            if (slug === 'the-who-quiz') {
                ...
                quizQuestions = allQuestions
                    .filter((q) => q.question.toLowerCase().includes('who') ||
                ...
                quizQuestions = allQuestions
                    .filter((q) => keywordMatches.some((keyword) => q.question.toLowerCase().includes(keyword.toLowerCase())))
                    .slice(0, 5) // Limit to 5 questions if too many matches
                    .map(processQuestion);
            }
            // If no questions found, use some generic questions
            if (quizQuestions.length === 0) {
                console.warn(`Warning: No questions found for quiz "${quiz.title}". Using generic questions.`);
                quizQuestions = allQuestions
        ...
        console.log('Review the file and replace the original quizzes.ts if it looks good.');
...
function getKeywordsFromTitle(title) {
    ...
    if (lowerTitle.includes('persuasion'))
        keywords.push('persuasion');
    if (lowerTitle.includes('gestalt'))
        keywords.push('gestalt');
    if (lowerTitle.includes('idea'))
        keywords.push('idea');
    if (lowerTitle.includes('process'))
        keywords.push('process');
    if (lowerTitle.includes('element'))
        keywords.push('element');
    if (lowerTitle.includes('performance'))
        keywords.push('performance');
    if (lowerTitle.includes('practice'))
        keywords.push('practice');
    if (lowerTitle.includes('slide'))
        keywords.push('slide');
    if (lowerTitle.includes('composition'))
        keywords.push('composition');
    if (lowerTitle.includes('storyboard'))
        keywords.push('storyboard');
    if (lowerTitle.includes('structure'))
        keywords.push('structure');
    if (lowerTitle.includes('table'))
        keywords.push('table');
    if (lowerTitle.includes('who'))
        keywords.push('who');
    if (lowerTitle.includes('why'))
        keywords.push('why');
    if (lowerTitle.includes('story'))
        keywords.push('story');
    if (lowerTitle.includes('visual'))
        keywords.push('visual');
    if (lowerTitle.includes('perception'))
        keywords.push('perception');
    // If no keywords found, use the most specific part of the title
    if (keywords.length === 0) {
        const parts = title.split(' ');
        const filteredParts = parts.filter((part) => !['quiz', 'the', 'and', 'of', 'in', 'for', '&'].includes(part.toLowerCase()));
...
// Generate the updated quizzes.ts file
function generateQuizzesFile(quizDefinitions) {
