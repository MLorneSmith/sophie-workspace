    ...
    `);
      ...
      `, [quiz.id]);
            ...
            if (questions.length > 0 && relEntries.length === 0) {
                console.log(`  Warning: Quiz has questions array but no relationship entries`);
                ...
                console.log(`  Warning: Quiz has relationship entries but no questions array`);
                results.quizzesWithMissingRelationships++;
                quizDetail.hasMissingRelationships = true;
            }
            else if (questions.length !== relEntries.length) {
                console.log(`  Warning: Quiz has different number of questions (${questions.length}) and relationship entries (${relEntries.length})`);
                results.quizzesWithMissingRelationships++;
                quizDetail.hasMissingRelationships = true;
            }
