 ...
 */
async function fixLessonQuizFieldNames(client) {
    ...
    `);
        if (checkColumn.rowCount === 0) {
            console.log('quiz_id column not found in course_lessons table, skipping field name fix');
            return;
        }
    ...
    `);
        console.log(`Found ${result.rowCount} lessons with quiz_id but without quiz_id_id`);
        ...
        AND (quiz_id_id IS NULL OR quiz_id_id = '')
      `);
            console.log(`Updated ${updateResult.rowCount} lessons with quiz_id_id from quiz_id`);
        }
        // Verify the fix
    ...
    `);
        if (parseInt(verifyResult.rows[0].count) === 0) {
            ...
            console.warn(`⚠️ ${verifyResult.rows[0].count} lessons still have inconsistent field names`);
        }
    }
    catch (error) {
 ...
 */
async function fixLessonQuizReferences(client) {
 ...
 */
async function fixLessonQuizReferencesWithSql(client) {
    ...
    `);
        console.log(`Created ${createRelResult.rowCount} missing relationship entries from quiz_id_id values`);
      ...
      RETURNING id
    `);
        console.log(`Removed ${removeInvalidRelsResult.rowCount} invalid relationship entries`);
    ...
    `);
        console.log(`Fixed ${fixCourseQuizzesIdResult.rowCount} relationship entries with incorrect course_quizzes_id`);
    }
    catch (error) {
...
async function verifyLessonQuizRelationships(client) {
    ...
    `);
      ...
      WHERE quiz_id_id IS NOT NULL AND quiz_id_id != ''
    `);
        const totalLessonsWithQuiz = parseInt(totalLessonsWithQuizResult.rows[0].count);
        // Report results
        console.log('\nVerification Results:');
        console.log(`- Total lessons with quiz_id_id: ${totalLessonsWithQuiz}`);
        console.log(`- Total lesson-quiz relationships: ${totalRels}`);
        console.log(`- Lessons with quiz_id_id but no relationship: ${missingRels}`);
