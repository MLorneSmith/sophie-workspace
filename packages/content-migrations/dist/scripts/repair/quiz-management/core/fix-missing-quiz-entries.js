 ...
 */
export async function fixMissingQuizEntries() {
    ...
    `);
        ...
        console.log(`Found ${missingQuizzes.length} missing quiz entries referenced by lessons`);
        ...
        for (const missingQuiz of missingQuizzes) {
            ...
            const quizSlug = lessonTitle
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, '-')
                .replace(/^-|-$/g, '') + '-quiz';
            console.log(`Creating missing quiz: "${quizTitle}" (${quizId}) for lesson: "${lessonTitle}"`);
      ...
      `, [
        ...
        console.log('Searching for quiz questions to associate with new quizzes...');
      ...
      `, [
            ...
            console.log(`Found ${potentialQuestions.length} potential questions for quiz "${quizTitle}"`);
        ...
        `, [questionIds, quizId]);
          ...
          `, [
                    ...
                    console.log(`Associated question "${question.question}" with quiz "${quizTitle}"`);
                }
            }
            else {
    ...
    `, createdQuizIds);
        ...
        verificationResult.rows.forEach((row) => {
            console.log(`Quiz "${row.title}" (${row.id}): ${row.question_count} questions in array, ${row.rel_count} in relationships, Has course_id: ${row.has_course_id}`);
        });
