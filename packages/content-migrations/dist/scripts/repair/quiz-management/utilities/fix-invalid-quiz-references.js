 ...
 */
export async function fixInvalidQuizReferences() {
    ...
    `);
        if (fixJsonResult.rowCount > 0) {
            console.log(`Fixed ${fixJsonResult.rowCount} lessons with invalid JSON object quiz references:`);
    ...
    `);
        const invalidCount = parseInt(checkResult.rows[0]?.invalid_count || '0', 10);
        if (invalidCount > 0) {
        ...
        RETURNING id, title
      `);
            console.log(`Updated ${updateResult.rowCount} lessons to remove invalid quiz references:`);
    ...
    `);
        ...
        RETURNING id, title
      `, [courseId]);
            console.log(`Updated ${updateResult.rowCount} quizzes to have course ID ${courseId}:`);
            updateResult.rows.forEach((row) => {
                console.log(`- Quiz ID: ${row.id}, Title: ${row.title}`);
            });
        }
        else {
            console.warn('Could not find main course ID. Please update quiz course IDs manually.');
        }
        await client.query('COMMIT');
        console.log('Successfully fixed invalid quiz references and added course IDs');
    }
    catch (error) {
