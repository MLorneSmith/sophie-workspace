...
async function verifyEnhancedFix() {
    console.log('Starting verification of enhanced quiz-question relationship fix...');
    let success = true;
    try {
        // 1. Check if questions array column exists in course_quizzes
        console.log('Checking if questions array column exists in course_quizzes...');
    ...
    `);
        if (columnResult.rows.length === 0) {
            console.error('❌ Questions array column does not exist in course_quizzes table');
            success = false;
        }
        else {
        ...
        console.log('Checking if quizzes have questions in their questions array...');
    ...
    `);
        ...
        for (const quiz of quizzes) {
            ...
            if (!questions || questions.length === 0) {
                ...
                console.log(`Quiz "${quiz.title}" (${quiz.id}) has no questions in its array`);
            }
            else {
                ...
                console.log(`Quiz "${quiz.title}" (${quiz.id}) has ${questionIds.length} questions in its array`);
            }
        }
        console.log(`✅ ${quizzesWithQuestions} quizzes have questions in their array`);
        console.log(`ℹ️ ${quizzesWithoutQuestions} quizzes have no questions in their array`);
    ...
    `);
        if (relsResult.rows.length === 0) {
            console.error('❌ No quiz-question relationship entries found in course_quizzes_rels');
            success = false;
        }
        else {
            console.log(`✅ Found ${relsResult.rows.length} relationship entries in course_quizzes_rels`);
        }
        // 4. Check for inconsistencies between questions array and relationship entries
        console.log('Checking for inconsistencies between questions array and relationship entries...');
        ...
        `, [quiz.id, questionId]);
                if (relResult.rows.length === 0) {
                    console.error(`❌ Inconsistency: Quiz ${quiz.id} has question ${questionId} in its array but no corresponding relationship entry`);
                    inconsistencies++;
                }
            }
        }
        if (inconsistencies === 0) {
            console.log('✅ No inconsistencies found between questions arrays and relationship entries');
        }
        else {
            console.error(`❌ Found ${inconsistencies} inconsistencies between questions arrays and relationship entries`);
            success = false;
        }
    ...
    `);
      ...
      `, [tableName]);
            ...
            if (missingColumns.length > 0) {
                console.error(`❌ UUID table ${tableName} is missing columns: ${missingColumns.join(', ')}`);
                tablesWithMissingColumns++;
            }
        }
        if (tablesWithMissingColumns === 0) {
            console.log(`✅ All ${uuidTables.length} UUID tables have the required columns`);
        }
        else {
            console.error(`❌ ${tablesWithMissingColumns} UUID tables are missing required columns`);
            success = false;
        }
        // Final verification result
        if (success) {
            console.log('🎉 Enhanced quiz-question relationship fix verification PASSED');
            console.log('Questions should now be visible in Payload CMS admin interface');
        }
        else {
            console.error('❌ Enhanced quiz-question relationship fix verification FAILED');
