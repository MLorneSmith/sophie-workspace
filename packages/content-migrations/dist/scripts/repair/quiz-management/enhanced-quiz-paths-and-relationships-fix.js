 ...
 */
export async function enhancedQuizPathsAndRelationships() {
    ...
    try {
        ...
        console.log(`- quiz_questions_rels entries: ${initialState.questionRelCount}`);
        console.log(`- course_quizzes_rels with NULL path: ${initialState.pathNullCount}`);
        console.log(`- Duplicate relationship records: ${initialState.duplicateCount}`);
        console.log(`- Quizzes with mismatched arrays: ${initialState.mismatchedArrayCount}`);
    ...
    `);
        ...
        console.log(`Removed ${duplicatesRemoved} duplicate records from course_quizzes_rels`);
    ...
    `);
        ...
        console.log(`Created ${relationshipsCreated} bidirectional relationships in quiz_questions_rels`);
        // Step 4: Normalize questions arrays to match relationship records
        console.log('Normalizing questions arrays to match relationship records...');
    ...
    `);
        ...
        console.log(`- quiz_questions_rels entries: ${finalState.questionRelCount}`);
        console.log(`- course_quizzes_rels with NULL path: ${finalState.pathNullCount}`);
        console.log(`- Duplicate relationship records: ${finalState.duplicateCount}`);
        console.log(`- Quizzes with mismatched arrays: ${finalState.mismatchedArrayCount}`);
        if (finalState.pathNullCount === 0 &&
            finalState.matchingCount &&
            finalState.duplicateCount === 0 &&
            finalState.mismatchedArrayCount === 0) {
            console.log('✅ All issues have been fixed! Path fields, relationship tables, and questions arrays are now correct.');
        }
        else {
            console.warn('⚠️ Some issues still remain after the fix:');
            if (finalState.pathNullCount > 0) {
                console.warn(`- ${finalState.pathNullCount} records in course_quizzes_rels still have NULL path`);
            }
            if (!finalState.matchingCount) {
                console.warn(`- Relationship counts still don't match: ${finalState.quizRelCount} vs ${finalState.questionRelCount}`);
            }
            if (finalState.duplicateCount > 0) {
                console.warn(`- ${finalState.duplicateCount} duplicate relationship records still exist`);
            }
            if (finalState.mismatchedArrayCount > 0) {
                console.warn(`- ${finalState.mismatchedArrayCount} quizzes still have mismatched question arrays`);
            }
        }
        // Generate a report
 ...
 */
async function verifyRelationshipConsistency() {
...
function generateReport(data) {
    const { duplicatesRemoved, pathsFixed, relationshipsCreated, arraysNormalized, initialState, finalState, } = data;
...
`);
        ...
        .catch((error) => {
        console.error('Error in enhanced quiz paths and relationships fix:', error);
        process.exit(1);
    });
}
