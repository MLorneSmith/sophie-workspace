 ...
 */
export async function fixBidirectionalQuizRelationships() {
    ...
    try {
        ...
        if (initialState.matchingCount) {
            console.log('Relationship tables already have matching counts. Skipping fix.');
    ...
    `);
        ...
        if (finalState.matchingCount) {
            ...
            console.warn(`Missing quiz_questions_rels entries: ${finalState.missingQuestionRels.length}`);
            ...
            if (finalState.missingQuestionRels.length > 0) {
                ...
                finalState.missingQuestionRels.slice(0, 5).forEach((rel) => {
                    console.warn(`- Quiz ID: ${rel.quiz_id}, Question ID: ${rel.question_id}`);
                });
                if (finalState.missingQuestionRels.length > 5) {
                    console.warn(`... and ${finalState.missingQuestionRels.length - 5} more`);
                }
            }
        }
        // Generate a report
        ...
        const reportFile = path.join(outputDir, 'bidirectional-relationship-fix-report.md');
