 ...
 */
async function fixQuestionQuizRelationshipsComprehensive() {
    ...
    `);
        if (!relsTableResult.rows[0].exists) {
            throw new Error('course_quizzes_rels table does not exist. This is a critical error.');
        }
        else {
            console.log('course_quizzes_rels table exists for unidirectional relationships');
        }
    ...
    `);
        ...
        const orphanedQuestions = [...questionIds].filter((id) => !relatedQuestionIds.has(id));
        console.log(`Found ${orphanedQuestions.length} questions not associated with any quiz`);
    ...
    `);
        const { total_questions, questions_in_relationships } = verificationResult.rows[0];
...
- Orphaned questions: ${total_questions - questions_in_relationships}
    `);
        if (orphanedQuestions.length === 0) {
            ...
            console.warn(`⚠️ Warning: Found ${orphanedQuestions.length} questions not associated with any quiz`);
        }
        // Commit transaction
