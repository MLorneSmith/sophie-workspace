...
export async function fixQuizRelationshipsComplete() {
    ...
    try {
        ...
        if (results && results.length >= 3) {
            ...
            console.log(`- Quizzes with course ID: ${results[0].quizzes_with_course_id}`);
            console.log(`- Course relationships: ${results[1].course_relationships}`);
            console.log(`- Question relationships: ${results[2].question_relationships}`);
            // Check if all quizzes have course IDs
            const allQuizzesResult = await client.query('SELECT COUNT(*) as total FROM payload.course_quizzes');
            ...
            console.log('Script executed but verification results were not as expected');
        }
