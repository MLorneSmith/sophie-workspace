...
export async function fixQuizCourseIds() {
    ...
    try {
        ...
        console.log(`- Quizzes with direct course ID: ${beforeStats.withDirectId} → ${afterStats.withDirectId}`);
        console.log(`- Quizzes with relationship entries: ${beforeStats.withRelationship} → ${afterStats.withRelationship}`);
        // Determine if fix was successful
        const allValid = afterStats.withDirectId === afterStats.totalQuizzes &&
            afterStats.withRelationship === afterStats.totalQuizzes;
        if (allValid) {
            console.log('\n✅ SUCCESS: All quizzes now have proper course IDs');
        }
        else if (afterStats.withDirectId > beforeStats.withDirectId ||
            afterStats.withRelationship > beforeStats.withRelationship) {
            console.log('\n⚠️ PARTIAL SUCCESS: Some quizzes fixed, but issues remain');
            console.log(`- ${afterStats.totalQuizzes - afterStats.withDirectId} quizzes missing direct course ID`);
            console.log(`- ${afterStats.totalQuizzes - afterStats.withRelationship} quizzes missing relationship entries`);
        }
        else {
            console.log('\n❌ FAILURE: No improvements were made');
        }
        // If there are invalid quizzes, list them
        const invalidQuizzes = detailedStatus.filter((quiz) => quiz.status === 'INVALID');
 ...
 */
async function getQuizStats(client) {
    try {
        // Query quiz stats
    ...
    `);
        ...
        const withRelationship = parseInt(relResult.rows[0].with_relationship || '0');
