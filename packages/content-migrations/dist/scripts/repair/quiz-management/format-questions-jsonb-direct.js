...
export const formatQuestionsJSONBDirect = async () => {
    try {
    ...
    `);
        ...
        `);
                logSuccess(`Updated ${formattedQuestions.length} questions for quiz "${quiz.quiz_title}"`);
                successCount++;
            }
            catch (err) {
        ...
        await fixSpecificQuiz(db, designQuizId, 'The Fundamental Elements of Design in Detail Quiz');
        ...
        logSuccess(`Direct format fix complete: processed ${successCount} quizzes successfully, ${errorCount} with errors`);
        return true;
    }
    catch (error) {
...
async function fixSpecificQuiz(db, quizId, quizName) {
    try {
    ...
    `);
      ...
      `);
        ...
        logSuccess(`Fixed specific quiz: ${quizName} with ${formattedQuestions.length} questions`);
    }
    catch (error) {
