...
export const formatQuestionsJSONB = async () => {
    // Database connection
    ...
    try {
        ...
        logAction('Formatting quiz questions JSONB arrays for Payload compatibility');
        // Begin transaction
    ...
    `);
        ...
        for (const [quizId, quizData] of quizMap.entries()) {
            ...
            await client.query(`UPDATE payload.course_quizzes
         SET questions = $1
         WHERE id = $2`, [JSON.stringify(formattedQuestions), quizId]);
            // Check for each question if it has a relationship record
            for (const question of quizData.questions) {
                // Check if relationship record exists
                const relationshipCheckResult = await client.query(`SELECT COUNT(*) as count
           FROM payload.course_quizzes_rels
           WHERE _parent_id = $1
           AND quiz_questions_id = $2
           AND field = 'questions'`, [quizId, question.questionId]);
                // If no relationship exists, create one
                if (relationshipCheckResult.rows[0].count === '0') {
                    await client.query(`INSERT INTO payload.course_quizzes_rels
             (_parent_id, field, quiz_questions_id, order)
             VALUES ($1, 'questions', $2, $3)`, [quizId, question.questionId, question.order || 0]);
                    relationshipsAdded++;
                    logAction(`Created missing relationship for quiz "${quizData.title}" and question ${question.questionId}`);
                }
            }
            successCount++;
            logAction(`Formatted questions for quiz "${quizData.title}" (${quizId}) with ${formattedQuestions.length} questions`);
        }
    ...
    `);
        if (performanceQuizResult.rows.length > 0) {
            logAction(`Found Performance Quiz: ${performanceQuizResult.rows[0].title} (${performanceQuizResult.rows[0].id})`);
      ...
      `, [performanceQuizResult.rows[0].id]);
            logAction(`Performance Quiz has ${questionCountResult.rows[0].count} related questions`);
        }
        else {
            logError('Performance Quiz not found! This may indicate a deeper data issue.');
        }
        // Commit transaction
        await client.query('COMMIT');
        logSuccess(`Successfully formatted questions JSONB for ${successCount} quizzes, added ${relationshipsAdded} missing relationships`);
        return true;
    }
    catch (error) {
