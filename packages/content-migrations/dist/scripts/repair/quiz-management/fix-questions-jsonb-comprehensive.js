 ...
 */
export const fixQuestionsJSONBComprehensive = async () => {
    try {
    ...
    `);
        ...
        for (const quiz of quizzes.rows) {
            ...
            if (quiz.questions_json && quiz.questions_json !== 'null') {
                try {
                    ...
                    logWarning(`Failed to parse questions JSON for quiz "${quiz.quiz_title}": ${e.message}`);
                }
            }
      ...
      `);
            ...
            const relationshipIds = relationshipRecords.rows.map((r) => r.question_id);
            ...
            if (allQuestionIds.length === 0) {
                logWarning(`No questions found for quiz "${quiz.quiz_title}" - skipping`);
                continue;
            }
            // Log the data we found
            logAction(`Found ${questionIds.length} questions in JSONB and ${relationshipIds.length} in relationship table`);
          ...
          `);
                    relationshipsAdded++;
                    logAction(`Created missing relationship for quiz ${quiz.quiz_title} and question ${questionId}`);
                }
            }
      ...
      `);
          ...
          - Type: ${result.type}
          - Count: ${result.count}
          - First element format: ${result.json_string && result.json_string !== 'null'
                    ? JSON.parse(result.json_string)[0]
                        ? JSON.stringify(JSON.parse(result.json_string)[0]).substring(0, 100)
                        : 'No elements'
                    : 'null'}
        `);
            }
            logSuccess(`Fixed quiz "${quiz.quiz_title}" with ${formattedQuestions.length} questions`);
            successCount++;
        }
 ...
 */
async function fixSpecificQuiz(db, quizId) {
    try {
        // Get quiz info
        const quizInfo = await db.execute(sql.raw `
      SELECT title FROM payload.course_quizzes WHERE id::text = ${quizId}
    `);
        const quizTitle = quizInfo.rows.length > 0 ? quizInfo.rows[0].title : `Quiz ${quizId}`;
        logAction(`Applying special fix for problematic quiz: ${quizTitle} (${quizId})`);
    ...
    `);
        logSuccess(`Applied special fix to "${quizTitle}" with ${questionIds.length} questions`);
    }
    catch (error) {
