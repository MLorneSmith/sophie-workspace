 ...
 */
export const formatQuestionsJSONBEnhanced = async () => {
    try {
        console.log(chalk.cyan('Running enhanced JSONB formatting for quiz questions'));
        // Begin transaction
      ...
      `);
            ...
            for (const quiz of quizzes.rows) {
                try {
                    ...
                    try {
                        ...
                        console.error(chalk.red(`Quiz ${quiz.title} (${quiz.id}) has invalid JSON in questions: ${e.message}`));
                        formatErrors++;
                        continue;
                    }
                    // Skip if not an array
                    if (!Array.isArray(questions)) {
                        console.error(chalk.red(`Quiz ${quiz.title} (${quiz.id}) has non-array questions: ${typeof questions}`));
                        formatErrors++;
                        continue;
                    }
                    ...
                    const formattedQuestions = questions.map((questionId) => {
                        ...
                        const id = typeof questionId === 'object'
                            ? questionId.id ||
                    ...
                    const beforeSample = Array.isArray(questions) && questions.length > 0
                        ? JSON.stringify(questions.slice(0, 2))
                        : 'empty or invalid';
                    const afterSample = formattedQuestions.length > 0
                        ? JSON.stringify(formattedQuestions.slice(0, 2))
                        : 'empty';
                    console.log(chalk.blue(`Quiz ${quiz.title} (${quiz.id}): Before: ${beforeSample}`));
                    console.log(chalk.blue(`Quiz ${quiz.title} (${quiz.id}): After: ${afterSample}`));
          ...
          `, [JSON.stringify(formattedQuestions), quiz.id]);
                    ...
                    console.error(chalk.red(`Error formatting quiz ${quiz.title} (${quiz.id}): ${error.message}`));
                    formatErrors++;
                }
            }
      ...
      `);
            if (whoQuiz.rows.length > 0) {
                console.log(chalk.cyan(`Special handling for "The Who" Quiz: ${whoQuiz.rows[0].title} (${whoQuiz.rows[0].id})`));
        ...
        `, [whoQuiz.rows[0].id]);
          ...
          `, [JSON.stringify(formattedQuestions), whoQuiz.rows[0].id]);
                    console.log(chalk.green(`Directly fixed "The Who" Quiz with ${formattedQuestions.length} questions`));
                }
                else {
                    console.warn(chalk.yellow(`No questions found in relationships for "The Who" Quiz`));
                }
            }
            // Performance Quiz
      ...
      `);
            if (performanceQuiz.rows.length > 0) {
                console.log(chalk.cyan(`Special handling for Performance Quiz: ${performanceQuiz.rows[0].title} (${performanceQuiz.rows[0].id})`));
                // Get questions from relationship table
                const questionRels = await executeSQL(`
          SELECT quiz_questions_id FROM payload.course_quizzes_rels 
          WHERE _parent_id::text = $1 AND field = 'questions'
        `, [performanceQuiz.rows[0].id]);
          ...
          `, [JSON.stringify(formattedQuestions), performanceQuiz.rows[0].id]);
                    console.log(chalk.green(`Directly fixed Performance Quiz with ${formattedQuestions.length} questions`));
                }
                else {
                    console.warn(chalk.yellow(`No questions found in relationships for Performance Quiz`));
                }
            }
      ...
      `);
            ...
            console.log(chalk.green(`Successfully formatted ${successCount} quizzes with ${formatErrors} errors`));
            // Final verification
      ...
      `);
            console.log(chalk.green(`Verified ${verificationCount.rows[0].count} quizzes with properly formatted question arrays`));
        ...
        console.error(chalk.red(`Failed to format questions JSONB: ${error.message}`));
        return false;
    }
};
/**
...
async function main() {
    try {
        ...
        console.error(chalk.red('Unhandled error formatting quiz questions:'), error);
        process.exit(1);
    }
}
