...
async function verifyRelationships() {
    ...
    `);
        ...
        const inconsistentQuizzes = results.rows.filter((row) => !row.is_consistent).length;
        if (inconsistentQuizzes > 0) {
            console.log(`Found ${inconsistentQuizzes} of ${totalQuizzes} quizzes with inconsistent relationships:`);
            results.rows
                .filter((row) => !row.is_consistent)
                .forEach((quiz) => {
                console.log(`- Quiz "${quiz.quiz_title}" (${quiz.quiz_id}): has ${quiz.array_count} questions in array but ${quiz.rel_count} in relationship table`);
                if (quiz.missing_questions && quiz.missing_questions.length > 0) {
                    console.log(`  Missing question IDs: ${quiz.missing_questions.join(', ')}`);
                }
            });
            return { success: false, totalQuizzes, inconsistentQuizzes };
        }
        console.log(`All quizzes have consistent relationships (${totalQuizzes} total)`);
...
async function applyFixes() {
    ...
    `);
        ...
        if (totalFixedQuizzes > 0 || totalFixedRelationships > 0) {
            console.log(`Fixed ${totalFixedQuizzes} quizzes and ${totalFixedRelationships} relationships`);
        }
        else {
...
async function main() {
    try {
        ...
        if (!migrationExists) {
            ...
            console.error('Please run the migration first: apps/payload/src/migrations/20250425_150000_consolidated_quiz_relationship_fix.ts');
            process.exit(1);
        }
        ...
        if (!verification.success) {
            ...
            if (fixResult) {
                ...
                if (postFixVerification.success) {
                    ...
                    console.error('Please check the database manually or reapply the migration.');
                    process.exit(1);
                }
            }
            else {
