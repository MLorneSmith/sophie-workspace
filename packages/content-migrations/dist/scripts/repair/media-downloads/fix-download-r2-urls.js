 ...
 */
export async function fixDownloadR2Urls() {
    ...
    `);
      ...
      `, [presentationToolsLessonId, presentationTemplateId]);
        ...
        `, [presentationToolsLessonId, presentationTemplateId]);
                console.log(`Created missing relationship between Lesson 104 and Presentation Template`);
            }
            else {
                console.log(`Relationship between Lesson 104 and Presentation Template already exists`);
            }
      ...
      `, [presentationTemplateId, presentationToolsLessonId]);
            console.log(`Removed Presentation Template from all lessons except Lesson 104`);
        }
        else {
            console.warn(`Could not find Lesson 104 - Presentation Tools & Course Resources`);
        }
      ...
      `, [lesson.slug, `%${lesson.lessonNumber}%`]);
          ...
          WHERE lesson_id = $1
        `, [lessonId]);
                console.log(`Removed ${deleteResult.rowCount} download relationships from Lesson ${lesson.lessonNumber}`);
            }
            else {
        ...
        const countResult = await client.query("SELECT COUNT(*) FROM payload.downloads WHERE url IS NOT NULL AND url NOT LIKE '%placeholder%'");
        await client.query('COMMIT');
        console.log(`Successfully updated download R2 URLs. ${countResult.rows[0].count} downloads now have valid URLs.`);
    }
    catch (error) {
