...
async function fixDownloadsRelationships() {
    console.log('Fixing Downloads relationships and removing problematic view...');
    try {
        // Connect to database
        ...
        try {
            ...
            console.log('Dropped downloads_diagnostic view that was causing ALTER TABLE errors');
        }
        catch (viewError) {
    ...
    `);
        const hasRelationship = parseInt(checkRelationshipResult.rows[0].count, 10) > 0;
        if (hasRelationship) {
            console.log('Our Process lesson already has a relationship with the Our Process PDF');
        }
        else {
      ...
      `);
            console.log('Created relationship between Our Process lesson and Our Process PDF');
        }
    ...
    `);
      ...
      `);
            ...
            console.log('Table course_quizzes_downloads does not exist, skipping quiz downloads fix');
        }
