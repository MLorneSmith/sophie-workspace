    ...
    try {
      ...
      `);
        ...
        `);
            ...
            const existingRels = await client.query('SELECT COUNT(*) FROM payload.posts_rels WHERE field = $1', ['image_id']);
            console.log(`Found ${existingRels.rows[0].count} existing post-image relationships`);
            // Get all posts
            ...
            await client.query('UPDATE payload.posts SET image_id = NULL, image_id_id = NULL');
            // Mapping of post slugs to image filenames
            const slugToImageMap = {
                'presentation-tips': 'Presentation Tips Optimized.png',
                'art-craft-business-presentation-creation': 'Art Craft of Presentation Creation.png',
            ...
            for (const post of posts.rows) {
                ...
                const mediaResult = await client.query('SELECT id FROM payload.media WHERE filename = $1', [imageFilename]);
                if (mediaResult.rows.length === 0) {
                    console.log(`No media found with filename: ${imageFilename} for post ${slug}`);
                    failureCount++;
                    continue;
                }
                const mediaId = mediaResult.rows[0].id;
                // Update the post record with image_id and image_id_id
                await client.query('UPDATE payload.posts SET image_id = $1, image_id_id = $1 WHERE id = $2', [mediaId, postId]);
        ...
        `, [uuidv4(), postId, 'image_id', mediaId, mediaId]);
                console.log(`Created relationship for post "${slug}" with image "${imageFilename}"`);
                successCount++;
            }
            console.log(`Relationship fixing completed: ${successCount} fixed, ${failureCount} failed`);
            // Verify relationships were created
            const finalCount = await client.query('SELECT COUNT(*) FROM payload.posts_rels WHERE field = $1', ['image_id']);
            console.log(`Final count of post-image relationships: ${finalCount.rows[0].count}`);
