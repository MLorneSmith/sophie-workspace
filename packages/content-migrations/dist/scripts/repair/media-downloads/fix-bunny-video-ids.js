...
async function fixBunnyVideoIds() {
    ...
    process.stdout.write('Starting to fix bunny_video_id values for course lessons...\n');
    ...
    try {
        ...
        lessonMetadata = yaml.load(yamlContent);
    ...
    try {
        ...
        for (const [slug, videoId] of lessonMap.entries()) {
            const result = await pool.query('UPDATE payload.course_lessons SET bunny_video_id = $1 WHERE slug = $2 RETURNING id, title', [videoId, slug]);
            if (result.rowCount > 0) {
                updateCount++;
                console.log(`✅ Updated bunny_video_id for lesson "${slug}" (${result.rows[0].title})`);
            }
            else {
        ...
        // Verify the updates
        const verifyResult = await pool.query('SELECT COUNT(*) FROM payload.course_lessons WHERE bunny_video_id IS NOT NULL');
        console.log(`There are now ${verifyResult.rows[0].count} lessons with bunny_video_id values in the database`);
        return true;
    }
    catch (error) {
