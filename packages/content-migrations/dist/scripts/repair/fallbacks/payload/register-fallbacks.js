...
export async function registerFallbacks() {
    try {
        const extensionsDir = path.join(process.cwd(), 'apps/payload/src/extensions');
        // Create extensions directory if it doesn't exist
        if (!fs.existsSync(extensionsDir)) {
            fs.mkdirSync(extensionsDir, { recursive: true });
        }
        // Create the fallback system extension file
        const fallbackExtensionPath = path.join(extensionsDir, 'fallback-system.ts');
...
`;
        ...
        const configPath = path.join(process.cwd(), 'apps/payload/src/payload.config.ts');
        if (fs.existsSync(configPath)) {
            ...
            if (!configContent.includes('registerFallbackSystem')) {
                ...
                configContent = configContent.replace("import { buildConfig } from 'payload/config';", "import { buildConfig } from 'payload/config';\nimport { registerFallbackSystem } from './extensions/fallback-system';");
                ...
                if (configContent.includes('export default buildConfig(')) {
                    ...
                    configContent = configContent.replace('export default buildConfig(', '// Apply fallback mechanisms\nconst updatedConfig = registerFallbackSystem(baseConfig);\n\nexport default buildConfig(');
                    // Update the passed variable to use the updated config
                    configContent = configContent.replace('export default buildConfig(baseConfig', 'export default buildConfig(updatedConfig');
                }
                else if (configContent.includes('export default buildConfig({')) {
                    // For inline config objects, wrap it first
                    configContent = configContent.replace('export default buildConfig({', 'const baseConfig = {\n');
                ...
                console.log(`Updated Payload config at ${configPath} to use fallback system`);
            }
            else {
