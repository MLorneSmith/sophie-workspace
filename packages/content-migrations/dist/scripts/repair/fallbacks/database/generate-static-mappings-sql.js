...
async function generateStaticMappings() {
    ...
    `;
        ...
        const lessonQuizMappings = lessonQuizResult.rows;
        ...
        fs.writeFileSync(lessonQuizPath, JSON.stringify(lessonQuizMappings, null, 2));
        console.log(chalk.green(`✓ Saved lesson-quiz mappings to ${lessonQuizPath}`));
    ...
    `;
        ...
        const quizQuestionMappings = quizQuestionResult.rows;
        console.log(`Found ${quizQuestionMappings.length} quiz-question relationships`);
        // Write quiz-question mappings to file
        const quizQuestionPath = path.join(mappingsDir, 'quiz-question-mappings.json');
        fs.writeFileSync(quizQuestionPath, JSON.stringify(quizQuestionMappings, null, 2));
        console.log(chalk.green(`✓ Saved quiz-question mappings to ${quizQuestionPath}`));
    ...
    `;
        ...
        const surveyQuestionMappings = surveyQuestionResult.rows;
        console.log(`Found ${surveyQuestionMappings.length} survey-question relationships`);
        // Write survey-question mappings to file
        const surveyQuestionPath = path.join(mappingsDir, 'survey-question-mappings.json');
        fs.writeFileSync(surveyQuestionPath, JSON.stringify(surveyQuestionMappings, null, 2));
        console.log(chalk.green(`✓ Saved survey-question mappings to ${surveyQuestionPath}`));
    ...
    `;
        ...
        const lessonDownloadMappings = lessonDownloadResult.rows;
        console.log(`Found ${lessonDownloadMappings.length} lesson-download relationships`);
        // Write lesson-download mappings to file
        const lessonDownloadPath = path.join(mappingsDir, 'lesson-download-mappings.json');
        fs.writeFileSync(lessonDownloadPath, JSON.stringify(lessonDownloadMappings, null, 2));
        console.log(chalk.green(`✓ Saved lesson-download mappings to ${lessonDownloadPath}`));
    ...
    `;
        ...
        const postMediaMappings = postMediaResult.rows;
        ...
        const allMappings = {
            ...
            metadata: {
                ...
                totalRelationships: lessonQuizMappings.length +
        ...
        const consolidatedPath = path.join(mappingsDir, 'all-relationship-mappings.json');
        fs.writeFileSync(consolidatedPath, JSON.stringify(allMappings, null, 2));
        console.log(chalk.green(`✓ Saved consolidated mappings to ${consolidatedPath}`));
        // 7. Create SQL dump for emergency restoration
        console.log(chalk.cyan('Creating SQL dump for emergency relationship restoration...'));
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    generateStaticMappings()
        .then((result) => {
        if (result.success) {
            console.log(chalk.green('Static relationship mappings generated successfully'));
            process.exit(0);
        }
        else {
            console.error(chalk.red('Static relationship mappings generation failed:'), result.error);
            process.exit(1);
        }
    })
        .catch((error) => {
