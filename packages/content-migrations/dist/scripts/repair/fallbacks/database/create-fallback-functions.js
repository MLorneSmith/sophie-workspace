...
export async function createFallbackFunctions() {
    ...
    try {
        logger.info({ script: 'create-fallback-functions' }, 'Creating fallback functions');
    ...
    `);
      ...
      $$ LANGUAGE plpgsql;
    `);
        logger.info({ script: 'create-fallback-functions' }, 'Fallback functions created successfully');
        return { success: true };
    }
    catch (error) {
        logger.error({ script: 'create-fallback-functions', error }, 'Failed to create fallback functions');
