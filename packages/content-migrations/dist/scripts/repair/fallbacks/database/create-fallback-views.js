...
export async function createFallbackViews() {
    ...
    `);
        ...
        for (const row of collections) {
            ...
            if (!collection.match(/^[a-zA-Z0-9_]+$/)) {
                logger.warn({ script: 'create-fallback-views', collection }, 'Skipping collection with invalid name pattern');
                continue;
            }
            try {
    ...
    `);
        logger.info({ script: 'create-fallback-views' }, 'Fallback views created successfully');
        return { success: true };
    }
    catch (error) {
        logger.error({ script: 'create-fallback-views', error }, 'Failed to create fallback views');
