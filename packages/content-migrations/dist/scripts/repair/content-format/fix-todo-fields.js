...
const yamlFilePath = path.resolve(process.cwd(), 'src/data/raw/lesson-metadata.yaml');
async function fixTodoFields() {
    try {
        ...
        console.log(`Database URI: ${process.env.DATABASE_URI || 'Using default connection string'}`);
        // Read YAML file
        ...
        const lessonWithTodoFields = metadata.lessons.filter((lesson) => lesson.todoFields);
        console.log(`Found ${lessonWithTodoFields.length} lessons with todoFields in YAML`);
        ...
        for (const lesson of metadata.lessons) {
            ...
            if (lesson.todoFields) {
                ...
                if (checkResult.rows.length > 0) {
                    ...
                    console.log(`  todo_complete_quiz: ${currentRecord.todo_complete_quiz}`);
                    console.log(`  todo_watch_content: ${currentRecord.todo_watch_content ? 'present' : 'null'}`);
                    console.log(`  todo_read_content: ${currentRecord.todo_read_content ? 'present' : 'null'}`);
                    console.log(`  todo_course_project: ${currentRecord.todo_course_project ? 'present' : 'null'}`);
                    ...
                    console.log(`  todo_watch_content: ${todoWatchContent ? 'present' : 'null'}`);
                    console.log(`  todo_read_content: ${todoReadContent ? 'present' : 'null'}`);
                    console.log(`  todo_course_project: ${todoCourseProject ? 'present' : 'null'}`);
