 ...
 */
async function executeWithRetry(client, sql, retryCount = 0) {
    try {
...
export async function fixPayloadRelationshipsStrict() {
    ...
    try {
        ...
        console.log(`Running ${txnCommands.length} transaction commands and ${verificationQueries.length} verification queries`);
        // Execute each SQL command and log the result
        let commandNumber = 1;
        for (const command of txnCommands) {
            try {
                console.log(`Executing command ${commandNumber}/${txnCommands.length}...`);
        ...
        try {
            const results = await Promise.all(verificationQueries.map((query) => query.trim() ? executeWithRetry(client, query) : null));
