...
const CRITICAL_COLUMNS = REQUIRED_COLUMNS.filter((col) => ['id', 'path', 'parent_id'].includes(col.name));
const OPTIONAL_COLUMNS = REQUIRED_COLUMNS.filter((col) => !CRITICAL_COLUMNS.map((c) => c.name).includes(col.name));
/**
...
export async function verifyUuidTables(tables) {
    ...
    if (tables.length === 0) {
        console.warn(chalk.yellow('⚠️ No tables to verify! This may indicate a database schema issue.'));
        console.log(chalk.blue('ℹ️ This is not necessarily an error - if this is a fresh installation,'));
        console.log(chalk.blue('   the tables may be created during content population.'));
    ...
    for (const table of tables) {
        ...
        const missingCriticalColumns = CRITICAL_COLUMNS.filter((col) => !existingColumnNames.includes(col.name)).map((col) => col.name);
        // Check optional columns
        const missingOptionalColumns = OPTIONAL_COLUMNS.filter((col) => !existingColumnNames.includes(col.name)).map((col) => col.name);
        if (missingCriticalColumns.length > 0) {
            console.error(chalk.red(`❌ Table ${table.name} is missing critical columns: ${missingCriticalColumns.join(', ')}`));
            tablesWithMissingCriticalColumns++;
            allCriticalValid = false;
        }
        if (missingOptionalColumns.length > 0) {
            console.warn(chalk.yellow(`⚠️ Table ${table.name} is missing optional columns: ${missingOptionalColumns.join(', ')}`));
            tablesWithMissingOptionalColumns++;
            allOptionalValid = false;
        }
        if (missingCriticalColumns.length === 0 &&
            missingOptionalColumns.length === 0) {
            console.log(chalk.green(`✅ Table ${table.name} has all required columns`));
        }
    }
    // Print summary
    console.log(chalk.blue('\n📊 VERIFICATION SUMMARY:'));
    console.log(`Total tables checked: ${tablesChecked}`);
    console.log(`Tables with all columns: ${tablesChecked - tablesWithMissingCriticalColumns - tablesWithMissingOptionalColumns}`);
    console.log(`Tables missing critical columns: ${tablesWithMissingCriticalColumns}`);
    console.log(`Tables missing optional columns only: ${tablesWithMissingOptionalColumns}`);
    ...
    if (allCriticalValid) {
        if (allOptionalValid) {
            ...
            console.log(chalk.yellow('   This is acceptable for migration to continue, but you may want to fix later.'));
        }
        return true;
    }
    else {
...
export function getVerificationReport(tables) {
    ...
    for (const table of tables) {
        ...
        const tableValid = tableCriticalValid && missingOptionalColumns.length === 0;
