 ...
 */
export async function repairUuidTables(options = {}) {
    // Default options
    ...
    if (!databaseUrl) {
        const error = 'DATABASE_URI or DATABASE_URL environment variable is required';
        console.error(error);
    ...
    try {
        ...
        try {
            ...
            if (config.createMonitoring) {
                ...
                console.log(`Monitoring system ${result.monitoringEnabled ? 'enabled' : 'failed to enable'}`);
            }
