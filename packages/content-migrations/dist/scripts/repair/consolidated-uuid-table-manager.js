      ...
      `, [table.table_name]);
        ...
        for (const table of tablesWithInfo) {
            ...
            if (table.has_path_column &&
                ...
                logger.info(`Table ${table.table_name} already has all required columns, skipping`);
                continue;
            }
        ...
        logger.info(`Successfully processed ${tablesWithInfo.length} UUID tables in a single transaction`);
        // Close connection
        await client.end();
        return true;
    }
