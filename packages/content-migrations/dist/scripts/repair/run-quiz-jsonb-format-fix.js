...
async function main() {
    try {
        ...
        console.log(chalk.yellow('This script fixes quiz question relationships by ensuring proper JSONB formatting'));
        // First verify the current state
        console.log(chalk.cyan('\n1. Verifying current state of quiz questions...'));
        const initialVerification = await verifyQuestionsJSONBFormat();
        if (initialVerification) {
            console.log(chalk.green('All quizzes already have properly formatted questions arrays.'));
        ...
        if (!fixResult) {
            console.error(chalk.red('Failed to format quiz questions JSONB. See errors above.'));
            return false;
        }
        ...
        if (afterVerification) {
            ...
            console.log(chalk.yellow('You may need to review the server logs and run more specific repairs.'));
            return false;
        }
    }
    catch (error) {
