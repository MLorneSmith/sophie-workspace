 ...
 */
export async function repairQuizSystem(options = {}) {
    const { skipVerification = false, continueOnError = false, dryRun = false, verbose = false, } = options;
    ...
    try {
        ...
        if (!dryRun) {
            ...
            await db.transaction(async (tx) => {
                ...
                logger.info('Step 2: Fixing primary relationships (quiz → question)...');
                primaryResult = await fixPrimaryRelationships(tx, state);
                // 3. Fix bidirectional relationships (question → quiz)
                logger.info('Step 3: Fixing bidirectional relationships (question → quiz)...');
            ...
            if (!skipVerification) {
                ...
                if (!verificationResult.success && !continueOnError) {
                    ...
                    logger.error('Verification failed post-commit: ' + verificationResult.message);
            ...
            jsonbResult = { quizzesUpdated: 0, updatedQuizzes: [] };
            if (!skipVerification) {
                logger.info('Step 5: Verifying quiz system integrity (dry run mode)...');
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    if (options.verifyOnly) {
        (async () => {
            ...
            logger.info(`Verification ${result.success ? 'PASSED' : 'FAILED'}: ${result.message}`);
            if (!result.success) {
                process.exit(1);
            }
        })();
