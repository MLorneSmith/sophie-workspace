 ...
 */
async function getActualQuestionCount(surveyId) {
    try {
...
async function fixSurveyProgressRecords() {
    try {
        ...
        for (const survey of surveys) {
            ...
            console.log(`Survey "${survey.title}" (${survey.id}) has ${actualQuestionCount} questions`);
            if (actualQuestionCount === 0) {
                console.log(`Skipping survey "${survey.title}" (${survey.id}) - no questions found`);
                continue;
            }
            ...
            if (progressError) {
                console.error(`Error getting progress records for survey ${survey.id}:`, progressError);
                continue;
            }
            console.log(`Found ${progressRecords?.length || 0} progress records for survey "${survey.title}"`);
            ...
            for (const record of progressRecords || []) {
                ...
                if (record.total_questions === actualQuestionCount) {
                    console.log(`Progress record ${record.id} already has correct question count`);
                    continue;
                }
                // Calculate new progress percentage
                const progressPercentage = ((record.current_question_index || 0) / actualQuestionCount) * 100;
                // Update the record
                const { error: updateError } = await supabase
                    ...
                    .eq('id', record.id);
                if (updateError) {
                    console.error(`Error updating progress record ${record.id}:`, updateError);
                }
                else {
                    console.log(`Updated progress record ${record.id} from ${record.total_questions} to ${actualQuestionCount} questions`);
                }
            }
        }
