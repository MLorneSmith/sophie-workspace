...
async function verifyPostsContent() {
    ...
    try {
        ...
        try {
            ...
            // Get all posts
            const postsResult = await client.query(`SELECT id, title, slug, content FROM payload.posts ORDER BY title`);
            ...
            for (const post of postsResult.rows) {
                ...
                try {
                    ...
                    console.error(`❌ Error parsing content JSON for post ${id} (${title}):`, error);
                    continue;
                }
                // Check if we have a root node
                if (!parsedContent.root) {
                    console.error(`❌ No root node found in content for post ${id} (${title})`);
                    continue;
                }
                // Check if we have children nodes
                if (!parsedContent.root.children ||
                    !Array.isArray(parsedContent.root.children)) {
                    console.error(`❌ No children nodes found in content for post ${id} (${title})`);
                    continue;
                }
                ...
                if (contentLength < 1000 && childrenCount < 3) {
                    console.warn(`⚠️ Content looks potentially truncated for post ${id} (${title})`);
                }
                else {
