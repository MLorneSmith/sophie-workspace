 ...
 */
export async function verifyUnidirectionalQuizQuestions() {
    ...
    `);
        ...
        ORDER BY _order ASC
      `, [quiz.id]);
            const questionIdsFromRels = relationshipRecords.rows.map((row) => row.question_id);
            ...
            if (questionIdsFromRels.length === 0) {
                ...
                console.log(`✅ Quiz "${quiz.title}" has ${questionIdsFromRels.length} questions in the relationship table`);
                // Verify that all question IDs are valid UUIDs
                const invalidQuestionIds = questionIdsFromRels.filter((id) => !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(id)));
                if (invalidQuestionIds.length > 0) {
                    console.warn(`❌ Quiz "${quiz.title}" has ${invalidQuestionIds.length} invalid question IDs`);
        ...
        // Summary
        if (overallSuccess) {
            console.log('✅ All quiz-question relationships are consistent with the unidirectional model');
            return true;
        }
        else {
            console.warn(`❌ Found ${problemsFound} quizzes with inconsistent relationships`);
            console.warn('Run fix:unidirectional-quiz-questions to repair these issues');
            return false;
        }
    }
    catch (error) {
        console.error('Error verifying unidirectional quiz-question relationships:', error);
        return false;
    }
    finally {
        await client.end();
