...
async function verifyDownloadsDiagnosticView() {
    ...
    `);
        const columns = viewColumns.rows.map((row) => row.column_name);
        ...
        for (const column of requiredColumns) {
            if (!columns.includes(column)) {
                console.error(`❌ Missing required column '${column}' in downloads_diagnostic view`);
                missingColumns = true;
            }
            else {
                console.log(`✅ Required column '${column}' exists in downloads_diagnostic view`);
            }
        }
        if (missingColumns) {
