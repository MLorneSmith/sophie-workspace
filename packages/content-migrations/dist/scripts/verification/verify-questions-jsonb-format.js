...
export const verifyQuestionsJSONBFormat = async () => {
    try {
        ...
        for (const quiz of quizzes.rows) {
            try {
                ...
                try {
                    ...
                    logError(`Invalid JSON in questions for quiz "${quiz.title}" (${quiz.id}): ${e.message}`);
                    improperlyFormatted++;
                    problemQuizzes.push({
                        id: quiz.id,
                        title: quiz.title,
                ...
                if (!Array.isArray(questions)) {
                    logWarning(`Questions is not an array for quiz "${quiz.title}" (${quiz.id}), type: ${typeof questions}`);
                    improperlyFormatted++;
                    problemQuizzes.push({
                        id: quiz.id,
                        title: quiz.title,
                ...
                for (let i = 0; i < questions.length; i++) {
                    ...
                    if (typeof q === 'string') {
                        ...
                        formatIssues.push({
                            index: i,
                            actual: JSON.stringify(q),
                            expected: '{ "id": "...", "relationTo": "quiz_questions", "value": { "id": "..." } }',
                        });
                    }
                }
                ...
                if (formatIssues.length === 0 && hasRelationshipMatch) {
                    ...
                    if (correctlyFormatted === 1) {
                        logAction(`Correctly formatted quiz example - "${quiz.title}": ${JSON.stringify(questions[0])}`);
                    }
                    continue;
                }
                // Handle identified issues
                if (formatIssues.length > 0) {
                    logWarning(`Quiz "${quiz.title}" (${quiz.id}) has ${formatIssues.length} improperly formatted questions`);
                    ...
                    logWarning(`Quiz "${quiz.title}" (${quiz.id}) has ${arrayCount} questions in array but ${relationshipCount} in relationship table`);
                ...
                logError(`Error processing quiz "${quiz.title}" (${quiz.id}): ${error.message}`);
                improperlyFormatted++;
                problemQuizzes.push({
                    id: quiz.id,
                    title: quiz.title,
    ...
    `);
        ...
        `;
                const performanceRels = await db.execute(sql.raw([performanceRelsQuery]));
        ...
        `;
                ...
                if (questionRels.rows.length > 0) {
                    logAction(`Questions from relationship table: ${JSON.stringify(questionRels.rows.map((r) => r.quiz_questions_id).slice(0, 3))}...`);
                }
            }
            catch (e) {
            ...
            logWarning('Performance Quiz not found! This is concerning as it should exist.');
        }
    ...
    `);
        if (whoQuiz.rows.length > 0) {
            logAction(`Found "The Who" Quiz: ${whoQuiz.rows[0].title} (${whoQuiz.rows[0].id})`);
      ...
      `;
            ...
            logAction(`"The Who" Quiz has ${whoQuizRels.rows[0].count} questions in relationship table`);
        }
        else {
            logWarning('The Who Quiz not found despite being referenced in server logs!');
        }
        // Final report
        if (improperlyFormatted === 0 &&
            missingQuestions === 0 &&
            relationshipMismatch === 0) {
            logSuccess(`All ${correctlyFormatted} quizzes have properly formatted questions arrays`);
            return true;
        }
        else {
            logWarning(`
...
const isMainModule = import.meta.url.endsWith('verify-questions-jsonb-format.ts') ||
