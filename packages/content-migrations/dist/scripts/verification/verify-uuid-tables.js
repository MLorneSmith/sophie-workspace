...
try {
    ...
    if (!process.env.DATABASE_URI && !process.env.DATABASE_URL) {
        console.log('DATABASE_URI not found in environment, using default connection string');
...
async function verifyUuidTables() {
    ...
    `;
      ...
      `;
            ...
            const missingCriticalColumns = criticalColumns.filter((col) => !existingColumns.includes(col));
            if (missingColumns.length > 0) {
                if (missingCriticalColumns.length > 0) {
                    console.log(chalk.red(`Table ${tableName} is missing critical columns: ${missingCriticalColumns.join(', ')}`));
                    tablesWithCriticalIssues++;
                    overallSuccess = false;
                }
                else {
                    console.log(chalk.yellow(`Table ${tableName} is missing optional columns: ${missingColumns.join(', ')}`));
                    tablesWithIssues++;
...
verifyUuidTables()
    .then((success) => {
    if (success) {
        console.log(chalk.green('UUID tables verification completed successfully.'));
        process.exit(0);
    }
    else {
        console.log(chalk.yellow('UUID tables verification found issues with critical columns that need to be fixed.'));
        process.exit(1);
    }
})
    .catch((error) => {
