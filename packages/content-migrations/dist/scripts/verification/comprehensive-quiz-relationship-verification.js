 ...
 */
async function verifyQuizRelationships() {
    console.log(chalk.cyan('Starting comprehensive quiz relationship verification...'));
    let client;
    try {
    ...
    `);
        ...
        for (const quiz of quizzes) {
            ...
            if (typeof quiz.id !== 'string') {
                console.warn(chalk.yellow(`Skipping quiz with non-string ID: ${quiz.id}`));
                continue; // Skip this iteration if quiz.id is not a string
            }
            const currentQuizId = quiz.id; // Now guaranteed to be a string
            // Extract question IDs from questions array
            let arrayQuestionIds = [];
            try {
                const questionsArray = typeof quiz.questions === 'string'
                ...
                arrayQuestionIds = questionsArray
                    ...
                    .filter((id) => id !== null); // Use type predicate for filtering
            }
            catch (e) {
                console.error(chalk.red(`Error parsing questions array for quiz "${quiz.title}" (${quiz.id}):`, e.message));
                arrayQuestionIds = [];
            }
      ...
      `, [currentQuizId]);
            ...
            const relQuestionIds = quizRelResult.rows
            ...
            const nullPathCount = quizRelResult.rows.filter((r) => r.path === null).length;
            // Check for missing items in either direction
            const missingFromArray = uniqueRelIds.filter((id) => !arrayQuestionIds.includes(id));
            const missingFromRels = arrayQuestionIds.filter((id) => !uniqueRelIds.includes(id));
            // Determine if this quiz is fully consistent
            const isConsistent = nullPathCount === 0 &&
                duplicateCount === 0 &&
        ...
        const totalPathNullCount = results.reduce((sum, r) => sum + r.nullPathCount, 0);
        const totalDuplicateCount = results.reduce((sum, r) => sum + r.duplicateCount, 0);
        const totalMismatchedArrayCount = results.filter((r) => r.arrayCount !== r.relCount).length;
        const totalMissingFromArrayCount = results.reduce((sum, r) => sum + r.missingFromArray.length, 0);
        const totalMissingFromRelsCount = results.reduce((sum, r) => sum + r.missingFromRels.length, 0);
 ...
 */
function reportVerificationResults(summary) {
    // Restore this function
    const { totalQuizzes, passedQuizzes, failedQuizzes, totalPathNullCount, totalDuplicateCount, totalMismatchedArrayCount, totalMissingFromArrayCount, totalMissingFromRelsCount, problemQuizzes, isFullyConsistent, } = summary;
    console.log(chalk.cyan('\n=== Quiz Relationship Verification Results ===\n'));
    if (isFullyConsistent) {
        console.log(chalk.green(`✅ All ${totalQuizzes} quizzes have fully consistent relationships!`));
    }
    else {
        console.log(chalk.yellow(`⚠️ Found issues with ${failedQuizzes} out of ${totalQuizzes} quizzes:`));
        console.log(chalk.yellow(`- Records with NULL path: ${totalPathNullCount}`));
        console.log(chalk.yellow(`- Duplicate relationship records: ${totalDuplicateCount}`));
        console.log(chalk.yellow(`- Quizzes with mismatched array counts: ${totalMismatchedArrayCount}`));
        console.log(chalk.yellow(`- Questions missing from arrays: ${totalMissingFromArrayCount}`));
        console.log(chalk.yellow(`- Questions missing from relationships: ${totalMissingFromRelsCount}`));
        ...
        problemQuizzes.slice(0, 5).forEach((quiz, index) => {
            ...
            if (quiz.nullPathCount > 0) {
                console.log(chalk.yellow(`   - Has ${quiz.nullPathCount} records with NULL path`));
            }
            if (quiz.duplicateCount > 0) {
                console.log(chalk.yellow(`   - Has ${quiz.duplicateCount} duplicate relationship records`));
            }
            if (quiz.arrayCount !== quiz.relCount) {
                console.log(chalk.yellow(`   - Array count (${quiz.arrayCount}) doesn't match relationship count (${quiz.relCount})`));
            }
            if (quiz.missingFromArray.length > 0) {
                console.log(chalk.yellow(`   - ${quiz.missingFromArray.length} questions in relationships missing from array`));
                console.log(chalk.gray(`     First few IDs: ${quiz.missingFromArray.slice(0, 3).join(', ')}${quiz.missingFromArray.length > 3 ? '...' : ''}`));
            }
            if (quiz.missingFromRels.length > 0) {
                console.log(chalk.yellow(`   - ${quiz.missingFromRels.length} questions in array missing from relationships`));
                console.log(chalk.gray(`     First few IDs: ${quiz.missingFromRels.slice(0, 3).join(', ')}${quiz.missingFromRels.length > 3 ? '...' : ''}`));
            }
        });
