...
export async function verifyDownloadsDataTypes() {
    ...
    console.log('Verifying data type consistency for downloads-related columns...');
    ...
    if (inconsistentColumns.length === 0) {
        console.log('✅ All downloads-related columns have consistent types (UUID)');
    }
    else {
        console.error(`❌ Found ${inconsistentColumns.length} column type inconsistencies:`);
        inconsistentColumns.forEach((col) => {
            console.error(`   Table: ${col.table_name}, Column: ${col.column_name}, ` +
                `Type: ${col.current_type} (expected: ${col.expected_type})`);
        });
        console.log('Run the 20250412_100000_fix_downloads_uuid_type_consistency migration to fix these issues');
    }
    ...
    const { data: tempTables, error: tempTablesError } = await supabase
        ...
        .filter('tablename', 'regexp', '^[0-9a-f]{8}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{12}$');
    if (tempTablesError) {
        console.error('Error checking temporary UUID tables:', tempTablesError);
        return;
    }
    if (tempTables && tempTables.length > 0) {
        console.log(`Found ${tempTables.length} temporary UUID tables. Checking column types...`);
        for (const table of tempTables) {
            ...
            if (columns.length === 0) {
                console.error(`❌ Table ${table.tablename} is missing downloads_id column`);
            }
            else if (columns[0].data_type !== 'uuid') {
                console.error(`❌ Table ${table.tablename} has downloads_id column with type ${columns[0].data_type} (expected: uuid)`);
            }
            else {
                console.log(`✅ Table ${table.tablename} has correctly typed downloads_id column`);
            }
        }
    }
    else {
