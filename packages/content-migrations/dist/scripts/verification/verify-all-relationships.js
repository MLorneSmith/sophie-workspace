...
async function verifyAllRelationships() {
    ...
    try {
      ...
      `);
            const surveyQuestionsCount = parseInt(surveyVerificationResult.rows[0]?.questions_count || '0');
            const surveyBidirectionalCount = parseInt(surveyVerificationResult.rows[0]?.bidirectional_count || '0');
            console.log(`Survey questions with surveys_id: ${surveyQuestionsCount}`);
            console.log(`Bidirectional relationships in surveys_rels: ${surveyBidirectionalCount}`);
            if (surveyQuestionsCount === surveyBidirectionalCount) {
                ...
                console.log(`  Missing: ${surveyQuestionsCount - surveyBidirectionalCount}`);
            }
      ...
      `);
            const quizQuestionsCount = parseInt(quizVerificationResult.rows[0]?.questions_count || '0');
            const quizBidirectionalCount = parseInt(quizVerificationResult.rows[0]?.bidirectional_count || '0');
            console.log(`Quiz questions with quiz_id: ${quizQuestionsCount}`);
            console.log(`Bidirectional relationships in course_quizzes_rels: ${quizBidirectionalCount}`);
            if (quizQuestionsCount === quizBidirectionalCount) {
                ...
                console.log(`  Missing: ${quizQuestionsCount - quizBidirectionalCount}`);
            }
      ...
      `);
            ...
            if (quizIdIdCount === 0) {
                ...
                console.log(`❌ Found ${quizIdIdCount} records with incorrect field name 'quiz_id_id'`);
            }
      ...
      `);
            ...
            if (surveysIdCount === 0) {
                ...
                console.log(`❌ Found ${surveysIdCount} records with incorrect field name 'surveys_id'`);
            }
