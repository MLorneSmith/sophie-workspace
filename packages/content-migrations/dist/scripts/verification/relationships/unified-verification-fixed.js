...
async function loadEnvironmentVariables() {
    try {
        ...
        console.log('Attempting to load environment variables from possible paths:');
        ...
        for (const envPath of envPaths) {
            try {
                ...
                if (result.parsed) {
                    ...
                    console.log(`No environment variables found in ${envPath} or file doesn't exist`);
                }
            }
            catch (e) {
        ...
        if (!envLoaded) {
            console.warn(chalk.yellow('Could not load any .env file from any location, will use default connection string'));
        }
        return envLoaded;
    }
    catch (error) {
        console.warn(chalk.yellow('Error in environment variable loading function:'), error);
        return false;
    }
}
...
(async () => {
    ...
    try {
        ...
        console.log(`Environment loading ${envLoadSuccess ? 'succeeded' : 'failed'}`);
        ...
        if (!DATABASE_URI) {
            ...
            console.log(chalk.yellow(`Using default connection string: ${DATABASE_URI}`));
        }
        else {
    ...
    console.log('Using default connection string due to critical initialization error');
    DATABASE_URI =
        'postgresql://postgres:postgres@localhost:54322/postgres?schema=payload';
});
/**
...
async function verifyQuizQuestionRelationships(pool, verbose = false) {
    try {
      ...
      `);
            ...
            const questionsInRelationships = parseInt(result.rows[0].questions_in_relationships || '0');
            if (verbose) {
...
async function verifyLessonQuizRelationships(pool, verbose = false) {
    try {
      ...
      `);
            const totalRelationships = parseInt(result.rows[0].total_relationships || '0');
            if (verbose) {
...
export async function verifyAllRelationships(verbose = false) {
    ...
    try {
        ...
        if (!quizQuestionRelationshipsValid) {
            console.log(chalk.yellow('⚠️ Quiz-question relationships verification found issues, but continuing...'));
            success = false;
        }
        ...
        if (!lessonQuizRelationshipsValid) {
            console.log(chalk.yellow('⚠️ Lesson-quiz relationships verification found issues, but continuing...'));
            success = false;
        }
...
if (import.meta.url && import.meta.url.endsWith(process.argv[1])) {
    console.log('Script executed directly, waiting for environment initialization...');
    ...
    const checkAndRun = async (attempts = 0, maxAttempts = 10) => {
        if (attempts >= maxAttempts) {
            console.error(chalk.red('Max attempts reached waiting for DATABASE_URI, using default'));
            DATABASE_URI =
                'postgresql://postgres:postgres@localhost:54322/postgres?schema=payload';
        }
        if (DATABASE_URI) {
            ...
            try {
                ...
                if (success) {
                    ...
                    console.error(chalk.yellow('Relationship verification found issues, but allowing migration to continue'));
                ...
                console.log(chalk.yellow('Exiting with success to allow migration to continue'));
                process.exit(0); // Exit with success to allow migration to continue
            }
        }
        else {
            console.log(`Waiting for DATABASE_URI to be initialized (attempt ${attempts + 1}/${maxAttempts})...`);
