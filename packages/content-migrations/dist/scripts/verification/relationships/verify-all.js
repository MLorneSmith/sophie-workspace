 ...
 */
async function verifyQuizQuestionRelationships(result) {
    ...
    `;
        ...
        if (verificationResult.rows.length > 0) {
            ...
            const quizTitleMap = quizTitlesResult.rows.reduce((map, row) => {
            ...
            for (const row of verificationResult.rows) {
                ...
                console.log(formatLogMessage(`Issue found: ${row.issue_type} for Quiz "${issuesByQuiz[quizId].title}" (${quizId}), Question ${row.question_id}`, 'warn'));
            ...
            // Log summary count
            console.log(formatLogMessage(`Quiz-Question relationships: ${totalIssues} total issues found across all quizzes`, 'warn'));
        }
        else {
 ...
 * @returns Updated verification result
 */
async function verifyLessonQuizRelationships(result) {
    ...
    `;
        ...
        if (verificationResult.rows.length > 0) {
            ...
            for (const row of verificationResult.rows) {
                ...
                // Log the detailed issue directly
                console.log(formatLogMessage(`Issue found: ${issueType} for Lesson ${lessonId}, Quiz ${quizId}`, 'warn'));
            ...
            console.log(formatLogMessage(`Lesson-Quiz relationships: ${totalLessonQuizIssues} total issues found`, 'warn'));
        }
        else {
 ...
 * @returns Updated verification result
 */
async function verifySurveyQuestionRelationships(result) {
 ...
 * @returns Updated verification result
 */
async function verifyDownloadRelationships(result) {
    ...
    `;
        ...
        if (verificationResult.rows.length > 0) {
            ...
            console.log(formatLogMessage(`Download relationships: ${verificationResult.rows.reduce((sum, row) => sum + parseInt(row.total_count), 0)} issues found`, 'warn'));
        }
        else {
