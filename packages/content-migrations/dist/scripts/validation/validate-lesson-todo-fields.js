...
function loadEnvFile() {
    ...
    if (fs.existsSync(envPath)) {
        ...
        const envVars = Object.fromEntries(envContent
            .split('\n')
...
async function validateLessonTodoFields() {
    ...
    try {
        ...
        try {
            ...
            const result = await client.query(`SELECT id, title, slug, todo, todo_complete_quiz, todo_watch_content, todo_read_content, todo_course_project 
         FROM payload.course_lessons`);
            ...
            for (const dbLesson of dbLessons) {
                ...
                if (!yamlLesson) {
                    console.log(`Warning: Lesson "${dbLesson.title}" (${dbLesson.slug}) not found in YAML file`);
                    continue;
                }
                // Check if database has todo fields
                const hasTodoFields = dbLesson.todo !== null ||
                ...
                const yamlHasTodoFields = yamlLesson.todoFields &&
            ...
            console.log(`Lessons with todo fields in database: ${validationResults.withTodoFields}`);
            console.log(`Lessons with todo fields matching YAML: ${validationResults.matchingYaml}`);
            ...
            if (success) {
                ...
                console.log('   Run the migration with the latest changes to populate the todo fields');
                return false;
            }
        }
        finally {
