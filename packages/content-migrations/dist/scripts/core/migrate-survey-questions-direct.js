        ...
        try {
            ...
            for (const file of yamlFiles) {
                try {
                    ...
                    if (!surveyFromDb) {
                        console.error(`Survey not found in database for ${slug}. Skipping questions.`);
                        continue;
                    }
                    const surveyId = surveyFromDb.id;
                    console.log(`Using survey ID from database: ${surveyId} for survey: ${slug}`);
                    // Process questions
                    if (data.questions && Array.isArray(data.questions)) {
                        console.log(`Processing ${data.questions.length} questions for survey: ${slug}`);
                        for (let i = 0; i < data.questions.length; i++) {
                            const q = data.questions[i];
                            // First check if the question already exists
                            const existingQuestion = await client.query(`SELECT sq.id 
                 FROM payload.survey_questions sq
                 JOIN payload.survey_questions_rels sqr ON sq.id = sqr._parent_id
                 WHERE sqr.surveys_id = $1 AND sq.question = $2`, [surveyId, q.question]);
                            let questionId;
                            if (existingQuestion.rows.length > 0) {
                                ...
                                const questionResult = await client.query(`INSERT INTO payload.survey_questions (
                    id, 
                    question, 
                    type, 
                    questionspin, 
                  ...
                  RETURNING id`, [
                    ...
                    updated_at,
                    created_at
                  ) VALUES (gen_random_uuid(), $1, $2, NOW(), NOW())`, [questionId, surveyId]);
                                console.log(`Created relationship between question ${questionId} and survey ${surveyId}`);
                            }
                            ...
                            if (questionId && options.length > 0) {
                                ...
                                await client.query(`DELETE FROM payload.survey_questions_options WHERE _parent_id = $1`, [questionId]);
                                // Then insert the new options
                                for (let j = 0; j < options.length; j++) {
                                    const option = options[j];
                                    if (option) {
                                        await client.query(`INSERT INTO payload.survey_questions_options (id, _order, _parent_id, option, updated_at, created_at)
                       VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())`, [j, questionId, option.option]);
                                    }
                                }
