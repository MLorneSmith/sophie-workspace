/**
 * Script to migrate private posts from Markdown files to Payload CMS directly in the PostgreSQL database
...
/**
 * Migrates private posts from HTML files directly to the PostgreSQL database
 * Includes enhanced reporting to clarify migration status
 */
async function migratePrivatePostsToDatabase() {
    ...
    try {
        ...
        `);
          ...
          `);
            ...
            // Path to the private posts files
            const postsDir = path.resolve(__dirname, '../../data/raw/bpm');
            console.log(`Private posts directory: ${postsDir}`);
            // Check if the directory exists
            if (!fs.existsSync(postsDir)) {
                console.log(`Private posts directory does not exist: ${postsDir}`);
                console.log('Skipping private posts migration.');
                return;
            }
            ...
            console.log(`Found ${postFiles.length} private post files to migrate.`);
            // Skip migration if no files found
            if (postFiles.length === 0) {
                console.log('No private post files found to migrate.');
                return;
            }
            console.log(`Starting migration of ${postFiles.length} private post files...`);
            ...
            for (const file of postFiles) {
                try {
                    ...
                    const existingPostResult = await client.query(`SELECT id FROM payload.private WHERE slug = $1`, [slug]);
                    let postId;
                    if (existingPostResult.rows.length > 0) {
                        // Update existing post
                        postId = existingPostResult.rows[0].id;
                        console.log(`Updating existing private post with slug ${slug} and ID ${postId}`);
                        await client.query(`UPDATE payload.private SET
                title = $1,
                description = $2,
                content = $3,
                published_at = $4,
              ...
              WHERE id = $6`, [
                        ...
                        try {
                            await client.query(`DELETE FROM payload.private_categories WHERE _parent_id = $1`, [postId]);
                        }
                        catch (delError) {
                            console.error(`Error deleting categories for private post ${postId}, but continuing:`, delError.message);
                        }
                        try {
                            await client.query(`DELETE FROM payload.private_tags WHERE _parent_id = $1`, [postId]);
                        }
                        catch (delError) {
                            console.error(`Error deleting tags for private post ${postId}, but continuing:`, delError.message);
                        }
                    }
                    else {
                        // Create a new post
                        postId = uuidv4();
                        console.log(`Creating new private post with slug ${slug} and ID ${postId}`);
                        await client.query(`INSERT INTO payload.private (
                id, 
                title, 
                slug, 
                description, 
                ...
                path
                    ...
                    if (data.categories && Array.isArray(data.categories)) {
                        for (let i = 0; i < data.categories.length; i++) {
                            try {
                                ...
                                await client.query(`INSERT INTO payload.private_categories (
                    id, 
                    _parent_id, 
                    category, 
                    updated_at, 
                    ...
                    "order"
                                ...
                                console.error(`Error adding category ${data.categories[i]} to private post ${postId}, but continuing:`, catError.message);
                            }
                        }
                    }
                    ...
                    if (data.tags && Array.isArray(data.tags)) {
                        for (let i = 0; i < data.tags.length; i++) {
                            try {
                                const tagId = uuidv4();
                                await client.query(`INSERT INTO payload.private_tags (
                    id, 
                    _parent_id, 
                    tag, 
                    updated_at, 
                    ...
                    "order"
                                ...
                                console.error(`Error adding tag ${data.tags[i]} to private post ${postId}, but continuing:`, tagError.message);
                            }
                        }
                    }
                    // Track the UUID table
                    const uuidTableName = `private_${postId.replace(/-/g, '_')}`;
                    try {
                    ...
                    console.log(`Migrated private post: ${slug} with ID: ${postId}`);
                }
                catch (error) {
                    console.error(`Error migrating ${file}:`, error);
                }
            }
            // Verify private posts were migrated successfully
            try {
                const countResult = await client.query(`SELECT COUNT(*) FROM payload.private`);
                const totalPosts = parseInt(countResult.rows[0].count);
                console.log(`Total private posts in database after migration: ${totalPosts}`);
                if (totalPosts === 0) {
                    console.warn(`WARNING: No private posts were found in the database after migration!`);
                    // Investigate possible issues
                    console.log('Checking for database structure issues...');
                    // Check if the private table exists and has expected columns
                    const tableResult = await client.query(`
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_schema = 'payload' 
            AND table_name = 'private'
          `);
                    console.log(`Private table columns: ${tableResult.rows.map((r) => r.column_name).join(', ')}`);
                    ...
                    console.log(`NOTE: No new private posts were migrated. All ${postFiles.length} private posts already exist in the database.`);
                }
                else {
                    console.log(`Successfully migrated/updated ${migratedCount} of ${postFiles.length} private posts. ✅`);
                }
            }
            catch (verifyError) {
            ...
            console.log('=========================================');
            console.log('Private posts migration complete! ✅');
            console.log(`Total private posts in database: ${await client.query('SELECT COUNT(*) FROM payload.private').then((res) => parseInt(res.rows[0].count))}`);
            console.log('=========================================');
        }
        finally {
            client.release();
        ...
        console.error('Error migrating private posts:', error);
        throw error;
    }
    finally {
        await pool.end();
...
// Run the migration
migratePrivatePostsToDatabase().catch((error) => {
    console.error('Private posts migration failed:', error);
    process.exit(1);
});
