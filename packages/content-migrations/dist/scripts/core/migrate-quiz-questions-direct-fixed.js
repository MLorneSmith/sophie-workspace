            ...
            for (const file of mdocFiles) {
                try {
                    ...
                    if (!quizId) {
                        ...
                        if (!quizFromDb) {
                            console.error(`Quiz not found in database for ${slug}. Skipping questions.`);
                            continue;
                        }
                        quizId = quizFromDb.id;
                    }
                    console.log(`Using quiz ID from database: ${quizId} for quiz: ${slug}`);
                    // Process questions
                    if (data.questions && Array.isArray(data.questions)) {
                        console.log(`Processing ${data.questions.length} questions for quiz: ${slug}`);
                        for (let i = 0; i < data.questions.length; i++) {
                            const q = data.questions[i];
                            // First check if the question already exists
                            const existingQuestion = await client.query(`SELECT id FROM payload.quiz_questions WHERE quiz_id = $1 AND question = $2`, [quizId, q.question]);
                            let questionId;
