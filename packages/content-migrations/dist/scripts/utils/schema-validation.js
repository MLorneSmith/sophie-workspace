 ...
 */
export async function validateDynamicUuidTablesSchema(client) {
    try {
    ...
    `);
        ...
        console.log('Available columns in dynamic_uuid_tables:', columnNames.join(', '));
        ...
        const hasDeprecatedHasDownloadsId = columnNames.includes('has_downloads_id');
        if (hasDeprecatedLastChecked ||
            hasDeprecatedHasParentId ||
            hasDeprecatedHasDownloadsId) {
            console.log('WARNING: Schema contains deprecated columns that might be removed in future versions.');
        }
        return {
            isValid: true,
            message: 'Schema validation successful',
            columns: columnNames,
            hasDeprecatedColumns: hasDeprecatedLastChecked ||
                hasDeprecatedHasParentId ||
                hasDeprecatedHasDownloadsId,
        };
    }
 ...
 */
export async function safeInsertIntoUuidTablesTracking(client, tableName) {
    try {
        // First validate the schema
        const schemaInfo = await validateDynamicUuidTablesSchema(client);
        if (!schemaInfo.isValid) {
            console.error(`Cannot insert into dynamic_uuid_tables: ${schemaInfo.message}`);
            return false;
        }
