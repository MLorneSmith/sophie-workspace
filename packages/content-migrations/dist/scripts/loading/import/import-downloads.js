...
async function main() {
    ...
    try {
        ...
        const outputDir = path.resolve(process.cwd(), 'packages/content-migrations/sql-output');
        ...
        console.log(`For example: psql -U postgres -d your_database -f ${outputFile}\n`);
        return 0;
    }
    catch (error) {
        console.error('Error during download import process:', error instanceof Error ? error.message : String(error));
        return 1;
    }
}
