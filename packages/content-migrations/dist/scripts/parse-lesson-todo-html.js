 ...
 */
function findLessonInYaml(lessons, title) {
    ...
    let lesson = lessons.find((l) => l.title.toLowerCase() === title.toLowerCase());
    if (lesson) {
    ...
    if (title.includes(':')) {
        ...
        const similiarLessons = lessons.filter((l) => normalizeTitle(l.title).includes(mainPartNormalized) ||
        ...
        if (similiarLessons.length > 1 && subPart) {
            ...
            const bestMatch = similiarLessons.reduce((best, current) => {
                ...
                const currentSimilarity = currentSubPart.includes(subPartNormalized) ||
                ...
                const bestSubPart = best?.title.includes(':')
                    ...
                    : '';
                const bestSimilarity = bestSubPart.includes(subPartNormalized) ||
    ...
    if (partialMatches.length > 1) {
        ...
        const shortestMatch = partialMatches.reduce((shortest, current) => current.title.length < shortest.title.length ? current : shortest, partialMatches[0]);
...
async function parseLessonTodoHtml() {
    ...
    for (const lessonSection of Array.from(lessonSections)) {
        ...
        while (currentElement && currentElement.tagName !== 'H1') {
            if (currentElement.tagName === 'H2') {
                ...
                if (sectionType === 'to-do') {
                    ...
                    console.log(`  ✓ Converted todo section to Lexical format (${sectionHtml.length} chars)`);
                    // Check if this includes "Complete the lesson quiz"
                    if (sectionHtml.toLowerCase().includes('complete the lesson quiz') ||
                    ...
                    console.log(`  ✓ Converted watch section to Lexical format (${sectionHtml.length} chars)`);
                }
                else if (sectionType === 'read') {
                    sectionContent.readContent = htmlToLexical(sectionHtml);
                    console.log(`  ✓ Converted read section to Lexical format (${sectionHtml.length} chars)`);
                }
                else if (sectionType === 'course project') {
                    sectionContent.courseProject = htmlToLexical(sectionHtml);
                    console.log(`  ✓ Converted course project section to Lexical format (${sectionHtml.length} chars)`);
                }
                else {
        ...
        if (yamlLesson) {
            ...
            console.log(`  ✓ Updated ${updatedFields} fields in YAML for "${yamlLesson.title}"`);
            updatedLessons.add(yamlLesson.title);
        }
        else {
            console.warn(`  ⚠ Could not find matching lesson in YAML for: "${lessonTitle}"`);
 ...
 */
function htmlToLexical(htmlContent) {
    if (!htmlContent.trim()) {
        return '';
    }
 ...
 */
function processElement(element, children) {
 ...
 */
function processListItems(element, children, listType) {
