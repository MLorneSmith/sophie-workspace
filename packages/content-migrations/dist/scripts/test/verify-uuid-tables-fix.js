...
async function verifyUuidTablesFix() {
    ...
    try {
        ...
        try {
            ...
            console.log(`Schema validation result: ${schemaValidation.isValid ? '✅ Valid' : '❌ Invalid'}`);
            console.log(`Available columns: ${schemaValidation.columns.join(', ')}`);
            if (schemaValidation.hasDeprecatedColumns) {
                console.log('⚠️ Schema contains deprecated columns that might be removed in future versions.');
            }
        ...
        `);
          ...
          `);
                    console.log(`Function executed successfully and processed ${result.rowCount} tables`);
                    if (result.rowCount > 0) {
                        console.log('Tables modified:');
                        result.rows.forEach((row) => {
                            console.log(`- ${row.table_name}: Added columns ${row.columns_added.join(', ')}`);
                        });
                    }
                    else {
                ...
                console.error('❌ Error executing scan_and_fix_uuid_tables function:', funcError.message);
            }
            ...
            try {
                ...
                const insertResult = await safeInsertIntoUuidTablesTracking(client, testTable);
                console.log(`Safe insert result: ${insertResult ? '✅ Success' : '❌ Failed'}`);
                // Verify the record was inserted
                const verifyResult = await client.query(`SELECT * FROM payload.dynamic_uuid_tables WHERE table_name = $1`, [testTable]);
                ...
                await client.query(`DELETE FROM payload.dynamic_uuid_tables WHERE table_name = $1`, [testTable]);
            }
            catch (insertError) {
                console.error('❌ Error testing safe insert function:', insertError.message);
            }
