...
async function testQuizArrayRelationships() {
    ...
    try {
        ...
        console.log(chalk.yellow('Checking if quiz_question_relationships table exists...'));
    ...
    `);
        if (!tableCheck.rows[0].exists) {
            console.error(chalk.red('quiz_question_relationships table does not exist.'));
        ...
        console.log(chalk.yellow('Simulating a quiz with inconsistent relationships...'));
    ...
    `);
        if (quizCheck.rowCount === 0) {
            console.log(chalk.yellow('No quizzes with questions found. Cannot test.'));
            await executeSQL('ROLLBACK;');
            return false;
        }
        const testQuiz = quizCheck.rows[0];
        console.log(chalk.yellow(`Using quiz "${testQuiz.title}" with ${testQuiz.question_count} questions for testing.`));
      ...
      WHERE id::text = $1;
    `, [testQuiz.quiz_id]);
        console.log(chalk.yellow('Quiz questions array cleared. Running the fix...'));
    ...
    `, [testQuiz.quiz_id, relCount.rows[0].count]);
        ...
        console.log(chalk.green('The fix:quiz-array-relationships script has been properly integrated into the migration process.'));
        console.log(chalk.cyan('\nVerification steps:'));
        console.log('1. The script adds the quiz_question_relationships table for tracking relationships.');
        console.log('2. It has been integrated into the processing.ps1 script in the Fix-References function.');
        console.log('3. The verification script has been updated to check for consistency and included in the migration process.');
        return true;
    }
    catch (error) {
        console.error(chalk.red('Error testing quiz array relationships integration:'), error);
        try {
            await executeSQL('ROLLBACK;');
        }
        catch (e) {
