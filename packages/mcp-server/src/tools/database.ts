...
export async function getSchemaFiles(): Promise<SchemaFile[]> {
	const schemasPath = join(process.cwd(), "apps", "web", "supabase", "schemas");
	...
	for (const file of files.filter((f) => f.endsWith(".sql"))) {
		...
		const topic = determineTopic(file, content);
...
export async function getFunctions(): Promise<DatabaseFunction[]> {
	...
	for (const schemaFile of schemaFiles) {
		...
		const fileFunctions = extractFunctionsFromContent(content, schemaFile.name);
...
export async function getSchemaContent(fileName: string): Promise<string> {
	const schemasPath = join(process.cwd(), "apps", "web", "supabase", "schemas");
...
export async function getSchemasByTopic(topic: string): Promise<SchemaFile[]> {
	const allSchemas = await getSchemaFiles();
...
export async function getSchemaBySection(
	section: string,
...
function extractFunctionsFromContent(
	content: string,
	sourceFile: string,
): DatabaseFunction[] {
	const functions: DatabaseFunction[] = [];
	...
	for (const match of content.matchAll(functionRegex)) {
		...
		const description = extractDescription(beforeFunction, body || "");
		const purpose = extractPurpose(description, functionName);
...
function extractDescription(beforeFunction: string, body: string): string {
...
function extractPurpose(description: string, functionName: string): string {
	// Map function names to purposes
	const purposeMap: Record<string, string> = {
		create_nonce: "Create one-time authentication tokens for secure operations",
		verify_nonce: "Verify and consume one-time tokens for authentication",
		is_mfa_compliant: "Check if user has completed multi-factor authentication",
...
function extractDependencies(content: string): string[] {
	...
	for (const match of content.matchAll(referencesRegex)) {
...
function determineTopic(fileName: string, content: string): string {
	...
	if (contentLower.includes("account") && contentLower.includes("team"))
		return "accounts";
	if (contentLower.includes("subscription") || contentLower.includes("billing"))
		return "billing";
	if (
