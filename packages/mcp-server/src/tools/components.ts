...
export async function getComponents(): Promise<ComponentInfo[]> {
	const packageJsonPath = join(process.cwd(), "packages", "ui", "package.json");
...
export async function getComponentProps(componentName: string): Promise<{
...
function extractVariants(
	...
	for (const variantMatch of variantsSection.matchAll(variantRegex)) {
		const [, variantName, variantOptions] = variantMatch;
		const options: string[] = [];

		// Extract option names
		const optionRegex = /(\w+):/g;
		for (const optionMatch of variantOptions.matchAll(optionRegex)) {
...
function determineCategory(filePath: string): "shadcn" | "makerkit" | "utils" {
...
async function generateDescription(
	...
	if (category === "shadcn") {
		return getShadcnDescription(componentName);
	} else if (category === "makerkit") {
		return getMakerkitDescription(componentName);
	} else {
		return getUtilsDescription(componentName);
	}
}

function getShadcnDescription(componentName: string): string {
	...
	return descriptions[componentName] || `Shadcn UI component: ${componentName}`;
}

function getMakerkitDescription(componentName: string): string {
	const descriptions: Record<string, string> = {
		...
		"language-selector": "Component for selecting application language/locale",
	...
	return (
		descriptions[componentName] || `Makerkit custom component: ${componentName}`
	);
}

function getUtilsDescription(componentName: string): string {
	const descriptions: Record<string, string> = {
		utils:
