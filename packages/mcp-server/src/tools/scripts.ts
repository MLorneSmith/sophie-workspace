...
export async function getScripts(): Promise<ScriptInfo[]> {
	...
	for (const [scriptName, command] of Object.entries(packageJson.scripts)) {
		if (typeof command === "string") {
			const scriptInfo = getScriptInfo(scriptName, command);
...
export async function getScriptDetails(
	scriptName: string,
): Promise<ScriptInfo> {
	const packageJsonPath = join(process.cwd(), "package.json");
	const packageJson = JSON.parse(await readFile(packageJsonPath, "utf8"));
	...
	return getScriptInfo(scriptName, command);
}

function getScriptInfo(scriptName: string, command: string): ScriptInfo {
...
export function getHealthcheckScripts(): ScriptInfo[] {
	const allScripts = ["typecheck", "lint", "lint:fix", "format:fix", "test"];

	return allScripts.map((scriptName) =>
		getScriptInfo(scriptName, `[healthcheck] ${scriptName}`),
	);
}
