...
export function PayloadContentRenderer({ content }: { content: unknown }) {
	...
	try {
		const lexicalContent = content as {
			root?: { children?: LexicalNode[]; text?: string };
		};
			...
			if (Array.isArray(lexicalContent.root.children)) {
				return (
					<div className="payload-content">
						{lexicalContent.root.children.map(
							(node: LexicalNode, i: number) => {
								...
								if (
									...
									const videoId = String(
										node.videoId || node.fields?.videoId || "",
									);
									const libraryId = String(
										node.libraryId || node.fields?.libraryId || "1234",
									);
									const title = String(
										node.title || node.fields?.title || "Video",
									);
								...
								if (node.type === "paragraph") {
									...
									return (
										<p key={`p-${i}-${node.text?.slice(0, 20) || "text"}`}>
											{node.text || ""}
										</p>
									);
								}
								...
								if (node.type === "heading") {
									...
									if (!Array.isArray(node.children)) {
										...
										switch (tag) {
											case "h1":
												return (
													<h1
														key={`h1-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h1>
												);
											case "h2":
												return (
													<h2
														key={`h2-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h2>
												);
											case "h3":
												return (
													<h3
														key={`h3-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h3>
												);
											case "h4":
												return (
													<h4
														key={`h4-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h4>
												);
											case "h5":
												return (
													<h5
														key={`h5-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h5>
												);
											case "h6":
												return (
													<h6
														key={`h6-${i}-${node.text?.slice(0, 20) || "heading"}`}
													>
														{node.text || ""}
													</h6>
												);
									...
									const headingContent = node.children.map(
										(textNode: LexicalNode, j: number) => (
											<span
												key={`heading-${i}-${j}-${textNode.text?.slice(0, 10) || "empty"}`}
											>
												{textNode.text || ""}
											</span>
										),
