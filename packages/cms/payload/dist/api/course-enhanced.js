...
export async function getQuizEnhanced(quizId, depth = 1, supabaseClient) {
    ...
    try {
        ...
        console.error(`getQuizEnhanced: Error extracting quiz ID from ${JSON.stringify(quizId)}:`, error);
    ...
    try {
        ...
        try {
            const quiz = await callPayloadAPI(`course_quizzes/${actualQuizId}?depth=${depth}`, {}, supabaseClient);
            if (!quiz || !quiz.id) {
                console.error(`getQuizEnhanced: Quiz not found for ID: ${actualQuizId}`);
            ...
            console.log(`getQuizEnhanced: Quiz ${quiz.title} has no questions, trying database fallback`);
            const dbQuestions = await getQuestionsForQuizFromDatabase(actualQuizId);
            if (dbQuestions && dbQuestions.length > 0) {
                console.log(`getQuizEnhanced: Found ${dbQuestions.length} questions via database fallback`);
            ...
            console.log(`getQuizEnhanced: No questions found for quiz ${quiz.title} using any method`);
            return quiz;
        }
        catch (fetchError) {
        ...
        console.log(`getQuizEnhanced: Successfully fetched quiz (fallback): ${quiz.title}`);
        ...
        console.error(`getQuizEnhanced: Error fetching quiz ${actualQuizId}:`, error);
        throw error;
    }
}
/**
 ...
 */
async function getQuestionsForQuizFromDatabase(quizId) {
    try {
