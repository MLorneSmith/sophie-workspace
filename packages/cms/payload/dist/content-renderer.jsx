...
function debugLog(...args) {
    if (DEBUG) {
        /* TODO: Async logger needed */ logger.info("[PayloadContentRenderer]", Object.assign({ data:  }, args));
    }
}
            ...
            if (Array.isArray(lexicalContent.root.children)) {
                return (<div className="payload-content">
                        ...
                        if (node.type === "custom-call-to-action" ||
                            ...
                            if (htmlContent) {
                                /* TODO: Async logger needed */ logger.info("Using HTML content for Call To Action:", { arg1: `${htmlContent.substring(0, arg2) });
                            }
                        }
                    })}: 100 })}...`,
									);
									// Note: Content trusted as it comes from Payload CMS admin
									return (
										<div key={`cta-html-${i}-${node.blockType || 'cta'}`} dangerouslySetInnerHTML={{ __html: htmlContent }}/>
									);
								}

								// Fallback rendering for Call To Action block
								return (
									<div key={`cta-fallback-${i}-${node.headline || node.text || 'cta'}`} className="my-6 rounded-md border border-blue-200 bg-blue-50 p-4">
                    ...
                    /* TODO: Async logger needed */ logger.info("Using HTML content for Test Block:", { arg1: `${htmlContent.substring(0, arg2) })}: 100 })}...`,
									...
									// Note: Content trusted as it comes from Payload CMS admin
									return (
										<div key={`test-fallback-${i}-${node.blockType || 'test'}`} dangerouslySetInnerHTML={{ __html: htmlContent }}/>
									);
								}

								// Fallback rendering for Test Block
								return (
									<div key={`test-fallback-${i}-${node.blockType || 'test'}`} className="my-6 rounded-md border border-blue-100 bg-blue-50 p-4">
										<h3 className="text-lg font-bold text-blue-700">
											Test Block
										</h3>
										<p className="mt-2 text-blue-600">
                    ...
                    /* TODO: Async logger needed */ logger.info("Using HTML content for Bunny Video:", { arg1: `${htmlContent.substring(0, arg2) })}: 100 })}...`,
									...
									// Note: Content trusted as it comes from Payload CMS admin
									return (
										<div key={`bunny-video-${i}-${node.videoId || 'video'}`} dangerouslySetInnerHTML={{ __html: htmlContent }}/>
									);
								}
								...
								if (!videoId) {}
									return (
										<div key={`node-${i}-${node.type || 'unknown'}`} className="my-6 rounded-md border border-gray-200 bg-gray-50 p-4">
                    ...
                    /* TODO: Async logger needed */ logger.info("Using HTML content for YouTube Video:", { arg1: `${htmlContent.substring(0, arg2) })}: 100 })}...`,
									...
									// Note: Content trusted as it comes from Payload CMS admin
									return (
										<div key={`youtube-video-${i}-${node.videoId || 'video'}`} dangerouslySetInnerHTML={{ __html: htmlContent }}/>
									);
								}
                ...
                // If it's a URL, return the extracted ID
								...
								if (!youtubeId) {}
									return (
										<div key={`youtube-video-${i}-${node.videoId || 'video'}`} className="my-6 rounded-md border border-gray-200 bg-gray-50 p-4">
											<h3 className="text-lg font-bold text-gray-700">
												{title}
											</h3>
											<div className="flex items-center justify-center rounded bg-gray-100 p-8">
                ...
                // Check if node.children exists and is an array
								...
								if (Array.isArray(node.children)) {}
									return (
										<p key={`p-block-${i}-${((_a = node.text) === null || _a === void 0 ? void 0 : _a.slice(0, 20)) || 'text'}`}>
											{node.children.map((textNode, j) => {
                        var _a;
                        return (<span key={`text-${i}-${j}-${((_a = textNode.text) === null || _a === void 0 ? void 0 : _a.slice(0, 10)) || 'empty'}`}>{textNode.text || ""}</span>);
                    })}
										</p>
									);
								}
								// Fallback for when children is not an array
								return <p key={`p-${i}-${((_b = node.text) === null || _b === void 0 ? void 0 : _b.slice(0, 20)) || 'text'}`}>{node.text || ""}</p>;
							}
                ...
                // Use switch for the fallback case too
									...
									switch (tag) {}
										case "h1":
											return <h1 key={`h1-${i}-${((_c = node.text) === null || _c === void 0 ? void 0 : _c.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h1>;
										case "h2":
											return <h2 key={`h2-${i}-${((_d = node.text) === null || _d === void 0 ? void 0 : _d.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h2>;
										case "h3":
											return <h3 key={`h3-${i}-${((_e = node.text) === null || _e === void 0 ? void 0 : _e.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h3>;
										case "h4":
											return <h4 key={`h4-${i}-${((_f = node.text) === null || _f === void 0 ? void 0 : _f.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h4>;
										case "h5":
											return <h5 key={`h5-${i}-${((_g = node.text) === null || _g === void 0 ? void 0 : _g.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h5>;
										case "h6":
											return <h6 key={`h6-${i}-${((_h = node.text) === null || _h === void 0 ? void 0 : _h.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h6>;
										default:
											return <h2 key={`h2-${i}-${((_j = node.text) === null || _j === void 0 ? void 0 : _j.slice(0, 20)) || 'heading'}`}>{node.text || ""}</h2>;
									}
								}
								...
								const headingContent = node.children.map(
									(textNode: LexicalNode, j: number) => (
										<span key={`heading-${i}-${j}-${((_k = textNode.text) === null || _k === void 0 ? void 0 : _k.slice(0, 10)) || 'empty'}`}>{textNode.text || ""}</span>
									),
								);

								switch (tag) {}
									case "h1":
										return <h1 key={`h1-${i}-${((_l = node.text) === null || _l === void 0 ? void 0 : _l.slice(0, 20)) || 'heading'}`}>{headingContent}</h1>;
									case "h2":
										return <h2 key={`h2-${i}-${((_m = node.text) === null || _m === void 0 ? void 0 : _m.slice(0, 20)) || 'heading'}`}>{headingContent}</h2>;
									case "h3":
										return <h3 key={`h3-${i}-${((_o = node.text) === null || _o === void 0 ? void 0 : _o.slice(0, 20)) || 'heading'}`}>{headingContent}</h3>;
									case "h4":
										return <h4 key={`h4-${i}-${((_p = node.text) === null || _p === void 0 ? void 0 : _p.slice(0, 20)) || 'heading'}`}>{headingContent}</h4>;
									case "h5":
										return <h5 key={`h5-${i}-${((_q = node.text) === null || _q === void 0 ? void 0 : _q.slice(0, 20)) || 'heading'}`}>{headingContent}</h5>;
									case "h6":
										return <h6 key={`h6-${i}-${((_r = node.text) === null || _r === void 0 ? void 0 : _r.slice(0, 20)) || 'heading'}`}>{headingContent}</h6>;
									default:
										return <h2 key={`h2-${i}-${((_s = node.text) === null || _s === void 0 ? void 0 : _s.slice(0, 20)) || 'heading'}`}>{headingContent}</h2>;
								}
							}
