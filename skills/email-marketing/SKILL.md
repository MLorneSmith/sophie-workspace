...
description: "Write compelling marketing emails using Andre Chaperon's methodology with SlideHeroes voice. Includes 118 annotated examples, hook quality scoring, two-stage campaign workflow, and Gmail export tool."
license: MIT
metadata:
  version: 1.0.0
  model: opus
...
Campaign Strategy (/email-campaign)
    ...
    ├── Create Mission Control task (board: Newsletter & Content)
    └── Save strategy YAML (includes mission_control.task_id)
    │
    ▼
Email Execution (/email-write)
    │
    ├── Load campaign strategy (incl. task_id)
    ├── Load shared contexts needed for writing
    ├── Load adjacent emails
    ├── Apply techniques
    ├── Self-review checklist
    ├── Save email YAML
    └── Update Mission Control progress (activity_note)
```
...
## Context Foundation (.ai/contexts)

This skill integrates with the shared context layer at:

```
.ai/contexts/
```

### Contexts loaded by `/email-campaign`
...
### Contexts loaded by `/email-write`

- `voice/mike-style.md`
- `guidelines/email-guidelines.md`
- `personas/{target}.md` (when available / specified in strategy)

---

## Mission Control Integration

This skill creates and updates a Mission Control task to track campaign progress.

### On campaign start (`/email-campaign`)

- Create task:
  - `POST http://localhost:3001/api/v1/tasks`
  - `name`: `"Email Campaign: {campaign-name}"`
  - `board_id`: `3` (Newsletter & Content)
- Store returned task ID in the saved strategy YAML:
...
- Patch the task with an activity note that includes **hook scores** (e.g., top hooks for key emails).

### As emails are written (`/email-write`)

- After saving the approved email, PATCH the task:
  - `activity_note`: `"Email {position}/{total} written"`

### On completion

- When the final email is written, mark the task complete:
...
118 emails (all annotated with techniques) organized two ways:
...
Add technique annotations to emails using rule-based pattern detection.

```bash
# Single file
python scripts/annotate_email.py path/to/email.yaml

# Batch process directory
python scripts/annotate_email.py corpus/by-type/onboarding --batch

# Dry run (preview without saving)
python scripts/annotate_email.py corpus/by-type/onboarding --batch --dry-run
```

**Detects:** Personal Greeting, Campaign Abbreviation, Open Loop, P.S. Section, Curiosity Gap, Permission to Leave, Social Proof, Soft Sell, Clever Signature, Contrarian, Story Hook, Question Hook

**Note:** Rule-based detection catches common patterns. For deeper analysis, review and enhance annotations manually.
