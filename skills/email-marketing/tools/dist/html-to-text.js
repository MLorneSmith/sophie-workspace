 ...
 */
function preprocessEmailHtml(html) {
    let processed = html;
    // Remove tracking pixels and tiny images
    processed = processed.replace(/<img[^>]*(?:width|height)\s*=\s*["']?[01](?:px)?["']?[^>]*>/gi, "");
    // Remove style and script tags entirely
    processed = processed.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "");
    processed = processed.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "");
    // Convert common signature markers
    processed = processed.replace(/<div[^>]*class="[^"]*gmail_signature[^"]*"[^>]*>/gi, '<div class="signature">\n---\n');
    // Handle quoted reply sections
    processed = processed.replace(/<div[^>]*class="[^"]*gmail_quote[^"]*"[^>]*>/gi, "<blockquote>");
...
function cleanQuotedText(text) {
    ...
    processed = processed.replace(/On .+? wrote:\s*>/gm, "On [previous message]:\n>");
 ...
 */
export function extractCleanText(plain, html) {
