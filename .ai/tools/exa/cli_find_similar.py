#!/usr/bin/env python3
"""
CLI script for Exa Find Similar Links API.

Provides command-line access to similar page discovery.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Find pages similar to a given URL using Exa API",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s https://example.com --num-results 10
  %(prog)s https://example.com --text --summary
  %(prog)s https://example.com --include-domains example.com,example.org
        """,
    )

    # Required arguments
    parser.add_argument("url", help="Source URL to find similar pages for")
...
def _print_results(response, args) -> None:
    """Print similar pages in human-readable format."""
    print(f"\nFound {len(response.results)} similar pages to: {args.url}\n")
