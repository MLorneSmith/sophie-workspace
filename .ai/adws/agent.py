...
def check_claude_auth() -> Tuple[bool, Optional[str]]:
    """Check if Claude Code is authenticated (either via session or API key).

    Returns:
        Tuple of (has_auth, auth_type) where auth_type is 'session', 'api_key', or None
    """
    ...
    # The Claude CLI stores session in ~/.claude/ directory
    claude_config_dir = os.path.expanduser("~/.claude")
    session_file = os.path.join(claude_config_dir, "session.json")

    if os.path.exists(session_file):
        try:
            # Verify session is valid by trying a simple command
            result = subprocess.run(
                [CLAUDE_PATH, "--version"],
                capture_output=True,
                text=True,
...
def get_claude_env() -> Dict[str, str]:
    ...
    1. Claude Code session authentication (Claude Max plan)
    ...
    if not has_auth:
        print("Warning: No Claude Code authentication found. Please either:", file=sys.stderr)
        print("  1. Run 'claude' and log in with your Max plan credentials, OR", file=sys.stderr)
        print("  2. Set ANTHROPIC_API_KEY environment variable", file=sys.stderr)
    else:
        print(f"Using Claude Code authentication: {auth_type}")
    ...
    required_env_vars = {
        ...
        # Basic environment variables Claude Code needs
        "HOME": os.getenv("HOME"),
        "USER": os.getenv("USER"),
        "PATH": os.getenv("PATH"),
    ...
    if api_key and auth_type == "api_key":
        required_env_vars["ANTHROPIC_API_KEY"] = api_key
