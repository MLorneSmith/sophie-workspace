...
Mastra validation spike: Audience Profiling workflow + parallel agents


## Description

1-2 week spike to validate Mastra before full commitment. Implement: (1) audienceProfilingWorkflow with suspend/resume + schema working memory, (2) postProcessWorkflow running Partner + Skeptic in parallel with structured suggestions, (3) observability export with token usage totals per run. Success criteria: workflows run end-to-end, HITL suspend/resume works, parallel agents return structured output, tracing captures token usage. Ref: deliverables/mastra-deep-evaluation.md
