...
function findInitiativeDir(
	...
	for (const specDir of specDirs) {
		...
		for (const item of contents) {
			...
			if (match?.[1] && parseInt(match[1], 10) === initiativeId) {
...
function loadTasksJson(featureDir: string): TasksJson | null {
	...
	try {
		...
		// Run parallelization analysis (cast to unknown first to handle type differences)
		const analyzed = analyzeTasksJson(tasksJson);
		return analyzed as unknown as TasksJson;
	} catch (error) {
...
function extractFeatureDependencies(featureDir: string): number[] {
	...
	try {
		...
		if (blockedBySection) {
			const issueMatches = blockedBySection.match(/#(\d+)/g);
			if (issueMatches) {
...
async function main() {
	...
	const initiativeIdArg = args[0];
	const initiativeId = initiativeIdArg ? parseInt(initiativeIdArg, 10) : NaN;
