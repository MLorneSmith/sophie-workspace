...
export function sendMetricsToNewRelic(testName, metrics) {
  ...
  try {
    const response = http.post(NEW_RELIC_API_URL, JSON.stringify(payload), params);
    if (response.status !== 202) {
      console.error(`Failed to send metrics to New Relic: ${response.status} ${response.body}`);
    }
  } catch (error) {
