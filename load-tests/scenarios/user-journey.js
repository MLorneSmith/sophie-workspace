...
export default function (authData) {
  ...
  group('User Journey', () => {
    ...
    if (!check(dashboardResponse, { 'dashboard loads': (r) => r.status === 200 })) {
...
function performProjectCreate(headers) {
  group('Create Project', () => {
    ...
    const createResponse = http.post(`${apiBaseUrl}/api/projects`, projectData, headers);
...
function performProjectEdit(headers) {
  group('Edit Project', () => {
    // First get a project
    const projectsResponse = http.get(`${apiBaseUrl}/api/projects?limit=1`, headers);
...
function performAnalyticsView(headers) {
  group('View Analytics', () => {
    const analyticsResponse = http.get(`${apiBaseUrl}/api/analytics/overview`, headers);
...
function performTeamManagement(headers) {
  group('Manage Team', () => {
    ...
    const invitationsResponse = http.get(`${apiBaseUrl}/api/team/invitations`, headers);
...
function performProfileUpdate(headers) {
  group('Update Profile', () => {
    ...
    const updateResponse = http.patch(`${apiBaseUrl}/api/user/profile`, updateData, headers);
    check(updateResponse, {
