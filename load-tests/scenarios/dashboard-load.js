...
export default function (authData) {
  ...
  group('Project Operations', () => {
    ...
    const projectsResponse = http.get(`${apiBaseUrl}/api/projects?limit=10&page=1`, headers);
    ...
    if (projectsResponse.status === 200) {
      ...
      if (projects.length > 0) {
        ...
        const projectDetailResponse = http.get(`${apiBaseUrl}/api/projects/${projectId}`, headers);
  ...
  group('Real-time Updates', () => {
    ...
    for (let i = 0; i < 3; i++) {
      const updatesResponse = http.get(`${apiBaseUrl}/api/updates/check`, headers);
