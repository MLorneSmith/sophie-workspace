...
describe("Private Collection", () => {
	describe("Collection Configuration", () => {
		...
		it("should use title as admin display field", () => {
			expect(Private.admin?.useAsTitle).toBe("title");
		});

		it("should have correct default columns", () => {
			expect(Private.admin?.defaultColumns).toEqual([
				"title",
				"status",
				"publishedAt",
			]);
		...
		it("should have admin description", () => {
			expect(Private.admin?.description).toBe(
	...
	describe("Access Control", () => {
		it("should allow public read access", () => {
			expect(Private.access?.read).toBeDefined();
			// @ts-expect-error - testing access function
			expect(Private.access?.read()).toBe(true);
		});
	});

	describe("Versioning", () => {
		it("should have drafts enabled", () => {
			expect(Private.versions).toEqual({
				drafts: true,
			});
		});
	});
	...
	describe("Fields", () => {
		...
		it("should have image_id upload field relating to downloads", () => {
		...
		it("should have featured_image_id upload field relating to downloads", () => {
			const featuredField = fields.find(
				(f) => "name" in f && f.name === "featured_image_id",
			);
	...
	describe("Slug Hook", () => {
		const slugField = Private.fields.find(
		...
		it("should generate slug from title when slug is empty", () => {
			const result = beforeValidateHook?.({
				value: null,
				data: { title: "My Private Post" },
			});
			expect(result).toBe("my-private-post");
		});

		it("should generate slug removing special characters", () => {
			const result = beforeValidateHook?.({
				value: "",
				data: { title: "Private: Secret Content!" },
			});
			expect(result).toBe("private-secret-content");
		});
