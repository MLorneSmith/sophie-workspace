...
import { runStage2_SeedCore, AggregatedIdMaps } from './stages/stage2-seed-core-content.js'; // Import Stage 2 orchestrator
...
(async () => {
  ...
  const logger = pinoModule.pino({ // Use pinoModule.pino
  ...
  try {
    ...
    const sourcePayloadConfigPath = path.resolve(projectRoot, argv.payloadConfigPath as string);
    ...
    const compiledConfigRelativePath = (argv.payloadConfigPath as string)
    ...
    let idMaps: AggregatedIdMaps = {}; // Use the imported type

    // Call Stages Sequentially:
    // Stage 0: Database Reset
    if (!argv.skipResetSchema) {
      await runStage0_ResetSchema(logger, argv as any);
    }

    // Stage 1: Apply Payload Migrations
    if (!argv.skipApplyMigrations) {
      await runStage1_ApplyMigrations(logger, argv as any);
    }

    // Stage 2: Core Content Seeding
    if (!argv.skipSeedCore) {
      idMaps = await runStage2_SeedCore(payloadClient, logger, argv as any);
    } else {
    ...
    if (payloadClient) {
      ...
      try {
        ...
        if (db && 'drizzle' in db) {
          const drizzleClient = (db as any).drizzle;
