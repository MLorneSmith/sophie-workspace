...
import { DOWNLOAD_DEFINITIONS, type DownloadDefinition } from '../data/definitions/download-definitions.js'; // Import the SSOT data
import { Stage2Args } from '../stages/stage2-seed-core-content.js'; // For cliArgs type
...
export async function seedDownloads(
  ...
  const idMap: Record<string, string> = {}; // Maps SSOT download key to live DB UUID
  ...
  try {
    ...
    for (const downloadDef of validatedDownloadDefs) {
      ...
      try {
        ...
        const dataToCreate: Partial<any> = { // Replace 'any' with actual Payload generated type for Download/Media
