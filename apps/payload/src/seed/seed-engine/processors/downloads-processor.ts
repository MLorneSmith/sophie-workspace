/**
 * Downloads collection processor with URL-based seeding
 *
...
/**
 * Specialized processor for downloads collection with URL-based seeding
 *
 * The downloads collection uses pre-uploaded files from Cloudflare R2.
 * This processor:
 ...
 *   "url": "https://downloads.{account_id}.r2.cloudflarestorage.com/marketing-template.pdf",
 ...
 *     _ref: "template-1",
 *     filename: "marketing-template.pdf",
 *     url: "https://downloads.account.r2.cloudflarestorage.com/marketing-template.pdf",
 *     title: "Marketing Template",
...
export class DownloadsProcessor extends BaseProcessor {
   ...
   * 1. Check required fields (url, filename, title)
  ...
  /**
   * Create download record with pre-uploaded R2 URL
   *
   * **URL-Based Process**:
   ...
   *
   * @param record - Download record with R2 URL and metadata
  ...
  async processRecord(record: SeedRecord): Promise<string> {
    ...
    // Set placeholder filesize if missing
    ...
    try {
      ...
      throw new Error(
        `Failed to create download record for '${record.filename}': ${errorMessage}`,
      );
    }
  }
   ...
   *
   * Logs statistics about downloads processing:
