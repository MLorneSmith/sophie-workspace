...
// Paths
const COURSE_LESSONS_JSON = path.join(__dirname, '../seed-data/course-lessons.json');
...
const mediaRecords: MediaRecord[] = JSON.parse(fs.readFileSync(MEDIA_JSON, 'utf-8'));

// Create filename → _ref mapping
const filenameToRef = new Map<string, string>();
mediaRecords.forEach(record => {
  const filenameStem = record.filename.replace(/\.(webp|jpg|png|gif|svg|ico)$/i, '');
...
const courseLessons: CourseLessonRecord[] = JSON.parse(fs.readFileSync(COURSE_LESSONS_JSON, 'utf-8'));
  ...
  if (typeof obj === 'string') {
    ...
    if (matches.length > 0) {
      ...
      for (const match of matches) {
        ...
        if (newRef !== oldRef) {
          result = result.replace(`{ref:media:${oldRef}}`, `{ref:media:${newRef}}`);
