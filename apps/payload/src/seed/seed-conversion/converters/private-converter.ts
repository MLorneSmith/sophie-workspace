...
import { parseHtmlFile, convertHtmlToLexical } from '../parsers/html-parser';
  ...
  try {
    ...
    for (const file of htmlFiles) {
      try {
        ...
        const lexicalContent = await convertHtmlToLexical(
          parsed.content,
          null
        );
    ...
    if (privatePosts.length > 0) {
      ...
      console.log(`    ✅ Wrote ${privatePosts.length} private posts to private.json`);
    } else {
