...
const currentMedia: MediaRecord[] = JSON.parse(fs.readFileSync(MEDIA_JSON, 'utf-8'));
...
const updatedMedia = currentMedia.map(record => {
  ...
  if (record._ref !== filenameStem) {
    console.log(`✓ Fixing: "${record._ref}" → "${filenameStem}" (${record.filename})`);
    return {
      ...record,
      _ref: filenameStem,
    };
...
// Summary of changes
const changedCount = updatedMedia.filter((record, index) =>
  record._ref !== currentMedia[index]._ref
).length;
