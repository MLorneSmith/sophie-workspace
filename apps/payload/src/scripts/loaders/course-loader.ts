...
import type { PayloadResponse, MarkdocLesson, MarkdocQuiz } from '../lib/types'
...
export async function loadCourseFromMarkdoc(options: {
  ...
  try {
    ...
    const lessons = lessonFiles.map((file) => {
      const parsedLesson = parseMarkdocFile(file) as MarkdocLesson
      return {
        ...parsedLesson,
    ...
    const quizzes = quizFiles.map((file) => {
      const parsedQuiz = parseMarkdocFile(file) as MarkdocQuiz
      return {
        ...parsedQuiz,
        filename: path.basename(file),
      }
    ...
    const checkResponse = (await api.callAPI(
      `courses?where[slug][equals]=${courseSlug}`,
    )) as PayloadResponse
    ...
    if (checkResponse.docs && checkResponse.docs.length > 0) {
      ...
      const courseData = {
        ...
        introContent: markdocToLexical(
          'Welcome to the Decks for Decision Makers course! This course will teach you how to create effective presentation decks.',
        ),
        completionContent: markdocToLexical(
          'Congratulations on completing the course! You now have the skills to create effective presentation decks.',
        ),
    ...
    for (const quiz of quizzes) {
      ...
      const checkQuizResponse = (await api.callAPI(
        `course_quizzes?where[title][equals]=${encodeURIComponent(quizTitle)}`,
      )) as PayloadResponse
    ...
    for (const lesson of lessons) {
      ...
      const checkLessonResponse = (await api.callAPI(
        `course_lessons?where[slug][equals]=${lessonSlug}`,
      )) as PayloadResponse
 ...
 */
function findAssociatedQuizFilename(lessonFilename: string, quizzes: any[]): string | null {
 ...
 */
export async function loadCourse(lessonsDir: string, quizzesDir: string) {
