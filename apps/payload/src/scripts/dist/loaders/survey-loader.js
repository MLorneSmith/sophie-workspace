...
export async function loadSurveyFromYaml(options) {
    ...
    try {
        ...
        const surveyYaml = parseYamlFile(yamlFilePath);
        ...
        const checkResponse = (await api.callAPI(`surveys?where[slug][equals]=${slug}`));
        let survey;
        if (checkResponse.docs && checkResponse.docs.length > 0) {
            ...
            if (typeof survey.doc.startMessage === 'string') {
                ...
                const lexicalObject = textToLexical('Welcome to the survey! This will help you evaluate your current skills and identify areas where you can improve.');
            ...
            const surveyData = {
                ...
                startMessage: textToLexical('Welcome to the survey! This will help you evaluate your current skills and identify areas where you can improve.'),
        ...
        const questions = surveyYaml.questions.map((q, index) => {
            ...
            return {
                ...
                options: q.answers.map((a) => ({ option: a.answer })),
                position: index,
            };
        });
 ...
 */
export async function loadSurvey(yamlFilePath) {
