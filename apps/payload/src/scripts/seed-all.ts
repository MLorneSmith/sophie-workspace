...
program
  ...
  .action(async (yamlFile: string, options: any, command: Command) => {
...
program
  ...
  .action(async (lessonsDir: string, quizzesDir: string, options: any, command: Command) => {
    const globalOptions = command.parent?.opts() || {}
    setupLogging(globalOptions)
...
function setupLogging(options: { verbose?: boolean; quiet?: boolean }) {
  if (options.verbose) {
