...
class DatabaseAdapterManager {
  ...
  private startBackgroundValidation(): void {
    ...
    this.validationPromise = this.performBackgroundValidation()
      .catch((error) => {
        this.log(`Background validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'warn');
  ...
  private async testConnectionAsync(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Async connection test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private startHealthMonitoring(): void {
    ...
    if (this.environment !== 'production' && !process.env.ENABLE_DB_HEALTH_MONITORING) {
      return;
    }

    const interval = parseInt(process.env.DB_HEALTH_CHECK_INTERVAL || '30000'); // Default 30 seconds
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      this.log(`Health check failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
      this.metrics.failedConnections++;
    }
  }
  ...
  private logMetrics(): void {
    if (this.environment === 'development' || process.env.ENABLE_DB_METRICS_LOGGING) {
 ...
 */
export async function getDatabaseAdapterAsync(): Promise<ReturnType<typeof postgresAdapter>> {
