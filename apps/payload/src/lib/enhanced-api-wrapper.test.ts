...
describe('EnhancedAPIManager', () => {
  ...
  describe('Constructor and Initialization', () => {
    ...
    it('should create environment logger with correct namespace', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Enhanced API Manager initialized');
    });
  });
  ...
  describe('Request ID Generation', () => {
    it('should generate unique request IDs with correct format', () => {
      ...
      const generateRequestId = (manager as any).generateRequestId.bind(manager);
  ...
  describe('Client Information Extraction', () => {
    ...
    it('should extract IP from x-forwarded-for header', () => {
      const manager = getEnhancedAPIManager();
      const extractClientInfo = (manager as any).extractClientInfo.bind(manager);
  ...
  describe('Enhanced Handler Creation - Successful Flow', () => {
    ...
    it('should wrap original handler successfully', async () => {
      ...
      const enhancedHandler = manager.createEnhancedHandler(mockOriginalHandler, 'GET');
      const request = createMockRequest();
      
      const response = await enhancedHandler(request, { collection: 'test' });
      
      expect(mockOriginalHandler).toHaveBeenCalledWith(request, { collection: 'test' });
    ...
    it('should update metrics for successful requests', async () => {
      const manager = getEnhancedAPIManager();
      const mockOriginalHandler = vi.fn().mockResolvedValue(new Response('OK', { status: 200 }));
    ...
    it('should log incoming requests', async () => {
      ...
      const enhancedHandler = manager.createEnhancedHandler(mockOriginalHandler, 'GET');
      const request = createMockRequest('https://example.com/api/test?param=value');
  ...
  describe('Enhanced Handler Creation - Error Handling', () => {
    ...
    it('should handle thrown errors and return appropriate response', async () => {
      ...
      const enhancedHandler = manager.createEnhancedHandler(mockOriginalHandler, 'POST');
      const request = createMockRequest();
    ...
    it('should update metrics for failed requests', async () => {
      const manager = getEnhancedAPIManager();
      const mockOriginalHandler = vi.fn().mockRejectedValue(new Error('Test error'));
  ...
  describe('Metrics Management', () => {
    it('should calculate moving average response time correctly', async () => {
      ...
      const updateResponseMetrics = (manager as any).updateResponseMetrics.bind(manager);
  ...
  describe('Error Log Management', () => {
    it('should maintain maximum error log size', () => {
      const manager = getEnhancedAPIManager();
      const addErrorToLog = (manager as any).addErrorToLog.bind(manager);
  ...
  describe('Error Response Creation', () => {
    const manager = getEnhancedAPIManager();
    const createErrorResponse = (manager as any).createErrorResponse.bind(manager);
  ...
  describe('Module-Level Functions', () => {
    ...
    it('should create enhanced Payload handlers', () => {
      const mockPayloadConfig = {} as any;
  ...
  describe('Edge Cases', () => {
    it('should handle requests with malformed URLs gracefully', async () => {
      ...
      // Create request with malformed URL - this will test URL parsing robustness
      const request = {
      ...
      // The URL constructor should handle this - if it throws, our handler should catch it
      await expect(enhancedHandler(request)).resolves.toBeInstanceOf(NextResponse);
    });
