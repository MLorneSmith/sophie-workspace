...
class RequestDeduplicationManager {
  ...
  constructor(config?: Partial<DeduplicationConfig>) {
    this.config = {
      ...
      enableLogging: process.env.NODE_ENV === 'development' || process.env.ENABLE_REQUEST_DEDUP_LOGGING === 'true',
      ...config,
    };
  ...
  private shouldDeduplicate(request: NextRequest): boolean {
    ...
    return this.config.protectedEndpoints.some(endpoint => 
   ...
   */
  private async serializeResponse(response: NextResponse): Promise<CachedResponse> {
   ...
   */
  private async waitForOngoingRequest(fingerprint: string): Promise<NextResponse | null> {
    ...
    while (Date.now() - startTime < this.config.processingTimeout) {
      ...
      if (!cached.isProcessing && cached.response) {
        this.log(`Request completed, returning cached response for fingerprint: ${fingerprint.slice(0, 8)}...`, 'debug');
    ...
    this.log(`Timeout waiting for ongoing request: ${fingerprint.slice(0, 8)}...`, 'warn');
    return null;
  }
  ...
  async processRequest(
    ...
    this.log(`Processing request with fingerprint: ${fingerprint.slice(0, 8)}...`, 'debug');
    ...
    if (cached && !cached.isProcessing && cached.response) {
      ...
      if (age < this.config.cacheDuration) {
        cached.requestCount++;
        this.log(`Returning cached response (age: ${age}ms, count: ${cached.requestCount})`, 'info');
        return this.deserializeResponse(cached.response);
      } else {
    ...
    if (cached && cached.isProcessing) {
      cached.requestCount++;
      this.log(`Duplicate request detected, waiting for ongoing processing (count: ${cached.requestCount})`, 'info');
      ...
      this.log(`Failed to wait for ongoing request, processing new one`, 'warn');
    }
  ...
  private cleanup(): void {
    ...
    for (const [fingerprint, entry] of this.cache.entries()) {
      ...
      const maxAge = entry.isProcessing ? this.config.processingTimeout : this.config.cacheDuration;
    ...
    if (removed > 0) {
      this.log(`Cleanup completed: removed ${removed}/${initialSize} entries`, 'debug');
    }
    ...
    if (this.cache.size > this.config.maxDuplicates * 2) {
      ...
      const toRemove = entries.slice(0, entries.length - this.config.maxDuplicates);
      toRemove.forEach(([fingerprint]) => {
        this.cache.delete(fingerprint);
      });
      
      this.log(`Cache size limit reached, removed ${toRemove.length} oldest entries`, 'info');
    }
  }
...
export function getDeduplicationManager(): RequestDeduplicationManager {
  if (!globalThis.__request_deduplication_manager) {
    globalThis.__request_deduplication_manager = new RequestDeduplicationManager();
  }
