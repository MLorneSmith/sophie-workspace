...
((): void => {
	...
	): "server-rendered" | "dynamic" | "unknown" {
	...
	function isUltraConservativeHydrationComplete(): boolean {
		...
		hydrationState.confirmationSignals = Math.max(
			hydrationState.confirmationSignals,
			currentSignals,
		);
		...
		const hasEnoughSignals =
			hydrationState.confirmationSignals >= hydrationState.requiredSignals;
	...
	function _waitForUltraConservativeHydration(callback: () => void): void {
		const checkHydration = () => {
			if (isUltraConservativeHydrationComplete()) {
				...
				const elapsed = Date.now() - hydrationState.startTime;
				log(
					`Ultra-conservative hydration detected complete after ${elapsed}ms with ${hydrationState.confirmationSignals} signals`,
					"info",
				);
				callback();
				return;
	...
	function protectFormInMemoryOnly(form: HTMLFormElement): void {
		...
		log(
			`Protected ${formType} form in MEMORY ONLY (no DOM changes): ${formId}`,
			"debug",
		);
	}
	...
	function scanAndProtectForms(): number {
		...
		for (const selector of CONFIG.formSelectors) {
			try {
				...
				log(
					`Error finding forms with selector "${selector}": ${errorMessage}`,
					"error",
				);
			}
		}
				...
				for (const node of mutation.addedNodes) {
					if (node.nodeType === Node.ELEMENT_NODE) {
						...
						if (
							...
							element.querySelectorAll("form").length > 0
						) {
							shouldCheck = true;
						}
					}
	...
	function _handleFormSubmission(event: SubmitEvent): void {
		...
		if (formState.state === "submitting") {
			...
			showMemoryOnlyMessage(
				form,
				"Form submission already in progress. Please wait...",
				"info",
			);
			return;
		}
		...
		log(
			`Form submission started - MEMORY ONLY TRACKING: ${formState.id} (${formState.type})`,
			"info",
		);
	}

	// Form state management - MEMORY ONLY
	function storeButtonStatesInMemoryOnly(
		form: HTMLFormElement,
		formState: FormState,
	): void {
		for (const selector of CONFIG.buttonSelectors) {
	...
	function enableFormMemoryOnly(form: HTMLFormElement): void {
		...
		log(
			`Form ${formState.id} enabled in memory only - NO DOM RESTORATION`,
			"debug",
		);
	...
	function showMemoryOnlyMessage(
		form: HTMLFormElement,
		message: string,
		...
		log(
			`Form ${formId} message (${type}): ${message}`,
	...
	const initialize = (): void => {
		...
		log(
			"Initializing ULTRA-CONSERVATIVE hydration-safe form protection...",
			"info",
		);
		...
		waitForUltraConservativeHydration(() => {
			log(
				"Ultra-conservative hydration complete, but staying in SAFE MODE (no DOM modifications)",
				"info",
			);
			// Even after hydration, we maintain ultra-conservative approach
			// NO DOM modifications whatsoever
			log(
				"Post-hydration setup complete - MEMORY ONLY MODE MAINTAINED",
				"info",
			);
		});
		...
		isInitialized = true;
		log(
			"ULTRA-CONSERVATIVE form protection initialized - ZERO DOM MODIFICATIONS",
			"info",
		);
	};
	...
	(
		globalThis as typeof globalThis & {
			__ultraConservativeFormProtectionStatus: () => object;
		}
	).__ultraConservativeFormProtectionStatus = () => {
		return {
			...
			formsByType: Array.from(formStates.values()).reduce(
				(acc, state) => {
			...
			formsByState: Array.from(formStates.values()).reduce(
				(acc, state) => {
