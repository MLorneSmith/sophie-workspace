...
function createMockR2Config(overrides: Partial<import('./storage-config').R2Config> = {}): import('./storage-config').R2Config {
  return {
...
describe('Storage URL Generators', () => {
  ...
  describe('generateMediaURL (R2)', () => {
    it('should generate R2 media URL with custom base URL', () => {
      // Arrange
      vi.mocked(getR2Config).mockReturnValue(createMockR2Config({
    ...
    it('should generate R2 media URL from bucket settings when no custom base URL', () => {
      ...
      // Assert
      expect(result).toBe('https://media-bucket.account123.r2.cloudflarestorage.com/test.jpg');
    });
    ...
    it('should handle special characters in filename', () => {
      ...
      // Act
      const result = generateMediaURL({ filename: 'test file with spaces & symbols.jpg' });

      // Assert
      expect(result).toBe('https://custom.domain.com/test file with spaces & symbols.jpg');
    });
  });
  ...
  describe('generateDownloadsURL (R2)', () => {
    ...
    it('should generate R2 downloads URL from bucket settings when no custom base URL', () => {
      ...
      // Assert
      expect(result).toBe('https://downloads-bucket.account123.r2.cloudflarestorage.com/document.pdf');
    });
  });
  ...
  describe('generateS3MediaURL', () => {
    ...
    it('should generate S3 media URL from bucket settings when no custom base URL', () => {
      ...
      // Assert
      expect(result).toBe('https://my-bucket.s3.us-west-2.amazonaws.com/image.png');
    });
  ...
  describe('generateS3DownloadsURL', () => {
    it('should generate S3 downloads URL with custom base URL', () => {
      // Arrange
      vi.stubEnv('PAYLOAD_PUBLIC_DOWNLOADS_BASE_URL', 'https://downloads.cdn.com');
    ...
    it('should generate S3 downloads URL with downloads prefix when no custom base URL', () => {
      ...
      // Assert
      expect(result).toBe('https://my-bucket.s3.us-west-2.amazonaws.com/downloads/file.zip');
    });
  });
  ...
  describe('getURLGenerator (Factory Function)', () => {
    ...
    it('should return fallback generator for unknown storage type', () => {
      // Act
      const generator = getURLGenerator('unknown' as any, 'media');
    ...
    it('should return fallback generator for unhandled storage type combination', () => {
      // Act
      const generator = getURLGenerator('local' as any, 'media');
  ...
  describe('createURLGenerator (Enhanced Generator)', () => {
    ...
    it('should handle undefined filename with warning log and placeholder URL', () => {
      ...
      // Act
      const result = generator({ filename: undefined as any });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle null filename in enhanced generator', () => {
      ...
      // Act
      const result = generator({ filename: null as any });
  ...
  describe('Error Scenarios', () => {
    it('should propagate R2 config errors in basic generators', () => {
      ...
      // Act & Assert
      expect(() => generateMediaURL({ filename: 'test.jpg' })).toThrow('R2 config error');
    });
  ...
  describe('Integration Scenarios', () => {
    it('should handle complete R2 workflow with mixed configurations', () => {
      ...
      expect(downloadsUrl).toBe('https://downloads-bucket.account123.r2.cloudflarestorage.com/file.pdf');
    });
    ...
    it('should handle complete S3 workflow with environment variables', () => {
      ...
      expect(downloadsUrl).toBe('https://my-bucket.s3.eu-west-1.amazonaws.com/downloads/archive.zip');
    });
    ...
    it('should work end-to-end with factory and enhanced generator', () => {
      ...
      // Assert
      expect(result).toBe('https://test-bucket.test-account.r2.cloudflarestorage.com/integration-test.jpg');
      expect(mockConsoleLog).toHaveBeenCalledWith(
