...
import { DocumentationDefinitionSchema } from '../data/schemas/documentation-definition.schema.js';
import { markdocToLexical } from '../utils/lexical-converter.js'; // Markdoc to Lexical converter
// Define __dirname for ES Modules
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
// Define the path to the documentation raw Markdoc files
const DOCUMENTATION_RAW_PATH = path.resolve(__dirname, '../data/raw/documentation/');
export async function seedDocumentation(payload, logger, cliArgs) {
    const collectionSlug = 'documentation'; // Adjust collection slug if different
    logger.info(`Starting seeder for collection: ${collectionSlug}...`);
    const idMap = {};
    // 1. Read all Markdoc files in the directory
    let mdocFiles;
    try {
        mdocFiles = await fs.readdir(DOCUMENTATION_RAW_PATH);
        logger.info(`Found ${mdocFiles.length} Markdoc files in ${DOCUMENTATION_RAW_PATH}`);
    }
    catch (error) {
        logger.error({ err: error, path: DOCUMENTATION_RAW_PATH }, `Failed to read directory: ${DOCUMENTATION_RAW_PATH}`);
    ...
    for (const file of mdocFiles) {
        ...
        const filePath = path.resolve(DOCUMENTATION_RAW_PATH, file);
        ...
        try {
            ...
            let validatedDocData;
            try {
                validatedDocData = DocumentationDefinitionSchema.parse(docData);
                logger.debug({ ssotId }, `Documentation data validated successfully for ${file}.`);
            }
            catch (error) {
                logger.error({ err: error, ssotId, file }, `Invalid documentation data structure for ${file}.`);
                throw error; // Propagate error
            }
            // 4. Convert Markdoc content to Lexical JSON
            const lexicalContent = await markdocToLexical(validatedDocData.content);
            // 5. Prepare data for Payload
            const slug = validatedDocData.slug; // Define slug variable
            const dataToCreate = {
                id: ssotId, // Use the SSOT ID (filename) as the Payload document ID
                slug: slug,
                title: validatedDocData.title,
                status: validatedDocData.status,
                description: validatedDocData.description,
                order: validatedDocData.order,
...
//# sourceMappingURL=seed-documentation.js.map