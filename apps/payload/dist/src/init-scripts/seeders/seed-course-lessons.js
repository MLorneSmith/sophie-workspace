...
import { LessonDefinitionsSchema } from '../data/schemas/lesson-definition.schema.js'; // Import lesson schema
import { generateSlug } from '../utils/slugify.js'; // Import slugify utility
import { markdocToLexical } from '../utils/lexical-converter.js'; // Import lexical converter utility
// ES Module __dirname
...
export async function seedCourseLessons(payload, logger, cliArgs) {
    const collectionSlug = 'course_lessons'; // Corrected slug
    logger.info(`Starting seeder for collection: ${collectionSlug}...`);
    const idMap = {}; // Maps SSOT lesson identifier (e.g., slug) to live DB UUID
    try {
        // 1. Load and Validate SSOT Data
        let ssotLessonDefs; // Use 'any' for initial load before validation
        try {
        ...
        for (const lessonDef of validatedLessonDefs) { // Explicitly assert type
            ...
            try {
                ...
                let lexicalContent; // TODO: Replace 'any' with actual Lexical JSON type
                if (lessonDef.content_file) {
                    ...
                    try {
                        ...
                        lexicalContent = await markdocToLexical(markdownContent); // Use the utility
                    }
                    catch (mdocError) {
...
//# sourceMappingURL=seed-course-lessons.js.map