...
import { QuizzesSchema } from '../data/schemas/quiz-definition.schema.js';
...
export async function seedQuizQuestions(payload, logger, cliArgs) {
    const collectionSlug = 'quiz_questions'; // Corrected slug
    logger.info(`Starting seeder for collection: ${collectionSlug}...`);
    const idMap = {}; // Maps SSOT question ID to live DB UUID
    const processedQuestionIds = new Set(); // To avoid processing duplicate questions if they appear in multiple quizzes
    try {
            ...
            for (const questionDef of quizDef.questions) {
                ...
                try {
                    ...
                    const dataToCreate = {
                        ...
                        options: (questionDef.options ?? []).map(option => ({
                            text: option.text,
                            isCorrect: option.isCorrect,
                        })), // Explicitly cast the mapped array
                        explanation: questionDef.explanation, // Lexical JSON object - Cast to any
...
//# sourceMappingURL=seed-quiz-questions.js.map