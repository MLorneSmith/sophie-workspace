...
import { CourseDefinitionsSchema } from '../data/schemas/course-definition.schema.js'; // Import course schema
...
export async function seedCourses(payload, logger, cliArgs) {
    ...
    const idMap = {}; // Maps SSOT course ID to live DB UUID
    try {
        // 1. Load and Validate SSOT Data
        let ssotCourses; // Use 'any' for initial load before validation
        try {
        ...
        for (const courseDef of validatedCourses) { // Explicitly assert type
