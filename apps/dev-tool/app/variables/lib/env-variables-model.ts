		...
		validate: ({ value, mode }) => {
			if (mode === "development") {
				return z
					...
					.url({
						message:
							"The NEXT_PUBLIC_SUPABASE_URL variable must be a valid URL",
					})
					.safeParse(value);
			}
		...
		validate: ({ value, variables }) => {
			return z
				...
				.refine(
					...
					{
						message:
							"The SUPABASE_SERVICE_ROLE_KEY variable must be different from NEXT_PUBLIC_SUPABASE_ANON_KEY",
					},
				)
				.safeParse(value);
		},
		...
		validate: ({ value, variables }) => {
			return z
				...
				.refine(
					...
					{
						message:
							"The SUPABASE_SECRET_KEY variable must be different from public keys",
					},
				)
				.optional()
				.safeParse(value);
			...
			validate: ({ value }) => {
				return z
					...
					.refine(
						...
						{
							message:
								"The NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY variable must start with pk_",
						},
					)
					.safeParse(value);
			},
			...
			validate: ({ value }) => {
				return z
					...
					.refine(
						...
						{
							message:
								"The STRIPE_SECRET_KEY variable must start with sk_ or rk_",
						},
					)
					.safeParse(value);
			},
			...
			validate: ({ value }) => {
				return z
					...
					.refine(
						...
						{
							message:
								"The STRIPE_WEBHOOK_SECRET variable must start with whsec_",
						},
					)
					.safeParse(value);
			},
