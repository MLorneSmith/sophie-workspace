...
export async function loadDatabaseToolsData() {
	...
	try {
		...
		const functions: DatabaseFunction[] = functionsResponse.map(
			(
				func: Partial<DatabaseFunction> & { name: string; returnType: string },
			) => {
		...
		const enums: DatabaseEnum[] = Object.entries(enumsResponse).map(
			([name, data]: [string, { values?: string[]; source?: string }]) => {
