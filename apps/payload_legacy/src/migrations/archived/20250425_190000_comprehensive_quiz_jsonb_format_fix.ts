...
/**
 * Migration 20250425_190000_comprehensive_quiz_jsonb_format_fix.ts
 *
...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  try {
    ...
    const initialCheck = await db.execute(sql`
      SELECT 
        ...
        COUNT(*) FILTER (WHERE 
          ...
          jsonb_array_elements(questions) ? 'relationTo'
        ) as formatted_count
      ...
      WITH quiz_questions AS (
        SELECT 
          ...
          jsonb_agg(
            ...
            ORDER BY order_field  -- Maintain order if possible
    ...
    await db.execute(sql`
      WITH quizzes_with_unlinked_questions AS (
        SELECT 
      ...
      SET questions = (
        SELECT jsonb_agg(
          jsonb_build_object(
            'id', question_id,
            'relationTo', 'quiz_questions',
            'value', jsonb_build_object('id', question_id)
          )
        )
        FROM jsonb_array_elements_text(uwq.questions) AS question_id
      )
      FROM quizzes_with_unlinked_questions uwq
      WHERE q.id::text = uwq.quiz_id::text
    `)
    ...
    const finalCheck = await db.execute(sql`
      SELECT 
        ...
        COUNT(*) FILTER (WHERE 
          jsonb_typeof(questions) = 'array' AND 
          jsonb_array_length(questions) > 0 AND
          jsonb_path_exists(questions, '$[*].relationTo')
        ) as formatted_count
      FROM payload.course_quizzes
      WHERE questions IS NOT NULL
    `)
      ...
      BEGIN
        ...
        SELECT 
          ...
          CASE 
            ...
            WHEN NOT jsonb_path_exists(questions, '$[*].relationTo') THEN false
            ELSE true
          END as is_formatted,
          CASE 
