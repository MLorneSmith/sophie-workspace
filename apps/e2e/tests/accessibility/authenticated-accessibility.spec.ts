...
test.describe('Authenticated Accessibility Tests - WCAG 2.1 AA', () => {
  ...
  test('Dashboard accessibility after authentication', async ({ page }) => {
    ...
    test.skip(!process.env.TEST_USER_EMAIL, 'No test user credentials available');
    ...
    try {
      ...
      console.log('Skipping authenticated test - auth not available:', error.message);
      test.skip();
    }
  });
  ...
  test('Account settings accessibility when authenticated', async ({ page }) => {
    test.skip(!process.env.TEST_USER_EMAIL, 'No test user credentials available');
