...
export async function generateCertificate({ userId, courseId, fullName, }) {
    ...
    if (!fs.existsSync(templatePath)) {
        ...
        throw new Error(`Certificate template file not found at path: ${templatePath}`);
    }
    ...
    if (fieldInfo.info?.FieldsInfo?.Fields) {
        ...
        const nameField = fields.find((field) => field.FieldName.toLowerCase().includes('name') ||
            field.Type === 'EditBox');
        if (nameField) {
    ...
    try {
        ...
        if (!certificatesBucket) {
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    if (error) {
                        ...
                        console.error(`Attempt ${retryCount + 1}: Failed to create certificates bucket:`, error.message);
                        retryCount++;
                    ...
                    createBucketError = error;
                    console.error(`Attempt ${retryCount + 1}: Exception creating certificates bucket:`, error);
                    retryCount++;
                    // Wait a bit before retrying
                    await new Promise((resolve) => setTimeout(resolve, 1000));
                }
            ...
            if (createBucketError) {
                console.error('Failed to create certificates bucket after multiple attempts:', createBucketError);
                throw new Error(`Failed to create certificates bucket: ${createBucketError?.message || String(createBucketError)}`);
            }
        }
        else {
    ...
    const { data: certificateData, error: certificateError } = await supabase.rpc('insert_certificate', {
        p_user_id: userId,
        p_course_id: courseId,
        p_file_path: fileName,
    });
    if (certificateError) {
        throw new Error(`Failed to store certificate information: ${certificateError.message}`);
    }
    // Get the certificate ID from the returned data
    const certificateId = Array.isArray(certificateData) && certificateData.length > 0
