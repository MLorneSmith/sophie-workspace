/**
 * Unit tests for QuizComponent - Quiz interface with scoring and navigation logic
 * Tests component initialization, question handling, scoring algorithms, and user interactions
 */
...
vi.mock('@kit/ui/card', () => ({
  ...
  CardTitle: ({ children, className, ...props }: any) => <h2 data-testid="card-title" className={className} {...props}>{children}</h2>,
}));
...
describe('QuizComponent', () => {
  ...
  describe('Component Initialization & Validation', () => {
    ...
    it('shows quiz passed message for successful previous attempts', () => {
      ...
      render(<QuizComponent {...defaultProps} previousAttempts={previousAttempts} />);
    ...
    it('renders questions unavailable when no questions exist', () => {
      ...
      expect(screen.getByText('Quiz Questions Unavailable')).toBeInTheDocument();
    });
  ...
  describe('Quiz Completion & Summary', () => {
    ...
    it('shows passed state for score >= passing score', async () => {
      ...
      expect(screen.getByText('Congratulations! 🎉')).toBeInTheDocument();
      expect(screen.getByText('You have successfully passed this quiz!')).toBeInTheDocument();
    ...
    it('shows failed state for score < passing score', async () => {
      ...
      expect(screen.getByText('You did not pass this quiz. Please try again.')).toBeInTheDocument();
  ...
  describe('Edge Cases & Error Handling', () => {
    it('handles malformed question options', () => {
      const malformedQuiz = {
        ...
        questions: [{
          id: 'q1',
          question: 'Test question',
          options: [null, undefined, { text: 'Valid option', isCorrect: true }],
        }],
      };
    ...
    it('uses default passing score when not provided', () => {
      ...
      render(<QuizComponent {...defaultProps} quiz={quizWithoutPassingScore} />);
    ...
    it('handles undefined options property', () => {
      ...
      // Should render without crashing
      expect(screen.getByText('Question with undefined options')).toBeInTheDocument();
    });
  });
});