 ...
 */
function convertSubmissionData(submission: SubmissionData, lexicalToTiptapFn: (data: any) => any): {
  convertedData: Record<string, string | null>;
  error?: string;
} {
  try {
    const convertedData = {
      situation: submission.situation !== null && submission.situation !== undefined
        ? JSON.stringify(lexicalToTiptapFn(submission.situation))
        : null,
      complication: submission.complication !== null && submission.complication !== undefined
        ? JSON.stringify(lexicalToTiptapFn(submission.complication))
        : null,
      answer: submission.answer !== null && submission.answer !== undefined
        ? JSON.stringify(lexicalToTiptapFn(submission.answer))
        : null,
      outline: submission.outline !== null && submission.outline !== undefined
...
function processSubmissions(
  ...
  for (const submission of submissions) {
    const { convertedData, error } = convertSubmissionData(submission, lexicalToTiptapFn);
...
describe('Data Conversion Business Logic', () => {
  ...
  describe('convertSubmissionData', () => {
    it('should convert all content fields correctly', () => {
      ...
      const mockTiptapResult = { type: 'doc', content: [{ type: 'paragraph', content: [] }] };
    ...
    it('should handle lexicalToTiptap conversion errors', () => {
      ...
      // Assert
      expect(result.error).toBe('ID sub-conversion-error: Conversion failed for malformed data');
      expect(result.convertedData).toEqual({});
    });
  ...
  describe('processSubmissions', () => {
    ...
    it('should handle mixed success/failure scenarios', () => {
      // Arrange
      const submissions: SubmissionData[] = [
        { id: 'sub-success-1', situation: '{"data":"1"}', complication: null, answer: null, outline: null },
        { id: 'sub-fail', situation: '{"data":"2"}', complication: null, answer: null, outline: null },
        { id: 'sub-success-2', situation: '{"data":"3"}', complication: null, answer: null, outline: null },
      ];
    ...
    it('should process large batch efficiently', () => {
      // Arrange
      const batchSize = 100;
      const submissions: SubmissionData[] = Array.from({ length: batchSize }, (_, i) => ({
        id: `sub-large-${i + 1}`,
        situation: `{"data":"${i + 1}"}`,
        complication: null,
        answer: null,
    ...
    it('should accurately track counts with various error patterns', () => {
      // Arrange
      const submissions: SubmissionData[] = [
        { id: 'sub-1', situation: '{"good":"1"}', complication: null, answer: null, outline: null },
        { id: 'sub-2', situation: '{"bad":"2"}', complication: null, answer: null, outline: null },
        { id: 'sub-3', situation: '{"good":"3"}', complication: null, answer: null, outline: null },
        { id: 'sub-4', situation: '{"bad":"4"}', complication: null, answer: null, outline: null },
        { id: 'sub-5', situation: '{"good":"5"}', complication: null, answer: null, outline: null },
      ];
    ...
    it('should preserve submission ID in error messages', () => {
      // Arrange
      const submissions: SubmissionData[] = [
        { id: 'error-submission-123', situation: '{"error":"data"}', complication: null, answer: null, outline: null },
      ];
      ...
      // Assert
      expect(result.failed).toBe(1);
      expect(result.errors[0]).toBe('ID error-submission-123: Specific conversion error');
    });
  });
  ...
  describe('JSON Serialization', () => {
    it('should properly serialize converted Tiptap documents', () => {
      ...
      // Assert
      expect(result.error).toBeUndefined();
      expect(result.convertedData.situation).toBe(JSON.stringify(mockTiptapResult));
  ...
  describe('Edge Cases & Integration', () => {
    ...
    it('should maintain data integrity throughout conversion process', () => {
      // Arrange
      const originalData = '{"original":{"data":"value","number":123,"boolean":true}}';
      const submission: SubmissionData = {
        id: 'sub-integrity',
      ...
      const tiptapResult = { type: 'doc', content: [{ type: 'paragraph', content: [] }] };
