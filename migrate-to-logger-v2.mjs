...
function isReactComponent(content) {
  ...
  return jsxPattern.test(content) || fragmentPattern.test(content) || hookPattern.test(content);
}

// Detect if a function is async
function isAsyncFunction(functionStr) {
  return /^async\s+/.test(functionStr.trim()) || /\basync\s+function/.test(functionStr) || /\basync\s*\(/.test(functionStr);
}
...
function getEnclosingFunction(content, position) {
  ...
  for (const pattern of functionPatterns) {
    const matches = [...beforePosition.matchAll(new RegExp(pattern.source, 'gm'))];
    if (matches.length > 0) {
...
function processReactConsole(content, consoleMatch, fullMatch) {
  ...
  const inUseEffect = /useEffect\s*\(\s*\(\s*\)\s*=>\s*{[^}]*$/.test(beforeMatch);
  ...
  if (inUseEffect) {
    ...
    const functionContext = getEnclosingFunction(content, content.indexOf(fullMatch));
...
async function processServerConsole(content) {
  ...
  if (loggerDeclarations.size > 0) {
    ...
    const sortedDeclarations = [...loggerDeclarations.entries()].sort((a, b) => b[1].position - a[1].position);
    
    for (const [key, { position, declaration }] of sortedDeclarations) {
      // Find the opening brace of the function
      const afterDeclaration = processed.substring(position + declaration.length);
      const braceMatch = afterDeclaration.match(/^\s*{/);
      
      if (braceMatch) {
        const insertPosition = position + declaration.length + braceMatch.index + braceMatch[0].length;
...
function addLoggerImport(content, hasLogger) {
  ...
  if (content.includes('getLogger') && content.includes(config.loggerImportPath)) {
    return content;
  }
  ...
  let importStatement = `import { getLogger } from '${config.loggerImportPath}';\n`;
  ...
  try {
    ...
    if (isReactComponent(content)) {
      ...
      processed = content.replace(consolePattern, (fullMatch, method, args) => {
        hasLogger = true;
        return processReactConsole(content, [fullMatch, method, args], fullMatch);
      });
    } else {
