...
class E2ETestRunner {
	...
	async runShard(shard, status) {
		return new Promise((resolve) => {
			...
			proc.on("close", (code) => {
				...
				if (code === 0) {
					console.log(
						`  ✅ Shard ${shard.id} completed: ${results.passed}/${results.total} passed (${duration}s)`,
					);
				} else {
					console.log(
						`  ❌ Shard ${shard.id} failed: ${results.failed} failures (${duration}s)`,
					);
				}
	...
	parseShardResults(output) {
		...
		if (
			output.includes("webServer timeout") ||
			output.includes("WebServer was not able to start")
		) {
	...
	aggregateResults(shardResults, _status) {
		const totals = {
			total: 0,
			passed: 0,
			failed: 0,
...
class TestController {
	...
	async generateFinalReport() {
		...
		console.log(
			`  Environment: ${this.status.status.infrastructure.environment}`,
		);
		...
		if (totalFailed === 0) {
			...
			if (e2e.infrastructureFailures > 0) {
				...
				console.log(
					"   3. Retry tests: node .claude/scripts/test-controller.js",
				);
			}
		}
