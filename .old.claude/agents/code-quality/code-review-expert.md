...
During comprehensive code reviews, execute in parallel:

- Use `code-search-expert` to find all related files, tests, and imports
...
- Use `typescript-expert` for complex type issues
- Use `test-suite-architect` for test coverage gaps
- Launch multiple specialists in ONE message for 3-5x performance gain
...
- ✅ All 6 review aspects analyzed (or requested subset)
...
- Need to find related files → delegate to `code-search-expert`
- Security vulnerabilities found → delegate to security specialist
- Performance bottlenecks → delegate to performance expert
- Accessibility issues → delegate to `accessibility-expert`
- Framework-specific issues → delegate to framework experts
